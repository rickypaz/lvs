...
class moodle1_mod_wikilv_handler extends moodle1_mod_handler {
    ...
    /** @var string initial content for creating first page from the optional 1.9 wikilv file */
     ...
     * defined.
     *
     * Note that the path /MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV does not
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'wikilv', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV',
                array(
                    'newfields' => array(
                        'introformat' => '0',
                    ...
                    'dropfields' => array(
                        'pagename', 'scaleid', 'ewikilvprinttitle', 'htmlmode', 'ewikilvacceptbinary', 'disablecamelcase',
                        'setpageflags', 'strippages', 'removepages', 'revertchanges'
                    )
                )
            ),
            new convert_path(
                'wikilv_entries', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV/ENTRIES',
                array(
                    'newfields' => array(
                        'synonyms' => '0',
                        'links' => 'collaborative',
            ...
            new convert_path(
                'wikilv_entry', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV/ENTRIES/ENTRY'
            ),
            new convert_path(
                'wikilv_pages', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV/ENTRIES/ENTRY/PAGES'
            ),
            new convert_path(
                'wikilv_entry_page', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV/ENTRIES/ENTRY/PAGES/PAGE',
                array(
                    'newfields' => array(
    ...
    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/WIKILV
     * data available
     */
    public function process_wikilv($data) {
        global $CFG;    // We need to check a config setting.

        if (!empty($data['initialcontent'])) {
            //convert file in <INITIALCONTENT>filename</INITIALCONTENT> into a subwikilv page if no entry created.
        ...
        if ($data['wikilvmode'] !== 'group') {
            $data['wikilvmode'] = 'individual';
            //@todo need to create extra subwikilvs due to individual wikilvmode?
            //this would then need to reference the users in the course that is being restored.(some parent class API needed)
        } else {
            $data['wikilvmode'] = 'collaborative';
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_wikilv');
        ...
        $this->open_xml_writer("activities/wikilv_{$this->moduleid}/wikilv.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'wikilv', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('wikilv', array('id' => $instanceid));
    ...
    public function on_wikilv_entries_start() {
        $this->xmlwriter->begin_tag('subwikilvs');
        $this->needinitpage = false; //backup has entries, so the initial_content file has been stored as a page in 1.9.
    }

    public function on_wikilv_entries_end() {
        $this->xmlwriter->end_tag('subwikilvs');
    }

    public function process_wikilv_entry($data) {
        $this->xmlwriter->begin_tag('subwikilv', array('id' => $data['id']));
        unset($data['id']);
    ...
    public function on_wikilv_entry_end() {
        $this->xmlwriter->end_tag('subwikilv');
    }

    public function on_wikilv_pages_start() {
        $this->xmlwriter->begin_tag('pages');
    }

    public function on_wikilv_pages_end() {
        $this->xmlwriter->end_tag('pages');
    }

    public function process_wikilv_entry_page($data) {
    ...
    public function on_wikilv_entry_page_end() {
        $this->xmlwriter->end_tag('page');
    }

    /**
     * This is executed when we reach the closing </MOD> tag of our 'wikilv' path
     */
    public function on_wikilv_end() {
        global $USER;
        ...
        if ($this->initialcontentfilename && $this->needinitpage) {
            ...
            $data_page = array(
                'id'            => $this->converter->get_nextid(), //just creating the first page in the wikilv
            ...
            $this->on_wikilv_entries_start();
            $this->process_wikilv_entry($data_entry);
            $this->on_wikilv_pages_start();
            $this->process_wikilv_entry_page($data_page);
            $this->on_wikilv_entry_page_end();
            $this->on_wikilv_pages_end();
            $this->on_wikilv_entry_end();
            $this->on_wikilv_entries_end();
        }

        //close wikilv.xml
        $this->xmlwriter->end_tag('wikilv');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/wikilv_{$this->moduleid}/inforef.xml");
