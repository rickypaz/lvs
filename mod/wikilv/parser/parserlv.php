...
class wikilv_parser_proxy {
    ...
    public static function parse(&$string, $type, $options = array()) {
        ...
        if(empty(self::$basepath)) {
            global $CFG;
            self::$basepath = $CFG->dirroot.'/mod/wikilv/parser/';
        }
...
abstract class generic_parser_lv { // @LVS adição do sufixo lvs
    ...
    private $nowikilvindex = array();

    protected $nowikilvtoken = "%!";
    ...
    public function parse(&$string, $options = array()) {
        ...
        $this->initialize_nowikilv_index();
    ...
    /**
     * Special nowikilv parser index
     */

    private function initialize_nowikilv_index() {
        $token = "\Q".$this->nowikilvtoken."\E";
        $this->string = preg_replace_callback("/".$token."\d+".$token."/", array($this, "initialize_nowikilv_index_callback"), $this->string);
    }

    private function initialize_nowikilv_index_callback($match) {
        return $this->protect($match[0]);
    }

    protected function protect($text) {
        $this->nowikilvindex[] = $text;

        return $this->nowikilvtoken.(count($this->nowikilvindex)-1).$this->nowikilvtoken;
    }

    private function commit_nowikilv_index() {
        $token = "\Q".$this->nowikilvtoken."\E";
        $this->string = preg_replace_callback("/".$token."(\d+)".$token."/", array($this, "commit_nowikilv_index_callback"), $this->string);
    }

    private function commit_nowikilv_index_callback($match) {
        return $this->nowikilvindex[intval($match[1])];
    }
