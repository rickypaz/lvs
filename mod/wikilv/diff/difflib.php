 ...
 */
function ouwikilv_diff_internal($file1,$file2) {
	// Basic variables
	$n=count($file2);
	$m=count($file1);
	...
	for($i=1;$i<=$m;$i++) {
		...
		$P[$i]=ouwikilv_diff_find_last($V,$E,crc32($file1[$i]));
	}
	...
	for($i=1;$i<=$m;$i++) {
		if($P[$i]!==0) {
			ouwikilv_diff_merge($K,$k,$i,$E,$P[$i],$candidates);
		}
	}
...
function ouwikilv_diff_merge(&$K,&$k,$i,&$E,$p,&$candidates) {
	$r=0;
	$c=$K[0];
...
// From Step 2
function ouwikilv_diff_sort_v($a,$b) {
    if($a->hash < $b->hash) {
    	return -1;
...
// From Step 4
function ouwikilv_diff_find_last(&$V,&$E,$hash) {
...
class ouwikilv_line {
    ...
    public function __construct($data,$linepos) {
        ...
        while(true) {
            ...
            if($space2===false) {
                ...
                $this->words[]=new ouwikilv_word(substr($data,$pos),$pos+$linepos);
                break;
            } else {
                $this->words[]=new ouwikilv_word(substr($data,$pos,$space2-$pos),$pos+$linepos);
                $pos=$space2;
            }
        }
    }
     ...
     */
    public function ouwikilv_line($data, $linepos) {
     ...
     * @param array $lines Array of ouwikilv_line
...
class ouwikilv_word {
     ...
     * @deprecated since Moodle 3.1
     */
    public function ouwikilv_word($word, $start) {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
 ...
 * @return array Array of ouwikilv_line objects
 */
function ouwikilv_diff_html_to_lines($content) {
    ...
    while($pos<strlen($content)) {
        ...
        $line=new ouwikilv_line($linestr,$pos);
        if(!$line->is_empty()) {
            $lines[$index++]=$line;
        }
        $pos=$nextline+1;
 ...
 * two source files. 
 */ 
class ouwikilv_change_range {
    var $file1start,$file1count;
    var $file2start,$file2count;
}

/**
 * A more logical representation of the results from ouwikilv_internal_diff()
 */
class ouwikilv_changes {
    ...
    public function __construct($diff,$count2) {
        ...
        $this->adds=self::internal_find_deletes(
            ouwikilv_diff_internal_flip($diff,$count2),count($diff));
        ...
        foreach($diff as $index1=>$index2) {
            ...
            if($index2===0 && !in_array($index1,$this->deletes)) {
                if($inrange===-1) {
                    ...
                    $this->changes[$inrange]=new ouwikilv_change_range;
     ...
     * @deprecated since Moodle 3.1
     */
    public function ouwikilv_changes($diff, $count2) {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
...
/**
 * Flips around the array returned by ouwikilv_diff_internal
 ...
 */
function ouwikilv_diff_internal_flip($diff,$count2) {
    $flip=array();
    for($i=1;$i<=$count2;$i++) {
        $flip[$i]=0;
    }
 ...
 * differ.
 * @param array $lines1 Array of ouwikilv_line
 * @param array $lines2 Array of ouwikilv_line
 * @return array (deleted,added); deleted and added are arrays of ouwikilv_word with
 *   position numbers from $lines1 and $lines2 respectively 
 */
function ouwikilv_diff_words($lines1,$lines2) {
    // Prepare arrays
    $deleted=array();
    $added=array();
    // Get line difference
    $linediff=ouwikilv_diff(
        ouwikilv_line::get_as_strings($lines1),
        ouwikilv_line::get_as_strings($lines2));
    ...
    foreach($linediff->changes as $changerange) {
        ...
        // Run diff on strings
        $worddiff=ouwikilv_diff($file1strings,$file2strings);
...
/**
 * Runs diff and interprets results into ouwikilv_changes object.
 * @param array $file1 Array of lines in file 1. The first line in the file
 *   MUST BE INDEX 1 NOT ZERO!!
 * @param array $file2 Array of lines in file 2, again starting from 1.
 * @return ouwikilv_changes Object describing changes
 */
function ouwikilv_diff($file1,$file2) {
    return new ouwikilv_changes(ouwikilv_diff_internal($file1,$file2),count($file2));
}

/**
 * Adds HTML span elements to $html around the words listed in $words.
 * @param string $html HTML content
 * @param array $words Array of ouwikilv_word to mark
 * @param string $markerclass Name of class for span element
 * @return HTML with markup added
 */
function ouwikilv_diff_add_markers($html,$words,$markerclass,$beforetext,$aftertext) {
    ...
    $spanstart="<ouwikilv_diff_add_markers>";
    $pos=0;
    $result='';
    foreach($words as $word) {
        ...
        // Add word
        $result.=$spanstart.$word->word.'</ouwikilv_diff_add_markers>';
        // Update position
    ...
    $result=preg_replace('^</ouwikilv_diff_add_markers>(\s*)<ouwikilv_diff_add_markers>^','$1',$result);
    
    // Turn markers into proper span
    $result=preg_replace('^<ouwikilv_diff_add_markers>^',$beforetext.'<span class="'.$markerclass.'">',$result);
    $result=preg_replace('^</ouwikilv_diff_add_markers>^','</span>'.$aftertext,$result);
 ...
 */
function ouwikilv_diff_html($html1,$html2) {
    $lines1=ouwikilv_diff_html_to_lines($html1);
    $lines2=ouwikilv_diff_html_to_lines($html2);
    list($deleted,$added)=ouwikilv_diff_words($lines1,$lines2);
    $result1=ouwikilv_diff_add_markers($html1,$deleted,'ouw_deleted',
        '<strong class="accesshide">'.get_string('deletedbegins','wikilv').'</strong>',
        '<strong class="accesshide">'.get_string('deletedends','wikilv').'</strong>');
    $result2=ouwikilv_diff_add_markers($html2,$added,'ouw_added',
        '<strong class="accesshide">'.get_string('addedbegins','wikilv').'</strong>',
        '<strong class="accesshide">'.get_string('addedends','wikilv').'</strong>');
