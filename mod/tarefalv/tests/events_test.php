...
/**
 * Contains the event tests for the module tarefalv.
 *
...
require_once($CFG->dirroot . '/mod/tarefalv/tests/fixtures/event_mod_tarefalv_fixtures.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_events_testcase extends mod_tarefalv_base_testcase {
    ...
    public function test_base_event() {
        ...
        /** @var \mod_tarefalv_unittests\event\nothing_happened $event */
        $event = \mod_tarefalv_unittests\event\nothing_happened::create($data);
        $tarefalv = $event->get_tarefalv();
        $this->assertDebuggingCalled();
        $this->assertInstanceOf('tarefalv', $tarefalv);

        $event = \mod_tarefalv_unittests\event\nothing_happened::create($data);
        $event->set_tarefalv($tarefalv);
        $tarefalv2 = $event->get_tarefalv();
        $this->assertDebuggingNotCalled();
        $this->assertSame($tarefalv, $tarefalv2);
    }
    ...
    public function test_submission_created() {
        ...
        $event = \mod_tarefalv_unittests\event\submission_created::create($eventinfo);
        $event->trigger();
        ...
        try {
            \mod_tarefalv_unittests\event\submission_created::create($params);
        ...
        try {
            \mod_tarefalv_unittests\event\submission_created::create($subinfo);
    ...
    public function test_submission_updated() {
        ...
        $sink = $this->redirectEvents();
        $event = \mod_tarefalv_unittests\event\submission_updated::create($eventinfo);
        $event->trigger();
        $result = $sink->get_events();
        $event = reset($result);
        $sink->close();
    ...
    public function test_extension_granted() {
        ...
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\extension_granted', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'grant extension',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            $this->students[0]->id,
    ...
    public function test_submission_locked() {
        ...
        $tarefalv->lock_submission($this->students[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\submission_locked', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            ...
            'lock submission',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('locksubmissionforstudent', 'tarefalv', array('id' => $this->students[0]->id,
    ...
    public function test_identities_revealed() {
        ...
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\identities_revealed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $expected = array(
            $tarefalv->get_course()->id,
            ...
            'reveal identities',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('revealidentities', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_status_viewed() {
        ...
        $expected = array(
            ...
            'view',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewownsubmissionstatus', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_submission_status_updated() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\mod_tarefalv\event\submission_status_updated', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($submission->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $this->assertEquals(TAREFALV_SUBMISSION_STATUS_DRAFT, $event->other['newstatus']);
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'revert submission to draft',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('reverttodraftforstudent', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_marker_updated() {
        ...
        $sink = $this->redirectEvents();
        $tarefalv->testable_process_set_batch_marking_allocation($this->students[0]->id, $this->teachers[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\marker_updated', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        ...
        $expected = array(
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('setmarkerallocationforlog', 'tarefalv', array('id' => $this->students[0]->id,
    ...
    public function test_workflow_state_updated() {
        ...
        $sink = $this->redirectEvents();
        $tarefalv->testable_process_set_batch_marking_workflow_state($this->students[0]->id, TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\workflow_state_updated', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $this->assertEquals($this->editingteachers[0]->id, $event->userid);
        $this->assertEquals(TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW, $event->other['newstate']);
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'set marking workflow state',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('setmarkingworkflowstateforlog', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]), 'state' => TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW)),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
        ...
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $this->assertEquals($this->editingteachers[0]->id, $event->userid);
        $this->assertEquals(TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE, $event->other['newstate']);
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'set marking workflow state',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('setmarkingworkflowstateforlog', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]), 'state' => TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE)),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
        ...
        $tarefalv->testable_process_save_quick_grades($data);
        ...
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $this->assertEquals($this->editingteachers[0]->id, $event->userid);
        $this->assertEquals(TAREFALV_MARKING_WORKFLOW_STATE_INMARKING, $event->other['newstate']);
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'set marking workflow state',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('setmarkingworkflowstateforlog', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]), 'state' => TAREFALV_MARKING_WORKFLOW_STATE_INMARKING)),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_duplicated() {
        ...
        $tarefalv = $this->create_instance();
        $submission1 = $tarefalv->get_user_submission($this->students[0]->id, true, 0);
        $submission2 = $tarefalv->get_user_submission($this->students[0]->id, true, 1);
        $submission2->status = TAREFALV_SUBMISSION_STATUS_REOPENED;
        $tarefalv->testable_update_submission($submission2, $this->students[0]->id, time(), $tarefalv->get_instance()->teamsubmission);

        $sink = $this->redirectEvents();
        $notices = null;
        $tarefalv->copy_previous_attempt($notices);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\submission_duplicated', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($submission2->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->userid);
        $submission2->status = TAREFALV_SUBMISSION_STATUS_DRAFT;
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'submissioncopied',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            $tarefalv->testable_format_submission_for_log($submission2),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_unlocked() {
        ...
        $tarefalv->unlock_submission($this->students[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_tarefalv\event\submission_unlocked', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($tarefalv->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            ...
            'unlock submission',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('unlocksubmissionforstudent', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_submission_graded() {
        ...
        $data->grade = '50.0';
        $tarefalv->testable_apply_grade_to_user($data, $this->students[0]->id, 0);
        $grade = $tarefalv->get_user_grade($this->students[0]->id, false, 0);

        $events = $sink->get_events();
        $this->assertCount(3, $events);
        $event = $events[2];
        $this->assertInstanceOf('\mod_tarefalv\event\submission_graded', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'grade submission',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            $tarefalv->format_grade_for_log($grade),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->view('viewplugintarefalvsubmission');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\submission_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $this->assertEquals($submission->id, $event->objectid);
        $expected = array(
            $tarefalv->get_course()->id,
            ...
            'view submission',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewsubmissionforuser', 'tarefalv', $this->students[0]->id),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_feedback_viewed() {
        ...
        $grade = new stdClass();
        $grade->assignment = $tarefalv->get_instance()->id;
        $grade->userid = $this->students[0]->id;
        $gradeid = $DB->insert_record('tarefalv_grades', $grade);
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->view('viewplugintarefalvfeedback');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\feedback_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'view feedback',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewfeedbackforuser', 'tarefalv', $this->students[0]->id),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_grading_form_viewed() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\grading_form_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'view grading form',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewgradingformforstudent', 'tarefalv', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_grading_table_viewed() {
        ...
        $expected = array(
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewsubmissiongradingtable', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_submission_form_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->view('editsubmission');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\submission_form_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('editsubmission', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_submission_confirmation_form_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->view('submit');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\submission_confirmation_form_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewownsubmissionform', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_reveal_identities_confirmation_page_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->view('revealidentities');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\reveal_identities_confirmation_page_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            'view',
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewrevealidentitiesconfirm', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_statement_accepted() {
        ...
        set_config('submissionreceipts', false, 'tarefalv');
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\statement_accepted', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
    ...
    public function test_batch_set_workflow_state_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->testable_view_batch_set_workflow_state($this->students[0]->id);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\batch_set_workflow_state_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewbatchsetmarkingworkflowstate', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_batch_set_marker_allocation_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->testable_view_batch_markingallocation($this->students[0]->id);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_tarefalv\event\batch_set_marker_allocation_viewed', $event);
        $this->assertEquals($tarefalv->get_context(), $event->get_context());
        $expected = array(
            $tarefalv->get_course()->id,
            'tarefalv',
            ...
            'view.php?id=' . $tarefalv->get_course_module()->id,
            get_string('viewbatchmarkingallocation', 'tarefalv'),
            $tarefalv->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_override_created() {
        ...
        $this->assertInstanceOf('\mod_tarefalv\event\user_override_created', $event);
        $this->assertEquals(context_module::instance($tarefalv->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_created() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_tarefalv\event\group_override_created', $event);
        $this->assertEquals(context_module::instance($tarefalv->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_updated() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_tarefalv\event\user_override_updated', $event);
        $this->assertEquals(context_module::instance($tarefalv->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_updated() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_tarefalv\event\group_override_updated', $event);
        $this->assertEquals(context_module::instance($tarefalv->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_deleted() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $tarefalvinstance = $this->getDataGenerator()->create_module('tarefalv', array('course' => $course->id));
        $cm = get_coursemodule_from_instance('tarefalv', $tarefalvinstance->id, $course->id);
        $context = context_module::instance($cm->id);
        $tarefalv = new tarefalv($context, $cm, $course);

        // Create an override.
        $override = new stdClass();
        $override->tarefalv = $tarefalvinstance->id;
        $override->userid = 2;
        $override->id = $DB->insert_record('tarefalv_overrides', $override);

        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $tarefalv->delete_override($override->id);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_tarefalv\event\user_override_deleted', $event);
    ...
    public function test_group_override_deleted() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_tarefalv\event\group_override_deleted', $event);
        $this->assertEquals(context_module::instance($cm->id), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
}
