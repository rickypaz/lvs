...
/**
 * Unit tests for (some of) mod/tarefalv/markerallocaion_test.php.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_tarefalv_markerallocation_testcase extends advanced_testcase {
    ...
    public function test_markerusers() {
        ...
        $tarefalvelement = $this->getDataGenerator()->create_module('tarefalv', $record, $modulesettings);

        $coursesectionid = course_add_cm_to_section($this->course->id, $tarefalvelement->id, 1);
        ...
        if (!empty($managerrole)) {
            // By default the context of the system is tarefalved.
            $idassignment = $this->getDataGenerator()->role_tarefalv($managerrole->id, $user4->id);
        }
        ...
        $oldmarkers = get_users_by_capability(context_course::instance($this->course->id), 'mod/tarefalv:grade', '', $sort);
        // New code, it must return 2 users: teacher1 and teacher2.
        $newmarkers = get_enrolled_users(context_course::instance($this->course->id), 'mod/tarefalv:grade', 0, 'u.*', $sort);
