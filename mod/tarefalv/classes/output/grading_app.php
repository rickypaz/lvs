...
class grading_app implements templatable, renderable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $export->caneditsettings = has_capability('mod/tarefalv:addinstance', $this->assignment->get_context());
        ...
        if ($export->duedate - $time <= 0) {
            ...
            $due = get_string('timeremainingcolon', 'tarefalv', format_time($export->duedate - $time));
        }
        ...
        if ($export->duedate < $time) {
            ...
            if ($cutoffdate) {
                if ($cutoffdate > $time) {
                    $late = get_string('latesubmissionsaccepted', 'tarefalv', userdate($export->cutoffdate));
                } else {
