...
define('TAREFALVFEEDBACK_FILE_FILEAREA', 'feedback_files');
define('TAREFALVFEEDBACK_FILE_BATCH_FILEAREA', 'feedback_files_batch');
define('TAREFALVFEEDBACK_FILE_IMPORT_FILEAREA', 'feedback_files_import');
define('TAREFALVFEEDBACK_FILE_MAXSUMMARYFILES', 5);
define('TAREFALVFEEDBACK_FILE_MAXSUMMARYUSERS', 5);
define('TAREFALVFEEDBACK_FILE_MAXFILEUNZIPTIME', 120);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_feedback_file extends tarefalv_feedback_plugin {
    ...
    public function get_name() {
        return get_string('file', 'tarefalvfeedback_file');
    }
    ...
    public function get_file_feedback($gradeid) {
        global $DB;
        return $DB->get_record('tarefalvfeedback_file', array('grade'=>$gradeid));
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (isset($filekey)) {
            ...
            $filecount = $this->count_files($grade->id, TAREFALVFEEDBACK_FILE_FILEAREA);
            if ($filecount != $draftareainfo['filecount']) {
                ...
                $files = $fs->get_area_files($this->assignment->get_context()->id,
                                     'tarefalvfeedback_file',
                                     TAREFALVFEEDBACK_FILE_FILEAREA,
                                     $grade->id,
                                     'id',
                                     false);
    ...
    public function update_file_count($grade) {
        ...
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, TAREFALVFEEDBACK_FILE_FILEAREA);
            return $DB->update_record('tarefalvfeedback_file', $filefeedback);
        } else {
            $filefeedback = new stdClass();
            $filefeedback->numfiles = $this->count_files($grade->id, TAREFALVFEEDBACK_FILE_FILEAREA);
            $filefeedback->grade = $grade->id;
            $filefeedback->assignment = $this->assignment->get_instance()->id;
            return $DB->insert_record('tarefalvfeedback_file', $filefeedback) > 0;
        }
    }
    ...
    public function view_summary(stdClass $grade, & $showviewlink) {
        ...
        $count = $this->count_files($grade->id, TAREFALVFEEDBACK_FILE_FILEAREA);

        // Show a view all link if the number of files is over this limit.
        $showviewlink = $count > TAREFALVFEEDBACK_FILE_MAXSUMMARYFILES;

        if ($count <= TAREFALVFEEDBACK_FILE_MAXSUMMARYFILES) {
            return $this->assignment->render_area_files('tarefalvfeedback_file',
                                                        TAREFALVFEEDBACK_FILE_FILEAREA,
                                                        $grade->id);
        } else {
            return get_string('countfiles', 'tarefalvfeedback_file', $count);
        }
    }
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('tarefalvfeedback_file',
    ...
    public function is_empty(stdClass $grade) {
        return $this->count_files($grade->id, TAREFALVFEEDBACK_FILE_FILEAREA) == 0;
    }
    ...
    public function get_file_areas() {
        return array(TAREFALVFEEDBACK_FILE_FILEAREA=>$this->get_name());
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        $filefeedback->grade = $grade->id;
        $filefeedback->assignment = $this->assignment->get_instance()->id;
        if (!$DB->insert_record('tarefalvfeedback_file', $filefeedback) > 0) {
            $log .= get_string('couldnotconvertgrade', 'mod_tarefalv', $grade->userid);
            return false;
        }
        return true;
    }
    ...
    public function get_grading_batch_operations() {
        return array('uploadfiles'=>get_string('uploadfiles', 'tarefalvfeedback_file'));
    }
    ...
    public function view_batch_upload_files($users) {
        ...
        require_capability('mod/tarefalv:grade', $this->assignment->get_context());
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/file/batchuploadfilesform.php');
        ...
        foreach ($users as $userid) {
            if ($usercount >= TAREFALVFEEDBACK_FILE_MAXSUMMARYUSERS) {
                $moreuserscount = count($users) - TAREFALVFEEDBACK_FILE_MAXSUMMARYUSERS;
                $usershtml .= get_string('moreusers', 'tarefalvfeedback_file', $moreuserscount);
                break;
            }
        ...
        $mform = new tarefalvfeedback_file_batch_upload_files_form(null, $formparams);
        ...
        if ($mform->is_cancelled()) {
            ...
            $data = file_postupdate_standard_filemanager($data,
                                                         ...
                                                         $this->assignment->get_context(),
                                                         'tarefalvfeedback_file',
                                                         TAREFALVFEEDBACK_FILE_BATCH_FILEAREA,
                                                         $USER->id);
            $fs = get_file_storage();
            ...
            foreach ($users as $userid) {
                ...
                if ($filefeedback) {
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 TAREFALVFEEDBACK_FILE_FILEAREA);
                    $DB->update_record('tarefalvfeedback_file', $filefeedback);
                } else {
                    $filefeedback = new stdClass();
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 TAREFALVFEEDBACK_FILE_FILEAREA);
                    $filefeedback->grade = $grade->id;
                    $filefeedback->assignment = $this->assignment->get_instance()->id;
                    $DB->insert_record('tarefalvfeedback_file', $filefeedback);
                }
            }
            ...
            $header = new tarefalv_header($this->assignment->get_instance(),
                                        $this->assignment->get_context(),
                                        false,
                                        $this->assignment->get_course_module()->id,
                                        get_string('batchuploadfiles', 'tarefalvfeedback_file'));
            $o = '';
            $o .= $this->assignment->get_renderer()->render($header);
            $o .= $this->assignment->get_renderer()->render(new tarefalv_form('batchuploadfiles', $mform));
    ...
    public function view_upload_zip() {
        ...
        require_capability('mod/tarefalv:grade', $this->assignment->get_context());
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/file/uploadzipform.php');
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/file/importziplib.php');
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/file/importzipform.php');

        $formparams = array('context'=>$this->assignment->get_context(),
                            'cm'=>$this->assignment->get_course_module()->id);
        $mform = new tarefalvfeedback_file_upload_zip_form(null, $formparams);
        ...
        $importer = new tarefalvfeedback_file_zip_importer();
        ...
        if ($mform->is_cancelled()) {
            ...
            $mform = new tarefalvfeedback_file_import_zip_form(null, $params);
            if ($mform->is_cancelled()) {
                $importer->delete_import_files($contextid);
                $urlparams = array('id'=>$this->assignment->get_course_module()->id,
                                   'action'=>'grading');
            ...
            $importer->delete_import_files($contextid);
                   ...
                   ($zipfile = $mform->save_stored_file('feedbackzip',
                                                        $contextid,
                                                        'tarefalvfeedback_file',
                                                        TAREFALVFEEDBACK_FILE_IMPORT_FILEAREA,
                                                        $USER->id,
                                                        '/',
                                                        'import.zip',
                                                        true))) {
            ...
            $header = new tarefalv_header($this->assignment->get_instance(),
                                        ...
                                        false,
                                        $this->assignment->get_course_module()->id,
                                        get_string('confirmuploadzip', 'tarefalvfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new tarefalv_form('confirmimportzip', $mform));
            ...
            $header = new tarefalv_header($this->assignment->get_instance(),
                                        ...
                                        false,
                                        $this->assignment->get_course_module()->id,
                                        get_string('uploadzip', 'tarefalvfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new tarefalv_form('uploadfeedbackzip', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function get_grading_actions() {
        return array('uploadzip'=>get_string('uploadzip', 'tarefalvfeedback_file'));
    }
