 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalvfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param tarefalv $assignment - The assignment instance
     ...
     * @param tarefalv_plugin $plugin - Set to the plugin that exported the file
     ...
     * @param tarefalv_plugin $plugin The matching plugin from the filename
    ...
    public function is_file_modified($assignment, $user, $plugin, $filename, $fileinfo) {
        ...
        } else if ($plugin->get_subtype() == 'tarefalvfeedback') {
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(TAREFALVFEEDBACK_FILE_MAXFILEUNZIPTIME);
     ...
     *
     * @param tarefalv $assignment - The assignment instance
     * @param tarefalv_feedback_file $fileplugin - The file feedback plugin
    ...
    public function import_zip_files($assignment, $fileplugin) {
            ...
            if ($this->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'tarefalvfeedback_file';
                        $newfilerecord->filearea = TAREFALVFEEDBACK_FILE_FILEAREA;
        ...
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/file/renderable.php');
        $importsummary = new tarefalvfeedback_file_import_summary($assignment->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $tarefalvrenderer = $assignment->get_renderer();
        $renderer = $PAGE->get_renderer('tarefalvfeedback_file');

        $o = '';

        $o .= $tarefalvrenderer->render(new tarefalv_header($assignment->get_instance(),
                                                        $assignment->get_context(),
                                                        false,
                                                        $assignment->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'tarefalvfeedback_file')));

        $o .= $renderer->render($importsummary);

        $o .= $tarefalvrenderer->render_footer();
        return $o;
    }
