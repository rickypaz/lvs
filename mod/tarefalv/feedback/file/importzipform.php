 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalvfeedback_file_import_zip_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'uploadzip', get_string('confirmuploadzip', 'tarefalvfeedback_file'));
            ...
            if ($importer->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($importer->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($assignment->is_blind_marking()) {
                        $userdesc = get_string('hiddenuser', 'tarefalv') .
                    ...
                    if (!$grade || !$exists) {
                        $updates[] = get_string('feedbackfileadded', 'tarefalvfeedback_file',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    } else {
                        $updates[] = get_string('feedbackfileupdated', 'tarefalvfeedback_file',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    }
                }
            }
        ...
        if (count($updates)) {
            ...
            $mform->addElement('html', get_string('nochanges', 'tarefalvfeedback_file'));
        }
