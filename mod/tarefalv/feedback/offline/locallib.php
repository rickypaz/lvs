 ...
 *
 *
 * @package   tarefalvfeedback_offline
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_feedback_offline extends tarefalv_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'tarefalvfeedback_offline');
    }
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified, $encoding = 'utf-8', $separator = 'comma') {
        ...
        $gradeimporter = new tarefalvfeedback_offline_grade_importer($importid, $this->assignment, $encoding, $separator);
        ...
        if (!$gradeimporter->init()) {
            $thisurl = new moodle_url('/mod/tarefalv/view.php', array('action'=>'viewpluginpage',
                                                                     'pluginsubtype'=>'tarefalvfeedback',
                                                                     'plugin'=>'offline',
                                                                     'pluginaction'=>'uploadgrades',
                                                                     'id' => $this->assignment->get_course_module()->id));
            print_error('invalidgradeimport', 'tarefalvfeedback_offline', $thisurl);
            return;
        }
        ...
        $o .= $renderer->render(new tarefalv_header($this->assignment->get_instance(),
                                                  $this->assignment->get_context(),
                                                  false,
                                                  $this->assignment->get_course_module()->id,
                                                  get_string('importgrades', 'tarefalvfeedback_offline')));
        $o .= $renderer->box(get_string('updatedgrades', 'tarefalvfeedback_offline', $updatecount));
    ...
    public function upload_grades() {
        ...
        require_capability('mod/tarefalv:grade', $this->assignment->get_context());
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/tarefalv/feedback/offline/importgradeslib.php');
        require_once($CFG->libdir . '/csvlib.class.php');

        $mform = new tarefalvfeedback_offline_upload_grades_form(null,
        ...
        if ($mform->is_cancelled()) {
            ...
            $importid = csv_import_reader::get_new_iid('tarefalvfeedback_offline');
            $gradeimporter = new tarefalvfeedback_offline_grade_importer($importid, $this->assignment,
            ...
            $mform = new tarefalvfeedback_offline_import_grades_form(null, array('assignment'=>$this->assignment,
                                                                       'csvdata'=>$csvdata,
            ...
            $o .= $renderer->render(new tarefalv_header($this->assignment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->assignment->get_course_module()->id,
                                                            get_string('confirmimport', 'tarefalvfeedback_offline')));
            $o .= $renderer->render(new tarefalv_form('confirmimport', $mform));
            $o .= $renderer->render_footer();
        } else if ($confirm) {
            ...
            $o .= $renderer->render(new tarefalv_header($this->assignment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->assignment->get_course_module()->id,
                                                            get_string('uploadgrades', 'tarefalvfeedback_offline')));
            $o .= $renderer->render(new tarefalv_form('batchuploadfiles', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function download_grades() {
        ...
        $filename = clean_filename(get_string('offlinegradingworksheet', 'tarefalvfeedback_offline') . '-' .
                                   $this->assignment->get_course()->shortname . '-' .
                                   $this->assignment->get_instance()->name . '-' .
                                   $groupname .
                                   $this->assignment->get_course_module()->id);

        $table = new tarefalv_grading_table($this->assignment, 0, '', 0, false, $filename);
    ...
    public function get_grading_actions() {
        return array('uploadgrades'=>get_string('uploadgrades', 'tarefalvfeedback_offline'),
                    'downloadgrades'=>get_string('downloadgrades', 'tarefalvfeedback_offline'));
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $gradingmanager = get_grading_manager($this->assignment->get_context(), 'mod_tarefalv', 'submissions');
