 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalvfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('tarefalv');
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'tarefalvfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($assignment->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'tarefalv') . $assignment->get_uniqueid_for_user($user->id);
            }
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'tarefalvfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'tarefalvfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'tarefalvfeedback_offline'));
        }
        ...
        $mform->addElement('hidden', 'pluginsubtype', 'tarefalvfeedback');
