 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalvfeedback_offline_grade_importer {
     ...
     * @param tarefalv $assignment The current assignment
     */
    public function __construct($importid, tarefalv $assignment, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'tarefalvfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'tarefalv');
        $strmodified = get_string('lastmodifiedgrade', 'tarefalv');
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'tarefalv')));
