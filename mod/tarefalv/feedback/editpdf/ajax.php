...
use \tarefalvfeedback_editpdf\combined_document;
...
if ($action === 'pollconversions') {
    ...
    if (!has_capability('mod/tarefalv:grade', $context)) {
        ...
        $readonly = true;
        $draft = false;
        require_capability('mod/tarefalv:submit', $context);
    }
    ...
    if ($added != count($page->comments)) {
        array_push($response->errors, get_string('couldnotsavepage', 'tarefalvfeedback_editpdf', $index+1));
    }
