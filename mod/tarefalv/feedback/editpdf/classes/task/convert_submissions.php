 ...
 */
namespace tarefalvfeedback_editpdf\task;

use core\task\scheduled_task;
use tarefalvfeedback_editpdf\document_services;
use tarefalvfeedback_editpdf\combined_document;
use context_module;
use tarefalv;
...
class convert_submissions extends scheduled_task {
    ...
    public function get_name() {
        return get_string('preparesubmissionsforannotation', 'tarefalvfeedback_editpdf');
    }
    ...
    public function execute() {
        ...
        $records = $DB->get_records('tarefalvfeedback_editpdf_queue');

        $assignmentcache = array();

        foreach ($records as $record) {
            $submissionid = $record->submissionid;
            $submission = $DB->get_record('tarefalv_submission', array('id' => $submissionid), '*', IGNORE_MISSING);
            if (!$submission) {
                // Submission no longer exists.
                $DB->delete_records('tarefalvfeedback_editpdf_queue', array('id' => $record->id));
                continue;
            }
            ...
            if (empty($assignmentcache[$assignmentid])) {
                $cm = get_coursemodule_from_instance('tarefalv', $assignmentid, 0, false, MUST_EXIST);
