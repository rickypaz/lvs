 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalvfeedback_editpdf_renderer extends plugin_renderer_base {
    ...
    private function render_toolbar_button($icon, $tool, $accesskey = null, $disabled=false) {
        ...
        $alttext->tool = get_string($tool, 'tarefalvfeedback_editpdf');
        if (!empty($accesskey)) {
            $alttext->shortcut = '(Alt/Shift-Alt/Ctrl-Option + ' . $accesskey . ')';
        } else {
            $alttext->shortcut = '';
        }
        $iconalt = get_string('toolbarbutton', 'tarefalvfeedback_editpdf', $alttext);

        $iconhtml = $this->image_icon($icon, $iconalt, 'tarefalvfeedback_editpdf');
     ...
     *
     * @param tarefalvfeedback_editpdf_widget $widget - Renderable widget containing assignment, user and attempt number.
     * @return string
     */
    public function render_tarefalvfeedback_editpdf_widget(tarefalvfeedback_editpdf_widget $widget) {
        global $CFG;

        $html = '';

        $html .= html_writer::div(get_string('jsrequired', 'tarefalvfeedback_editpdf'), 'hiddenifjs');
        $linkid = html_writer::random_id();
        if ($widget->readonly) {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('viewfeedbackonline', 'tarefalvfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        } else {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('launcheditor', 'tarefalvfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        }
        $links = $launcheditorlink;
        $html .= '<input type="hidden" name="tarefalvfeedback_editpdf_haschanges" value="false"/>';

        $html .= html_writer::div($links, 'visibleifjs');
        $header = get_string('pluginname', 'tarefalvfeedback_editpdf');
        $body = '';
        ...
        $iconalt = get_string('navigateprevious', 'tarefalvfeedback_editpdf');
        $iconhtml = $this->image_icon($nav_prev, $iconalt, 'tarefalvfeedback_editpdf');
        ...
        $navigation1 .= html_writer::tag('select', null, array('disabled'=>'true',
            'aria-label' => get_string('gotopage', 'tarefalvfeedback_editpdf'), 'class'=>'navigate-page-select',
            'accesskey' => $this->get_shortcut('navigate-page-select')));
        $iconalt = get_string('navigatenext', 'tarefalvfeedback_editpdf');
        $iconhtml = $this->image_icon($nav_next, $iconalt, 'tarefalvfeedback_editpdf');
        $navigation1 .= html_writer::tag('button', $iconhtml, array('disabled'=>'true',
        ...
        $progressbar = html_writer::div($progressbar, 'progress progress-info progress-striped active',
            array('title' => get_string('loadingeditor', 'tarefalvfeedback_editpdf'),
                  'role'=> 'progressbar', 'aria-valuenow' => 0, 'aria-valuemin' => 0,
                  'aria-valuemax' => 100));
        $progressbarlabel = html_writer::div(get_string('generatingpdf', 'tarefalvfeedback_editpdf'),
            'progressbarlabel');
        ...
        $changesmessage = html_writer::tag('div',
                                           get_string('draftchangessaved', 'tarefalvfeedback_editpdf'),
                                           array(
                                               'class' => 'tarefalvfeedback_editpdf_unsavedchanges warning label label-info'
                                           ));
        ...
        $this->page->requires->yui_module('moodle-tarefalvfeedback_editpdf-editor',
                                          'M.tarefalvfeedback_editpdf.editor.init',
                                          $editorparams);
