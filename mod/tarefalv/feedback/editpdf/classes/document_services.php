...
/**
 * This file contains the ingest manager for the tarefalvfeedback_editpdf plugin
 *
...
EOD;
    ...
    private static function get_assignment_from_param($assignment) {
        ...
        if (!is_object($assignment)) {
            $cm = get_coursemodule_from_instance('tarefalv', $assignment, 0, false, MUST_EXIST);
            $context = \context_module::instance($cm->id);

            $assignment = new \tarefalv($context, null, null);
        }
        return $assignment;
    }
            ...
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                ...
                foreach ($pluginfiles as $filename => $file) {
                    if ($file instanceof \stored_file) {
                        ...
                        $record->component = 'tarefalvfeedback_editpdf';
    ...
    public static function page_number_for_attempt($assignment, $userid, $attemptnumber, $readonly = false) {
        ...
        if ($readonly) {
            $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($assignment->get_context()->id, 'tarefalvfeedback_editpdf',
    ...
    protected static function generate_page_images_for_attempt($assignment, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('tarefalvfeedback_editpdf/pageimages/' . self::hash($assignment, $userid, $attemptnumber));
    ...
    public static function generate_feedback_document($assignment, $userid, $attemptnumber) {
        ...
        $tmpdir = make_temp_directory('tarefalvfeedback_editpdf/final/' . self::hash($assignment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        ...
        $fs = get_file_storage();
        $stamptmpdir = make_temp_directory('tarefalvfeedback_editpdf/stamps/' . self::hash($assignment, $userid, $attemptnumber));
        $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
     ...
     *
     * @param int|\tarefalv $assignment The assignment.
