...
class comments_quick_list {
    ...
    public static function get_comments() {
        ...
        $comments = array();
        $records = $DB->get_records('tarefalvfeedback_editpdf_quick', array('userid'=>$USER->id));
    ...
    public static function add_comment($commenttext, $width, $colour) {
        ...
        $comment->id = $DB->insert_record('tarefalvfeedback_editpdf_quick', $comment);
        return $comment;
    }
    ...
    public static function get_comment($commentid) {
        ...
        $record = $DB->get_record('tarefalvfeedback_editpdf_quick', array('id'=>$commentid), '*', IGNORE_MISSING);
        if ($record) {
            return $record;
        }
        return false;
    ...
    public static function remove_comment($commentid) {
        global $DB, $USER;
        return $DB->delete_records('tarefalvfeedback_editpdf_quick', array('id'=>$commentid, 'userid'=>$USER->id));
    }
}
