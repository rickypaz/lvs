...
/**
 * This file contains the editor class for the tarefalvfeedback_editpdf plugin
 *
...
namespace tarefalvfeedback_editpdf;
...
class page_editor {
    ...
    public static function get_comments($gradeid, $pageno, $draft) {
        ...
        $records = $DB->get_records('tarefalvfeedback_editpdf_cmnt', $params, 'y, x');
    ...
    public static function set_comments($gradeid, $pageno, $comments) {
        ...
        $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'pageno'=>$pageno, 'draft'=>1));
    ...
    public static function get_comment($commentid) {
        $record = $DB->get_record('tarefalvfeedback_editpdf_cmnt', array('id'=>$commentid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function add_comment(comment $comment) {
        global $DB;
        $comment->id = null;
        return $DB->insert_record('tarefalvfeedback_editpdf_cmnt', $comment);
    }
    ...
    public static function remove_comment($commentid) {
        global $DB;
        return $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('id'=>$commentid));
    }
    ...
    public static function get_annotations($gradeid, $pageno, $draft) {
        ...
        $annotations = array();
        $records = $DB->get_records('tarefalvfeedback_editpdf_annot', $params);
        foreach ($records as $record) {
    ...
    public static function set_annotations($gradeid, $pageno, $annotations) {
        ...
        $DB->delete_records('tarefalvfeedback_editpdf_annot', array('gradeid' => $gradeid, 'pageno' => $pageno, 'draft' => 1));
        $added = 0;
    ...
    public static function get_annotation($annotationid) {
        ...
        $record = $DB->get_record('tarefalvfeedback_editpdf_annot', array('id'=>$annotationid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function unrelease_drafts($gradeid) {
        ...
        $result = $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $result = $DB->delete_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0)) && $result;
        return $result;
    }
    ...
    public static function release_drafts($gradeid) {
        ...
        $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $DB->delete_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('tarefalvfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('tarefalvfeedback_editpdf_cmnt', $record);
        }
    ...
    public static function has_annotations_or_comments($gradeid, $includedraft) {
        ...
        if ($DB->count_records('tarefalvfeedback_editpdf_cmnt', $params)) {
            return true;
        }
        if ($DB->count_records('tarefalvfeedback_editpdf_annot', $params)) {
            return true;
        }
        return false;
    }
    ...
    public static function revert_drafts($gradeid) {
        ...
        // Delete the previous non-draft annotations and comments.
        $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        $DB->delete_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('tarefalvfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('tarefalvfeedback_editpdf_annot', $record);
    ...
    public static function add_annotation(annotation $annotation) {
        ...
        $annotation->id = null;
        return $DB->insert_record('tarefalvfeedback_editpdf_annot', $annotation);
    }
    ...
    public static function remove_annotation($annotationid) {
        ...
        return $DB->delete_records('tarefalvfeedback_editpdf_annot', array('id'=>$annotationid));
    }
    ...
    public static function copy_drafts_from_to($assignment, $grade, $sourceuserid) {
        ...
        $DB->delete_records('tarefalvfeedback_editpdf_annot', array('gradeid' => $grade->id));
        $DB->delete_records('tarefalvfeedback_editpdf_cmnt', array('gradeid' => $grade->id));
        ...
        $annotations = $DB->get_records('tarefalvfeedback_editpdf_annot', array('gradeid' => $sourceusergrade->id, 'draft' => 1));
        $comments = $DB->get_records('tarefalvfeedback_editpdf_cmnt', array('gradeid' => $sourceusergrade->id, 'draft' => 1));
        ...
        foreach ($annotations as $annotation) {
            ...
            $DB->insert_record('tarefalvfeedback_editpdf_annot', $annotation);
        }
        foreach ($comments as $comment) {
            $comment->gradeid = $grade->id;
            $DB->insert_record('tarefalvfeedback_editpdf_cmnt', $comment);
        }
    ...
    public static function delete_draft_content($gradeid) {
        ...
        $result = $DB->delete_records('tarefalvfeedback_editpdf_annot', $conditions);
        $result = $result && $DB->delete_records('tarefalvfeedback_editpdf_cmnt', $conditions);
        return $result;
    }
}
