...
require_once($CFG->dirroot.'/mod/tarefalv/feedback/editpdf/fpdi/fpdi.php');
...
class pdf extends \FPDI {
    ...
    const BLANK_PDF = '/mod/tarefalv/feedback/editpdf/fixtures/blank.pdf';
    ...
    public function append_comments($allcomments) {
        ...
        $this->setHeaderData('', 0, '', get_string('commentindex', 'tarefalvfeedback_editpdf'));
        ...
        foreach ($allcomments as $pageno => $comments) {
            foreach ($comments as $index => $comment) {
                ...
                $label = get_string('commentlabel', 'tarefalvfeedback_editpdf', array('pnum' => $pageno + 1, 'cnum' => $index + 1));
    ...
    public function get_image($pageno) {
        ...
        if ($generate) {
            ...
            if (!file_exists($imagefile)) {
                $fullerror = '<pre>'.get_string('command', 'tarefalvfeedback_editpdf')."\n";
                $fullerror .= $command . "\n\n";
                $fullerror .= get_string('result', 'tarefalvfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars($result) . "\n\n";
                $fullerror .= get_string('output', 'tarefalvfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars(implode("\n", $output)) . '</pre>';
                throw new \moodle_exception('errorgenerateimage', 'tarefalvfeedback_editpdf', '', $fullerror);
            }
        }
    ...
    public static function get_error_image($errorimagefolder, $pageno) {
        ...
        $pdf->copy_page();
        $pdf->add_comment(get_string('errorpdfpage', 'tarefalvfeedback_editpdf'), 250, 300, 200, "red");
    ...
    public static function test_gs_path($generateimage = true) {
        ...
        $testfile = $CFG->dirroot.'/mod/tarefalv/feedback/editpdf/tests/fixtures/testgs.pdf';
        if (!file_exists($testfile)) {
            $ret->status = self::GSPATH_NOTESTFILE;
            return $ret;
        }

        $testimagefolder = \make_temp_directory('tarefalvfeedback_editpdf_test');
