 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_tarefalvfeedback_editpdf_subplugin extends backup_subplugin {
    ...
    protected function define_grade_subplugin_structure() {
        ...
        $subpluginelementfiles->set_source_sql('SELECT id AS gradeid from {tarefalv_grades} where id = :gradeid', array('gradeid' => backup::VAR_PARENTID));
        $subpluginelementannotation->set_source_table('tarefalvfeedback_editpdf_annot', array('gradeid' => backup::VAR_PARENTID));
        $subpluginelementcomment->set_source_table('tarefalvfeedback_editpdf_cmnt', array('gradeid' => backup::VAR_PARENTID));
        // We only need to backup the files in the final pdf area, and the readonly page images - the others can be regenerated.
        $subpluginelementfiles->annotate_files('tarefalvfeedback_editpdf',
            \tarefalvfeedback_editpdf\document_services::FINAL_PDF_FILEAREA, 'gradeid');
        $subpluginelementfiles->annotate_files('tarefalvfeedback_editpdf',
            \tarefalvfeedback_editpdf\document_services::PAGE_IMAGE_READONLY_FILEAREA, 'gradeid');
        $subpluginelementfiles->annotate_files('tarefalvfeedback_editpdf', 'stamps', 'gradeid');
        return $subplugin;
    }
