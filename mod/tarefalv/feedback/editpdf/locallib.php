 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_feedback_editpdf extends tarefalv_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'tarefalvfeedback_editpdf');
    }
     ...
     * @param bool $readonly
     * @return tarefalvfeedback_editpdf_widget
     */
    public function get_widget($userid, $grade, $readonly) {
        ...
        $widget = new tarefalvfeedback_editpdf_widget($this->assignment->get_instance()->id,
                                                    $userid,
                                                    $attempt,
                                                    $url,
                                                    $filename,
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $renderer = $PAGE->get_renderer('tarefalvfeedback_editpdf');

        $widget = $this->get_widget($userid, $grade, false);

        $html = $renderer->render($widget);
        $mform->addElement('static', 'editpdf', get_string('editpdf', 'tarefalvfeedback_editpdf'), $html);
        $mform->addHelpButton('editpdf', 'editpdf', 'tarefalvfeedback_editpdf');
    ...
    public function view(stdClass $grade) {
        ...
        if (page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->assignment->render_area_files('tarefalvfeedback_editpdf',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('tarefalvfeedback_editpdf_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('tarefalvfeedback_editpdf_annot', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('tarefalv_grades', array('assignment'=>$this->assignment->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('tarefalvfeedback_editpdf_annot', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('tarefalvfeedback_editpdf_cmnt', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $testpath = tarefalvfeedback_editpdf\pdf::test_gs_path(false);
            $this->enabledcache = ($testpath->status == tarefalvfeedback_editpdf\pdf::GSPATH_OK);
        }
