 ...
 *
 * This class provides all the functionality for the new tarefalv module.
 *
...
define('TAREFALV_SUBMISSION_STATUS_NEW', 'new');
define('TAREFALV_SUBMISSION_STATUS_REOPENED', 'reopened');
define('TAREFALV_SUBMISSION_STATUS_DRAFT', 'draft');
define('TAREFALV_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('TAREFALV_FILTER_SUBMITTED', 'submitted');
define('TAREFALV_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('TAREFALV_FILTER_SINGLE_USER', 'singleuser');
define('TAREFALV_FILTER_REQUIRE_GRADING', 'require_grading');
define('TAREFALV_FILTER_GRANTED_EXTENSION', 'granted_extension');

// Marker filter for grading page.
define('TAREFALV_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('TAREFALV_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('TAREFALV_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('TAREFALV_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('TAREFALV_UNLIMITED_ATTEMPTS', -1);

// Special value means no grade has been set.
define('TAREFALV_GRADE_NOT_SET', -1);

// Grading states.
define('TAREFALV_GRADING_STATUS_GRADED', 'graded');
define('TAREFALV_GRADING_STATUS_NOT_GRADED', 'notgraded');

// Marking workflow states.
define('TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('TAREFALV_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('TAREFALV_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('TAREFALV_MARKING_WORKFLOW_STATE_RELEASED', 'released');

/** TAREFALV_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("TAREFALV_MAX_EVENT_LENGTH", "432000");

// Name of file area for intro attachments.
define('TAREFALV_INTROATTACHMENT_FILEAREA', 'introattachment');

// Event types.
define('TAREFALV_EVENT_TYPE_DUE', 'due');
define('TAREFALV_EVENT_TYPE_GRADINGDUE', 'gradingdue');
define('TAREFALV_EVENT_TYPE_OPEN', 'open');
define('TAREFALV_EVENT_TYPE_CLOSE', 'close');

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/tarefalv/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/tarefalv/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/tarefalv/submissionplugin.php');
require_once($CFG->dirroot . '/mod/tarefalv/renderable.php');
require_once($CFG->dirroot . '/mod/tarefalv/gradingtable.php');
require_once($CFG->libdir . '/eventslib.php');
require_once($CFG->libdir . '/portfolio/caller.php');

use \mod_tarefalv\output\grading_app;

/**
 * Standard base class for mod_tarefalv (assignment types).
 *
 * @package   mod_tarefalv
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class tarefalv {
    ...
    /** @var stdClass the assignment record that contains the global settings for this tarefalv instance */
    private $instance;

    /** @var grade_item the grade_item record for this tarefalv instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this tarefalv instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this tarefalv instance belongs to */
    private $course;

    /** @var stdClass the admin config for all tarefalv instances  */
    private $adminconfig;

    /** @var tarefalv_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this tarefalv instance */
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('tarefalvsubmission');
        $this->feedbackplugins = $this->load_plugins('tarefalvfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_tarefalv_useridlist)) {
            $SESSION->mod_tarefalv_useridlist = [];
        }
    }
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new moodle_url('/mod/tarefalv/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new moodle_url('/mod/tarefalv/index.php', array('id' => $this->get_course()->id));
        }
     ...
     *
     * @param string $subtype tarefalvsubmission | tarefalvfeedback
     * @param string $type
     * @return mixed tarefalv_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('tarefalv'));
     ...
     * @return mixed tarefalv_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('tarefalvfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed tarefalv_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('tarefalvsubmission', $type);
    }
    ...
    public function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('tarefalv'));
                $pluginclass = 'tarefalv_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof tarefalv_plugin) {
    ...
    public function view($action='', $args = array()) {
        ...
        if (!empty($action)) {
            $PAGE->set_pagetype('mod-tarefalv-' . $action);
        }
        // Now show the right view page.
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/tarefalv/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'tarefalv');
        ...
        } else if ($action == 'viewplugintarefalvfeedback') {
            $o .= $this->view_plugin_content('tarefalvfeedback');
        } else if ($action == 'viewplugintarefalvsubmission') {
            $o .= $this->view_plugin_content('tarefalvsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'tarefalv'), $notices);
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = TAREFALV_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('tarefalv', $update);
        $this->instance = $DB->get_record('tarefalv', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        if ($callplugins) {
            ...
            if (!empty($formdata->completionexpected)) {
                \core_completion\api::update_completion_date_event($formdata->coursemodule, 'tarefalv', $this->instance,
        ...
        $DB->update_record('tarefalv', $update);
    ...
    protected function delete_grades() {
        ...
        $result = grade_update('mod/tarefalv',
                               $this->get_course()->id,
                               'mod',
                               'tarefalv',
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('tarefalv_submission', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('tarefalv_grades', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('tarefalv_plugin_config', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('tarefalv_user_flags', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('tarefalv_user_mapping', array('assignment' => $this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('tarefalv', array('id'=>$this->get_instance()->id));

        return $result;
    }

    /**
     * Deletes a tarefalv override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        if (empty($cm)) {
            ...
            $cm = get_coursemodule_from_instance('tarefalv', $instance->id, $instance->course);
        }

        $override = $DB->get_record('tarefalv_overrides', array('id' => $overrideid), '*', MUST_EXIST);

        // Delete the events.
        $conds = array('modulename' => 'tarefalv', 'instance' => $this->get_instance()->id);
        ...
        $DB->delete_records('tarefalv_overrides', array('id' => $overrideid));
        ...
        $params = array(
            ...
            'other' => array(
                'tarefalvid' => $override->tarefalvid
            )
        );
        // Determine which override deleted event to fire.
        if (!empty($override->userid)) {
            $params['relateduserid'] = $override->userid;
            $event = \mod_tarefalv\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_tarefalv\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('tarefalv_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all tarefalv overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('tarefalv_overrides', array('tarefalvid' => $this->get_instance()->id), 'id');
    ...
    /**
     * Updates the tarefalv properties with override information for a user.
     *
     * Algorithm:  For each tarefalv setting, if there is a matching user-specific override,
     *   then use that otherwise, if there are group-specific overrides, return the most
     *   lenient combination of them.  If neither applies, leave the tarefalv setting unchanged.
     *
    ...
    /**
     * Returns whether an tarefalv has any overrides.
     *
    ...
    public function has_overrides() {
        ...
        $override = $DB->record_exists('tarefalv_overrides', array('tarefalvid' => $this->get_instance()->id));
     ...
     *   lowest sort order. If neither applies, leave the tarefalv setting unchanged.
     *
     * @param int $userid The userid.
    ...
    public function override_exists($userid) {
        ...
        $getuseroverride = function($userid) use ($DB) {
            $useroverride = $DB->get_record('tarefalv_overrides', ['tarefalvid' => $this->get_instance()->id, 'userid' => $userid]);
        ...
        $getgroupoverride = function($userid) use ($DB) {
            ...
            $sql = "SELECT * FROM {tarefalv_overrides}
                    WHERE groupid $extra AND tarefalvid = ? ORDER BY sortorder ASC";
    ...
    public function is_override_calendar_event(\calendar_event $event) {
        ...
        if ($DB->get_record('tarefalv_overrides', $overrideparams)) {
            return true;
        } else {
            return false;
        }
    ...
    function get_valid_calendar_event_timestart_range(\calendar_event $event) {
        ...
        if ($event->eventtype == TAREFALV_EVENT_TYPE_DUE) {
            ...
            if ($submissionsfromdate) {
                $mindate = [
                    $submissionsfromdate,
                    get_string('duedatevalidation', 'tarefalv'),
                ];
            }
            ...
            if ($cutoffdate) {
                $maxdate = [
                    $cutoffdate,
                    get_string('cutoffdatevalidation', 'tarefalv'),
                ];
            }
            ...
            if ($gradingduedate) {
                ...
                if (!$cutoffdate || $gradingduedate < $cutoffdate) {
                    $maxdate = [
                        $gradingduedate,
                        get_string('gradingdueduedatevalidation', 'tarefalv'),
                    ];
                }
            }
            ...
            if ($duedate) {
                ...
                $mindate = [
                    $submissionsfromdate,
                    get_string('gradingduefromdatevalidation', 'tarefalv'),
                ];
            }
        }
    ...
    public function reset_userdata($data) {
        ...
        if (!empty($data->reset_tarefalv_submissions)) {
            ...
            foreach ($this->submissionplugins as $plugin) {
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'tarefalv'),
            ...
            $tarefalvids = $DB->get_records('tarefalv', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($tarefalvids));

            $DB->delete_records_select('tarefalv_submission', "assignment $sql", $params);
            $DB->delete_records_select('tarefalv_user_flags', "assignment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('tarefalv_grades', "assignment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/tarefalv/lib.php');
                tarefalv_reset_gradebook($data->courseid);
            }

            // Reset revealidentities for tarefalv if blindmarking is enabled.
            if ($this->get_instance()->blindmarking) {
                $DB->set_field('tarefalv', 'revealidentities', 0, array('id' => $this->get_instance()->id));
            }
        }

        // Remove user overrides.
        if (!empty($data->reset_tarefalv_user_overrides)) {
            $DB->delete_records_select('tarefalv_overrides',
                'tarefalvid IN (SELECT id FROM {tarefalv} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('useroverridesdeleted', 'tarefalv'),
                'error' => false);
        }
        // Remove group overrides.
        if (!empty($data->reset_tarefalv_group_overrides)) {
            $DB->delete_records_select('tarefalv_overrides',
                'tarefalvid IN (SELECT id FROM {tarefalv} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('groupoverridesdeleted', 'tarefalv'),
                'error' => false);
        }

        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            $DB->execute("UPDATE {tarefalv_overrides}
                         SET allowsubmissionsfromdate = allowsubmissionsfromdate + ?
                       WHERE tarefalvid = ? AND allowsubmissionsfromdate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {tarefalv_overrides}
                         SET duedate = duedate + ?
                       WHERE tarefalvid = ? AND duedate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {tarefalv_overrides}
                         SET cutoffdate = cutoffdate + ?
                       WHERE tarefalvid =? AND cutoffdate <> 0",
                array($data->timeshift, $this->get_instance()->id));

            // Any changes to the list of dates that needs to be rolled should be same during course restore and course reset.
            // See MDL-9367.
            shift_course_mod_dates('tarefalv',
     ...
     *
     * @param tarefalv_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(tarefalv_plugin $plugin, stdClass $formdata) {
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/tarefalv/lib.php');
        $tarefalv = clone $this->get_instance();
        $tarefalv->cmidnumber = $coursemoduleid;

        // Set tarefalv gradebook feedback plugin status (enabled and visible).
        $tarefalv->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return tarefalv_grade_item_update($tarefalv, $param);
    }
     ...
     * @return integer
     */
    public function get_tarefalv_perpage() {
        $perpage = (int) get_user_preferences('tarefalv_perpage', 10);
        $adminconfig = $this->get_admin_config();
        $maxperpage = -1;
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('tarefalv');
        return $this->adminconfig;
    }
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            $event->name = get_string('calendardue', 'tarefalv', $instance->name);
            ...
            $select = "modulename = :modulename
                       ...
                       AND groupid = 0
                       AND courseid <> 0";
            $params = array('modulename' => 'tarefalv', 'instance' => $instance->id, 'eventtype' => $eventtype);
            ...
            $DB->delete_records('event', array('modulename' => 'tarefalv', 'instance' => $instance->id,
                'eventtype' => $eventtype));
        }

        $eventtype = TAREFALV_EVENT_TYPE_GRADINGDUE;
        if ($instance->gradingduedate) {
            $event->name = get_string('calendargradingdue', 'tarefalv', $instance->name);
            $event->eventtype = $eventtype;
            $event->timestart = $instance->gradingduedate;
            $event->timesort = $instance->gradingduedate;
            $event->id = $DB->get_field('event', 'id', array('modulename' => 'tarefalv',
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('tarefalv', $update);
        $this->instance = $DB->get_record('tarefalv', array('id'=>$update->id), '*', MUST_EXIST);
        ...
        \core_completion\api::update_completion_date_event($this->get_course_module()->id, 'tarefalv', $this->instance,
                $completionexpected);
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_tarefalv', TAREFALV_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param tarefalv_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(tarefalv_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'tarefalv'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'tarefalv'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'tarefalv'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'tarefalv'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'tarefalv');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'tarefalv');
        return self::$modulenameplural;
    }
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('tarefalv', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
    ...
    /**
     * Get the primary grade item for this tarefalv instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_tarefalv', TAREFALV_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        $tarefalvid = $this->get_instance()->id;
        $params['assignmentid1'] = $tarefalvid;
        $params['assignmentid2'] = $tarefalvid;
        $params['assignmentid3'] = $tarefalvid;

        $fields = 'SELECT u.id, s.status, s.timemodified AS stime, g.timemodified AS gtime, g.grade, uf.extensionduedate';
        $from = ' FROM {user} u
                         LEFT JOIN {tarefalv_submission} s
                                ON u.id = s.userid
                               AND s.assignment = :assignmentid1
                               AND s.latest = 1
                         LEFT JOIN {tarefalv_grades} g
                                ON u.id = g.userid
                               AND g.assignment = :assignmentid2
                               AND g.attemptnumber = s.attemptnumber
                         LEFT JOIN {tarefalv_user_flags} uf
                                ON u.id = uf.userid
        ...
        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {tarefalv_user_mapping} um
                             ON u.id = um.userid
                            AND um.assignment = :assignmentid4 ';
            $params['assignmentid4'] = $tarefalvid;
        ...
        foreach ($records as $userid => $submissioninfo) {
            ...
            if (!empty($submissioninfo->stime) && $submissioninfo->status == TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
                $submitted = true;
            }
    ...
    public function list_participants($currentgroup, $idsonly) {
        ...
        if (!isset($this->participants[$key])) {
            list($esql, $params) = get_enrolled_sql($this->context, 'mod/tarefalv:submit', $currentgroup,
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {tarefalv_user_mapping} um
                                  ON u.id = um.userid
            ...
            if ($instance->markingworkflow &&
                    ...
                    !has_capability('mod/tarefalv:manageallocations', $this->get_context()) &&
                    has_capability('mod/tarefalv:grade', $this->get_context())) {

                $additionaljoins .= ' LEFT JOIN {tarefalv_user_flags} uf
                                     ON u.id = uf.userid
                                     AND uf.assignment = :assignmentid3';
    ...
    public function get_participant($userid) {
        ...
        if (!is_enrolled($this->context, $participant, 'mod/tarefalv:submit', $this->show_only_active_users())) {
            return null;
        }
    ...
    public function count_submissions_need_grading() {
        ...
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/tarefalv:submit', $currentgroup, true);

        $params['tarefalvid'] = $this->get_instance()->id;
    ...
    public function count_grades() {
        ...
        $sql = 'SELECT COUNT(g.userid)
                   FROM {tarefalv_grades} g
                   JOIN(' . $esql . ') e ON e.id = g.userid
                   WHERE g.assignment = :tarefalvid';
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = TAREFALV_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/tarefalv:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status) {
        ...
        $params['tarefalvid'] = $this->get_instance()->id;
        $params['tarefalvid2'] = $this->get_instance()->id;
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('tarefalv_filter', '');
        $table = new tarefalv_grading_table($this, 0, $filter, 0, false);
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastcron = $DB->get_field('modules', 'lastcron', array('name' => 'tarefalv'));
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {tarefalv} a
                 JOIN {tarefalv_grades} g ON g.assignment = a.id
            LEFT JOIN {tarefalv_user_flags} uf ON uf.assignment = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'tarefalv'
                 JOIN {grade_items} gri ON gri.iteminstance = a.id AND gri.courseid = a.course AND gri.itemmodule = md.name
            LEFT JOIN {tarefalv_user_mapping} um ON g.id = um.userid AND um.assignment = a.id
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => TAREFALV_MARKING_WORKFLOW_STATE_RELEASED,
        );
        ...
        if (!empty($submissions)) {
            ...
            foreach ($submissions as $submission) {
                ...
                $eventtype = 'tarefalv_notification';
                $updatetime = $submission->lastmodified;
                $modulename = get_string('modulename', 'tarefalv');
                ...
                $flags = $DB->get_record('tarefalv_user_flags', array('userid'=>$user->id, 'assignment'=>$submission->assignment));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('tarefalv_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->assignment = $submission->assignment;
                    $flags->mailed = 1;
                    $DB->insert_record('tarefalv_user_flags', $flags);
                }
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('tarefalv', $record->id, 0, false, MUST_EXIST);
            $context = context_module::instance($cm->id);
    ...
    public function update_user_flags($flags) {
        ...
        $result = $DB->update_record('tarefalv_user_flags', $flags);
        return $result;
    }
    ...
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('tarefalv_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_tarefalv\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $CFG;
        require_once($CFG->dirroot . '/mod/tarefalv/extensionform.php');
        ...
        if (empty($mform)) {
            $mform = new mod_tarefalv_extension_form(null, $formparams);
        }
        $mform->set_data($data);
        $header = new tarefalv_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'tarefalv'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new tarefalv_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'tarefalv') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $submission = null;
        $submissions = $DB->get_records('tarefalv_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('tarefalv_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('tarefalv_submission', 'latest', 0, $params);
            }
            $submission->status = TAREFALV_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('tarefalv_submission', $submission);
            return $DB->get_record('tarefalv_submission', array('id' => $sid));
        }
        return false;
    }
    ...
    private function view_course_index() {
        ...
        $strplural = get_string('modulenameplural', 'tarefalv');

        if (!$cms = get_coursemodules_in_course('tarefalv', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new tarefalv_course_index_summary($usesections, $strsectionname);
        ...
        foreach ($modinfo->instances['tarefalv'] as $cm) {
            ...
            if (has_capability('mod/tarefalv:grade', $context)) {
                $submitted = $assignment->count_submissions_with_status(TAREFALV_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/tarefalv:submit', $context)) {
                $usersubmission = $assignment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'tarefalv');
                } else {
                    $submitted = get_string('submissionstatus_', 'tarefalv');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'tarefalv', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_tarefalv_info($cm->id, $cm->get_formatted_name(), $sectionname, $timedue, $submitted, $grade);
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_PLUGIN);
        $item = null;
        if ($pluginsubtype == 'tarefalvsubmission') {
            ...
            $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new tarefalv_submission_plugin_submission($plugin,
                                                              $item,
                                                              tarefalv_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_tarefalv\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new tarefalv_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              tarefalv_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_tarefalv\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param tarefalv_plugin $plugin - The assignment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'tarefalv') . '-';
            }
        }

        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'tarefalv');
        ...
        if (!get_user_preferences('tarefalv_downloadasfolders', 1)) {
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component, $shortentext = false) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/tarefalv:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('tarefalv_portfolio_caller', $portfolioparams, 'mod_tarefalv');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'tarefalv')));
        $gradingresult = new tarefalv_gradingmessage(get_string('savegradingresult', 'tarefalv'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'tarefalv')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new tarefalv_gradingmessage(get_string('quickgradingresult', 'tarefalv'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_group_submission($groupid) {
        if (!$this->can_view_group_submission($groupid)) {
            throw new required_capability_exception($this->context, 'mod/tarefalv:viewgrades', 'nopermission', '');
        }
    }
    ...
    public function can_view_grades() {
        // Permissions check.
        if (!has_any_capability(array('mod/tarefalv:viewgrades', 'mod/tarefalv:grade'), $this->context)) {
            return false;
        }
    ...
    protected function download_submissions($userids = false) {
        ...
        $students = get_enrolled_users($this->context, "mod/tarefalv:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'tarefalv'));
                ...
                if ($submission) {
                    $downloadasfolders = get_user_preferences('tarefalv_downloadasfolders', 1);
                    foreach ($this->submissionplugins as $plugin) {
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
        ...
        if (count($filesforzipping) == 0) {
            $header = new tarefalv_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'tarefalv'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'tarefalv'));
            $url = new moodle_url('/mod/tarefalv/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_tarefalv\event\all_submissions_downloaded::create_from_tarefalv($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_tarefalv add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_tarefalv', null, RENDERER_TARGET_GENERAL);
        return $this->output;
    }
    ...
    protected function get_submission($submissionid) {
        ...
        return $DB->get_record('tarefalv_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $flags = $DB->get_record('tarefalv_user_flags', $params);
        ...
        if ($create) {
            ...
            $fid = $DB->insert_record('tarefalv_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('tarefalv_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            ...
            $gid = $DB->insert_record('tarefalv_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        return $DB->get_record('tarefalv_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grading_panel($args) {
        ...
        require_capability('mod/tarefalv:grade', $this->context);
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new tarefalv_submission_status_compact($instance->allowsubmissionsfromdate,
                                                                     ...
                                                                     $this->get_course_module()->id,
                                                                     $this->get_course()->id,
                                                                     tarefalv_submission_status::GRADER_VIEW,
                                                                     $showedit,
                                                                     false,
                                                                     $viewfullnames,
                                                                     $extensionduedate,
        ...
        if ($attemptnumber != -1 && ($attemptnumber + 1) != count($allsubmissions)) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'tarefalv', $params);
        ...
        $mform = new mod_tarefalv_grade_form(null,
                                           $formparams,
                                           'post',
                                           '',
        ...
        $o .= $this->get_renderer()->render(new tarefalv_form('gradingform', $mform));

        if (count($allsubmissions) > 1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new tarefalv_attempt_history_chooser($allsubmissions,
                                                          $allgrades,
                                                          $this->get_course_module()->id,
                                                          $userid);
        ...
        \mod_tarefalv\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_single_grade_page($mform) {
        ...
        $header = new tarefalv_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'tarefalv'));
        $o .= $this->get_renderer()->render($header);
        ...
        if (!$userid) {
            ...
            if (empty($SESSION->mod_tarefalv_useridlist[$useridlistkey])) {
                $SESSION->mod_tarefalv_useridlist[$useridlistkey] = $this->get_grading_userid_list();
            }
            $useridlist = $SESSION->mod_tarefalv_useridlist[$useridlistkey];
        } else {
            $rownum = 0;
            $useridlistid = 0;
            $useridlist = array($userid);
        ...
        if ($user) {
            $this->update_effective_access($userid);
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->get_context());
            $usersummary = new tarefalv_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
                                                   $this->is_blind_marking(),
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new tarefalv_submission_status($instance->allowsubmissionsfromdate,
                                                             $instance->alwaysshowdescription,
                                                             $submission,
                                                             $instance->teamsubmission,
                                                             $teamsubmission,
        ...
        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new tarefalv_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/tarefalv:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/tarefalv/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/tarefalv/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'tarefalv'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_tarefalv\event\reveal_identities_confirmation_page_viewed::create_from_tarefalv($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/tarefalv/view.php', $params);
    ...
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/tarefalv/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/tarefalv/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/tarefalv/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'tarefalv');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/tarefalv/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'tarefalv');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/tarefalv:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/tarefalv/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'tarefalv');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/tarefalv/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=tarefalvfeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'tarefalv'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_tarefalv', 'submissions');

        $perpage = $this->get_tarefalv_perpage();
        $filter = get_user_preferences('tarefalv_filter', '');
        $markerfilter = get_user_preferences('tarefalv_markerfilter', '');
        $workflowfilter = get_user_preferences('tarefalv_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('tarefalv_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('tarefalv_downloadasfolders', 1);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/tarefalv:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql('u');
            // Only enrolled users could be tarefalved as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/tarefalv:grade', 0, 'u.*', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'tarefalv');
            $markingallocationoptions[TAREFALV_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'tarefalv');
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'tarefalv');
            $markingworkflowoptions[''] = get_string('filternone', 'tarefalv');
            $markingworkflowoptions[TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
        ...
        $gradingoptionsform = new mod_tarefalv_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_tarefalv_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($this->is_blind_marking() && has_capability('mod/tarefalv:viewblinddetails', $this->get_context())) {
            $o .= $this->get_renderer()->notification(get_string('blindmarkingenabledwarning', 'tarefalv'), 'notifymessage');
        }

        // Load and print the table of submissions.
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new tarefalv_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_tarefalv_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new tarefalv_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new tarefalv_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_tarefalv_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $tarefalvform = new tarefalv_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($tarefalvform);
        }
        $tarefalvform = new tarefalv_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_tarefalv.init_grading_options');
        $o .= $this->get_renderer()->render($tarefalvform);
        return $o;
    }
    ...
    protected function view_grader() {
        ...
        \mod_tarefalv\event\grading_table_viewed::create_from_tarefalv($this)->trigger();
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/tarefalv/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/tarefalv:viewblinddetails', $this->get_context());
            ...
            if ($hasviewblind) {
                return get_string('participant', 'tarefalv') . ' ' . $uniqueid . ' (' .
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/tarefalv/submission_form.php');
        // Need submit permission to submit an assignment.
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/tarefalv:submit', $this->context);
            $title = get_string('editsubmission', 'tarefalv');
        } else {
            ...
            $title = get_string('editsubmissionother', 'tarefalv', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'tarefalv'));
            return $this->view_notices($title, $message);
        }

        $postfix = '';
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_tarefalv', TAREFALV_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                      $this->get_context(),
                                                      $this->show_intro(),
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_tarefalv_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new tarefalv_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_tarefalv\event\submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/tarefalv:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_tarefalv_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/tarefalv/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            ...
            $usershtml .= $this->get_renderer()->render(new tarefalv_user_summary($user,
                                                                $this->get_course()->id,
                                                                $viewfullnames,
                                                                $this->is_blind_marking(),
                                                                $this->get_uniqueid_for_user($user->id),
        ...
        $mform = new mod_tarefalv_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new tarefalv_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'tarefalv'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new tarefalv_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_tarefalv\event\batch_set_workflow_state_viewed::create_from_tarefalv($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/tarefalv/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be tarefalved as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/tarefalv:grade', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_tarefalv_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new tarefalv_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'tarefalv'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new tarefalv_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_tarefalv\event\batch_set_marker_allocation_viewed::create_from_tarefalv($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/tarefalv/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_tarefalv_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $o .= $this->get_renderer()->render(new tarefalv_header($this->get_instance(),
                                                              ...
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              get_string('confirmsubmissionheading', 'tarefalv')));
        $submitforgradingpage = new tarefalv_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_tarefalv\event\submission_confirmation_form_viewed::create_from_tarefalv($this)->trigger();

        return $o;
    }

    /**
     * Creates an tarefalv_submission_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @param bool $showlinks return plain text or links to the profile
     * @return tarefalv_submission_status renderable object
     */
    public function get_tarefalv_submission_status_renderable($user, $showlinks) {
        global $PAGE;
        ...
        $gradingmanager = get_grading_manager($this->context, 'mod_tarefalv', 'submissions');
        ...
        $submissionstatus = new tarefalv_submission_status($instance->allowsubmissionsfromdate,
                                                          ...
                                                          $this->get_course_module()->id,
                                                          $this->get_course()->id,
                                                          tarefalv_submission_status::STUDENT_VIEW,
                                                          $showedit,
                                                          $showsubmit,
                                                          $viewfullnames,
                                                          $extensionduedate,
    ...
    /**
     * Creates an tarefalv_feedback_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return tarefalv_feedback_status renderable object
     */
    public function get_tarefalv_feedback_status_renderable($user) {
        global $CFG, $DB, $PAGE;
        ...
        if ($this->get_instance()->markingworkflow && $gradingstatus != TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
            $emptyplugins = true; // Don't show feedback plugins until released either.
        }

        $cangrade = has_capability('mod/tarefalv:grade', $this->get_context());
        ...
        if (($hasgrade || !$emptyplugins) && $gradevisible) {
            ...
            $feedbackstatus = new tarefalv_feedback_status($gradefordisplay,
                                                  $gradeddate,
                                                  $grader,
                                                  $this->get_feedback_plugins(),
                                                  $grade,
    ...
    /**
     * Creates an tarefalv_attempt_history renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return tarefalv_attempt_history renderable object
     */
    public function get_tarefalv_attempt_history_renderable($user) {
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            if (has_capability('mod/tarefalv:submit', $this->get_context(), $user, false)) {
                $submissionstatus = $this->get_tarefalv_submission_status_renderable($user, $showlinks);
                $o .= $this->get_renderer()->render($submissionstatus);
            }

            // If there is a visible grade, show the feedback.
            $feedbackstatus = $this->get_tarefalv_feedback_status_renderable($user);
            if ($feedbackstatus) {
                $o .= $this->get_renderer()->render($feedbackstatus);
            }

            // If there is more than one submission, show the history.
            $history = $this->get_tarefalv_attempt_history_renderable($user);
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('tarefalv_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('tarefalv_submission', $params, 'attemptnumber ASC');

        return $submissions;
    }

    /**
     * Creates an tarefalv_grading_summary renderable.
     *
     * @return tarefalv_grading_summary renderable object
     */
    public function get_tarefalv_grading_summary_renderable() {

        $instance = $this->get_instance();

        $draft = TAREFALV_SUBMISSION_STATUS_DRAFT;
        $submitted = TAREFALV_SUBMISSION_STATUS_SUBMITTED;
        ...
        if ($instance->teamsubmission) {
            ...
            $summary = new tarefalv_grading_summary($this->count_teams($activitygroup),
            ...
            $summary = new tarefalv_grading_summary($countparticipants,
                                                  $instance->submissiondrafts,
                                                  $this->count_submissions_with_status($draft),
                                                  $this->is_any_submission_plugin_enabled(),
                                                  $this->count_submissions_with_status($submitted),
    ...
    protected function view_submission_page() {
        ...
        $o .= $this->get_renderer()->render(new tarefalv_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new tarefalv_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            // Group selector will only be displayed if necessary.
            $currenturl = new moodle_url('/mod/tarefalv/view.php', array('id' => $this->get_course_module()->id));
            $o .= groups_print_activity_menu($this->get_course_module(), $currenturl->out(), true);

            $summary = $this->get_tarefalv_grading_summary_renderable();
        ...
        \mod_tarefalv\event\submission_status_viewed::create_from_tarefalv($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $tarefalv = clone $this->get_instance();
        $tarefalv->cmidnumber = $this->get_course_module()->idnumber;
        // Set tarefalv gradebook feedback plugin status (enabled and visible).
        $tarefalv->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return tarefalv_grade_item_update($tarefalv, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        $anysubmitted = false;
        $result = true;
        if ($submission->status != TAREFALV_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != TAREFALV_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('tarefalv_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = TAREFALV_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = TAREFALV_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('tarefalv_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('tarefalv_submission', $submission) && $result;
        }
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->tarefalv_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid) {
            if ($this->submissions_open($userid) &&
                    has_capability('mod/tarefalv:submit', $this->context, $graderid)) {
        ...
        if (!has_capability('mod/tarefalv:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/tarefalv:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/tarefalv:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'tarefalv', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/tarefalv/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/tarefalv/view.php?id=' . $coursemodule->id . '">' .
                     format_string($assignmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'tarefalv', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
    ...
    public static function send_assignment_notification($userfrom,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'tarefalv') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'tarefalv');
        ...
        $info->url = $CFG->wwwroot.'/mod/tarefalv/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'tarefalv', $info);
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'tarefalv');
            return false;
        }
        ...
        if ($submission->status != TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_tarefalv\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_tarefalv\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an assignment.
        require_capability('mod/tarefalv:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/tarefalv:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_tarefalv\event\extension_granted::create_from_tarefalv($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'tarefalv');
        }
        ...
        $sql = 'SELECT u.id AS userid, g.grade AS grade, g.timemodified AS lastmodified,
             ...
             LEFT JOIN {tarefalv_grades} g ON
                       u.id = g.userid AND
                       g.assignment = :tarefalvid2 AND
                       g.attemptnumber = gmx.maxattempt
             LEFT JOIN {tarefalv_user_flags} uf ON uf.assignment = g.assignment AND uf.userid = g.userid
                 WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'tarefalv');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('tarefalvfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_tarefalv\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/tarefalv',
                                          $this->course->id,
                                          'mod',
                                          'tarefalv',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'tarefalv');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/tarefalv:revealidentities', $this->context);
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('tarefalvfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('tarefalv_grades', array('assignment'=>$this->get_instance()->id));
        ...
        \mod_tarefalv\event\identities_revealed::create_from_tarefalv($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $gradingoptionsparams = array('cm'=>$this->get_course_module()->id,
                                      ...
                                      'showonlyactiveenrol' => $this->show_only_active_users(),
                                      'downloadasfolders' => get_user_preferences('tarefalv_downloadasfolders', 1));
        $mform = new mod_tarefalv_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('tarefalv_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('tarefalv_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('tarefalv_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('tarefalv_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('tarefalv_quickgrading', isset($formdata->quickgrading));
            }
            if (isset($formdata->downloadasfolders)) {
                set_user_preference('tarefalv_downloadasfolders', 1); // Enabled.
            } else {
                set_user_preference('tarefalv_downloadasfolders', 0); // Disabled.
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'tarefalv', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'tarefalv');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'tarefalv');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'tarefalv');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'tarefalv', $params) . ' <br>';
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != TAREFALV_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'tarefalv');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'tarefalv');
            return false;
        }
        ...
        \mod_tarefalv\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            \mod_tarefalv\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($this->new_submission_empty($data)) {
            $notices[] = get_string('submissionempty', 'mod_tarefalv');
            return false;
        }
        ...
        if (isset($data->lastmodified) && ($submission->timemodified > $data->lastmodified)) {
            ...
            if ($submission->status !== TAREFALV_SUBMISSION_STATUS_NEW) {
                $notices[] = $instance->teamsubmission ? get_string('submissionmodifiedgroup', 'mod_tarefalv')
                                                       : get_string('submissionmodified', 'mod_tarefalv');
                return false;
            }
        }
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'tarefalv');
            return true;
        }
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        require_sesskey();
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('duedatereached', 'tarefalv');
            return false;
        }
        $instance = $this->get_instance();
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'tarefalv', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'tarefalv');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $strgradelocked = get_string('gradelocked', 'tarefalv');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'tarefalv');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'tarefalv')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'tarefalv'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'tarefalv');
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            // Only enrolled users could be tarefalved as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/tarefalv:grade', 0, 'u.*', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'tarefalv'));
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker, $viewfullnames);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'tarefalv'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'tarefalv');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', TAREFALV_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', TAREFALV_MARKING_WORKFLOW_STATE_RELEASED);
        }
        $gradestring = '<span class="currentgrade">' . $gradestring . '</span>';
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'tarefalv'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'tarefalv', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'tarefalv'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'tarefalv'));
        ...
        if (($attemptnumber == -1 ||
            ...
            $this->get_instance()->attemptreopenmethod != TAREFALV_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'tarefalv'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'tarefalv');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'tarefalv'), $attemptreopenmethod);
            ...
            if ($maxattempts == TAREFALV_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'tarefalv');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'tarefalv'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'tarefalv'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == TAREFALV_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == TAREFALV_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'tarefalv'));
                $mform->setDefault('addattempt', 0);
            }
        }
        if (!$gradingpanel) {
            $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'tarefalv'));
        } else {
            $mform->addElement('hidden', 'sendstudentnotifications', get_string('sendstudentnotifications', 'tarefalv'));
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            if (!$gradingpanel) {
                $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', TAREFALV_MARKING_WORKFLOW_STATE_RELEASED);
            }
        } else {
        ...
        if (!$gradingpanel) {
            ...
            $buttonarray = array();
            $name = get_string('savechanges', 'tarefalv');
            $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
            if (!$last) {
                $name = get_string('savenext', 'tarefalv');
            ...
            if ($rownum > 0) {
                $name = get_string('previous', 'tarefalv');
                $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
            }

            if (!$last) {
                $name = get_string('nosavebutnext', 'tarefalv');
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_tarefalv\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_tarefalv\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to tarefalv this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    if ($state == TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
                        // Update Gradebook.
                        $tarefalv = clone $this->get_instance();
                        $tarefalv->cmidnumber = $this->get_course_module()->idnumber;
                        // Set tarefalv gradebook feedback plugin status.
                        $tarefalv->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                        tarefalv_update_grades($tarefalv, $userid);
                    }

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_tarefalv\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_tarefalv\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_tarefalv\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function apply_grade_to_user($formdata, $userid, $attemptnumber) {
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_tarefalv\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
        ...
        if ($this->get_instance()->markingworkflow &&
                isset($formdata->workflowstate) &&
                $formdata->workflowstate != TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != TAREFALV_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == TAREFALV_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
        ...
        if ($instance->attemptreopenmethod == TAREFALV_ATTEMPT_REOPEN_METHOD_MANUAL &&
                !empty($addattempt)) {
            $shouldreopen = true;
        }
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_tarefalv_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/tarefalv/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_tarefalv', $url);
            }
            $useridlist = $SESSION->mod_tarefalv_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
    ...
    public static function can_upgrade_assignment($type, $version) {
        $assignment = new tarefalv(null, null, null);
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == TAREFALV_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != TAREFALV_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = TAREFALV_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('tarefalvfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('tarefalvid1'=>$assignmentid,
                        'tarefalvid2'=>$assignmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                FROM {user} u
                                                LEFT JOIN {tarefalv_submission} s
                                                    ON u.id = s.userid and s.assignment = :tarefalvid1 AND
                                                    s.latest = 1
                                                JOIN {tarefalv_grades} g
                                                    ON u.id = g.userid and g.assignment = :tarefalvid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - tarefalv them a random id.
     *
     * @param int $tarefalvid The tarefalvid to lookup
     */
    public static function allocate_unique_ids($tarefalvid) {
        global $DB;

        $cm = get_coursemodule_from_instance('tarefalv', $tarefalvid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        ...
        foreach ($users as $user) {
            $record = $DB->get_record('tarefalv_user_mapping',
                                      array('assignment'=>$tarefalvid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->assignment = $tarefalvid;
                $record->userid = $user->id;
                $DB->insert_record('tarefalv_user_mapping', $record);
            }
        }
    }
     ...
     *
     * @param int $tarefalvid The assignment id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($tarefalvid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('assignment'=>$tarefalvid, 'userid'=>$userid);
        if ($record = $DB->get_record('tarefalv_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($tarefalvid);
        ...
        return $DB->insert_record('tarefalv_user_mapping', $record);
    }
     ...
     *
     * @param int $tarefalvid The id of the assignment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($tarefalvid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('tarefalv_user_mapping',
                                      array('assignment'=>$tarefalvid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/tarefalv:grade', $this->context)) {
            $states[TAREFALV_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'tarefalv');
            $states[TAREFALV_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'tarefalv');
        }
        if (has_any_capability(array('mod/tarefalv:reviewgrades',
                                     'mod/tarefalv:managegrades'), $this->context)) {
            $states[TAREFALV_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'tarefalv');
            $states[TAREFALV_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'tarefalv');
        }
        if (has_any_capability(array('mod/tarefalv:releasegrades',
                                     'mod/tarefalv:managegrades'), $this->context)) {
            $states[TAREFALV_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'tarefalv');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {
            $attemptnumber = optional_param('attemptnumber', -1, PARAM_INT);
            $grade = $this->get_user_grade($userid, false, $attemptnumber);

            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return TAREFALV_GRADING_STATUS_GRADED;
            } else {
                return TAREFALV_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the tarefalv object.
     *
     * @return string
     */
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/tarefalv:grade', $this->get_context()) && get_config('tarefalv', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \moodle_url('/mod/tarefalv/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
            \core\notification::warning(get_string('fixrescalednullgrades', 'mod_tarefalv', ['link' => $link->out()]));
        }
    }
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        require_capability('mod/tarefalv:grade', $this->get_context());
        ...
        if ($confirm) {
            ...
            unset_config('has_rescaled_null_grades_' . $instance->id, 'tarefalv');

            // Display the notice.
            $o .= $this->get_renderer()->notification(get_string('fixrescalednullgradesdone', 'tarefalv'), 'notifysuccess');
            $url = new moodle_url(
                '/mod/tarefalv/view.php',
                array(
            ...
            $continue = new \moodle_url('/mod/tarefalv/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \moodle_url('/mod/tarefalv/view.php', array('id' => $this->get_course_module()->id));
            $o .= $OUTPUT->confirm(get_string('fixrescalednullgradesconfirm', 'mod_tarefalv'), $continue, $cancel);
        }
...
/**
 * Portfolio caller class for mod_tarefalv.
 *
 * @package   mod_tarefalv
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('tarefalv', $this->cmid, 0, false, MUST_EXIST);
    }
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_tarefalv');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'tarefalv' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return tarefalv_submission_plugin
     */
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/tarefalv:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'tarefalv');
    }
 ...
 * @return void
 */
function tarefalv_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.tarefalvid
                  FROM {tarefalv_overrides} o
                  JOIN {tarefalv} tarefalv ON tarefalv.id = o.tarefalvid
                 WHERE tarefalv.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all tarefalv in course.
        $sql = "SELECT o.id, o.tarefalvid
                  FROM {tarefalv_overrides} o
                  JOIN {tarefalv} tarefalv ON tarefalv.id = o.tarefalvid
    ...
    $DB->delete_records_list('tarefalv_overrides', 'id', array_keys($records));
}
 ...
 * @param int $tarefalvid of the assignment
 * @return bool success of operation
 */
function move_group_override($id, $move, $tarefalvid) {
    global $DB;

    // Get the override object.
    if (!$override = $DB->get_record('tarefalv_overrides', array('id' => $id), 'id, sortorder')) {
        return false;
    }
    // Count the number of group overrides.
    $overridecountgroup = $DB->count_records('tarefalv_overrides', array('userid' => null, 'tarefalvid' => $tarefalvid));
    ...
    $params = array('sortorder' => $neworder, 'tarefalvid' => $tarefalvid);
    if ($swapoverride = $DB->get_record('tarefalv_overrides', $params, 'id, sortorder')) {
        ...
        $DB->update_record('tarefalv_overrides', $override);
        $DB->update_record('tarefalv_overrides', $swapoverride);
    }

    reorder_group_overrides($tarefalvid);
    return true;
}
 ...
 *
 * @param int $tarefalvid of the assigment
 */
function reorder_group_overrides($tarefalvid) {
    global $DB;

    $i = 1;
    if ($overrides = $DB->get_records('tarefalv_overrides', array('userid' => null, 'tarefalvid' => $tarefalvid), 'sortorder ASC')) {
        foreach ($overrides as $override) {
            $f = new stdClass();
            $f->id = $override->id;
            $f->sortorder = $i++;
            $DB->update_record('tarefalv_overrides', $f);

            // Update priorities of group overrides.
            $params = [
                'modulename' => 'tarefalv',
                'instance' => $override->tarefalvid,
