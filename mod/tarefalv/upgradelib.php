...
/**
 * This file contains the upgrade code to upgrade from mod_assignment to mod_tarefalv
 *
 ...
 */
define('TAREFALV_MAX_UPGRADE_TIME_SECS', 300);

/**
 * Class to manage upgrades from mod_assignment to mod_tarefalv
 *
 * @package   mod_tarefalv
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_upgrade_manager {
    ...
    public function upgrade_assignment($oldassignmentid, & $log) {
        ...
        core_php_time_limit::raise(TAREFALV_MAX_UPGRADE_TIME_SECS);
        ...
        if (!has_capability('mod/tarefalv:addinstance', $oldcontext)) {
            $log = get_string('couldnotcreatenewassignmentinstance', 'mod_tarefalv');
            return false;
        }

        // First insert an tarefalv instance to get the id.
        ...
        $data->maxattempts = TAREFALV_UNLIMITED_ATTEMPTS;

        $newassignment = new tarefalv(null, null, null);
        ...
        $newmodule = $DB->get_record('modules', array('name'=>'tarefalv'), '*', MUST_EXIST);
        $newcoursemodule = $this->duplicate_course_module($oldcoursemodule,
                                                          $newmodule->id,
                                                          $newassignment->get_instance()->id);
        if (!$newcoursemodule) {
            $log = get_string('couldnotcreatenewcoursemodule', 'mod_tarefalv');
            return false;
        }
        ...
        try {
            ...
            $newassignment->copy_area_files_for_upgrade($oldcontext->id, 'mod_assignment', 'intro', 0,
                                            $newassignment->get_context()->id, 'mod_tarefalv', 'intro', 0);
            ...
            $DB->set_field('log', 'module', 'tarefalv', $logparams);
            ...
            foreach ($oldsubmissions as $oldsubmission) {
                ...
                $submission->latest = 1;
                $submission->id = $DB->insert_record('tarefalv_submission', $submission);
                if (!$submission->id) {
                    $log .= get_string('couldnotinsertsubmission', 'mod_tarefalv', $submission->userid);
                    $rollback = true;
                }
                foreach ($newassignment->get_submission_plugins() as $plugin) {
                    if ($plugin->can_upgrade($oldassignment->assignmenttype, $oldversion)) {
                ...
                if ($oldsubmission->timemarked) {
                    ...
                    $grade->id = $DB->insert_record('tarefalv_grades', $grade);
                    if (!$grade->id) {
                        $log .= get_string('couldnotinsertgrade', 'mod_tarefalv', $grade->userid);
                        $rollback = true;
                    }
            ...
            // Reassociate grade_items from the old assignment instance to the new tarefalv instance.
            // This includes outcome linked grade_items.
            $params = array('tarefalv', $newassignment->get_instance()->id, 'assignment', $oldassignment->id);
            ...
            $rollback = true;
            $log .= get_string('conversionexception', 'mod_tarefalv', $exception->getMessage());
        }

        if ($rollback) {
            // Roll back the grades changes.
            if ($gradesdone) {
                // Reassociate grade_items from the new tarefalv instance to the old assignment instance.
                $params = array('assignment', $oldassignment->id, 'tarefalv', $newassignment->get_instance()->id);
                $sql = 'UPDATE {grade_items} SET itemmodule = ?, iteminstance = ? WHERE itemmodule = ? AND iteminstance = ?';
                $DB->execute($sql, $params);
            }
     ...
     * tarefalv module alongside the old assignment module.
     *
     * @param stdClass $cm The old course module record
     * @param int $moduleid The id of the new tarefalv module
     * @param int $newinstanceid The id of the new instance of the tarefalv module
...
function get_assignments_with_rescaled_null_grades() {
    ...
    $query = 'SELECT id, assignment FROM {tarefalv_grades}
