...
/**
 * This file contains the classes for the admin settings of the tarefalv module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_admin_page_manage_tarefalv_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/tarefalv/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'tarefalv'),
                            $url);
    }
...
class tarefalv_plugin_manager {
     ...
     * @param string $subtype - either tarefalvsubmission or tarefalvfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/tarefalv/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'tarefalv'),
                get_string('version'), get_string('hideshow', 'tarefalv'),
        ...
        $shortsubtype = substr($this->subtype, strlen('tarefalv'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/tarefalv/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'tarefalv'));
    }
