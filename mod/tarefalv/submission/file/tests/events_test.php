...
class tarefalvsubmission_file_events_testcase extends advanced_testcase {
    ...
    /** @var stdClass $tarefalv The assignment object. */
    protected $tarefalv;
    ...
    protected function setUp() {
        ...
        $this->cm = get_coursemodule_from_instance('tarefalv', $instance->id);
        $this->context = context_module::instance($this->cm->id);
        $this->tarefalv = new testable_tarefalv($this->context, $this->cm, $this->course);

        $this->setUser($this->user->id);
        $this->submission = $this->tarefalv->get_user_submission($this->user->id, true);
        ...
        $dummy = (object) array(
            ...
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'mytarefalvmnent.png'
        );
        $this->fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $this->files = $fs->get_area_files($this->context->id, 'tarefalvsubmission_file', TAREFALVSUBMISSION_FILE_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $data = new stdClass();
        $plugin = $this->tarefalv->get_submission_plugin_by_type('file');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\tarefalvsubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\tarefalvsubmission_file\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\tarefalvsubmission_file\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
