 ...
 *
 * @package   tarefalvsubmission_comments
 ...
 * @return bool
 */
function tarefalvsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('tarefalv_submission', array('id'=>$options->itemid))) {
 ...
 * @return array
 */
function tarefalvsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
 ...
 */
function tarefalvsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($assignment->is_blind_marking() && !empty($comments)) {
        ...
        $userinteam = false;
        if ($assignment->get_instance()->teamsubmission && has_capability('mod/tarefalv:submit', $context)) {
        ...
        foreach ($comments as $comment) {
            ...
            if (has_capability('mod/tarefalv:viewblinddetails', $context) && $USER->id != $comment->userid) {
                ...
                $comment->fullname = get_string('blindmarkingviewfullname', 'tarefalvsubmission_comments', $a);
                ...
                if (empty($usermappings[$comment->userid])) {
                    ...
                    $commenter->firstname = get_string('blindmarkingname', 'tarefalvsubmission_comments', $anonid);
                    $commenter->lastname = '';
 ...
 */
function tarefalvsubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('tarefalv_submission', array('id' => $submissionid));
