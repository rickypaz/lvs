...
define('TAREFALVSUBMISSION_ONLINETEXT_FILEAREA', 'submissions_onlinetext');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_submission_onlinetext extends tarefalv_submission_plugin {
    ...
    public function get_name() {
        return get_string('onlinetext', 'tarefalvsubmission_onlinetext');
    }
    ...
    private function get_onlinetext_submission($submissionid) {
        ...
        return $DB->get_record('tarefalvsubmission_onlinetext', array('submission'=>$submissionid));
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $name = get_string('wordlimit', 'tarefalvsubmission_onlinetext');

        // Create a text box that can be enabled/disabled for onlinetext word limit.
        $wordlimitgrp = array();
        $wordlimitgrp[] = $mform->createElement('text', 'tarefalvsubmission_onlinetext_wordlimit', '', $options);
        $wordlimitgrp[] = $mform->createElement('checkbox', 'tarefalvsubmission_onlinetext_wordlimit_enabled',
                '', get_string('enable'));
        $mform->addGroup($wordlimitgrp, 'tarefalvsubmission_onlinetext_wordlimit_group', $name, ' ', false);
        $mform->addHelpButton('tarefalvsubmission_onlinetext_wordlimit_group',
                              'wordlimit',
                              'tarefalvsubmission_onlinetext');
        $mform->disabledIf('tarefalvsubmission_onlinetext_wordlimit',
                           'tarefalvsubmission_onlinetext_wordlimit_enabled',
                           'notchecked');

        // Add numeric rule to text field.
        $wordlimitgrprules = array();
        $wordlimitgrprules['tarefalvsubmission_onlinetext_wordlimit'][] = array(null, 'numeric', null, 'client');
        $mform->addGroupRule('tarefalvsubmission_onlinetext_wordlimit_group', $wordlimitgrprules);

        // Rest of group setup.
        $mform->setDefault('tarefalvsubmission_onlinetext_wordlimit', $defaultwordlimit);
        $mform->setDefault('tarefalvsubmission_onlinetext_wordlimit_enabled', $defaultwordlimitenabled);
        $mform->setType('tarefalvsubmission_onlinetext_wordlimit', PARAM_INT);
        $mform->disabledIf('tarefalvsubmission_onlinetext_wordlimit_group',
                           'tarefalvsubmission_onlinetext_enabled',
                           'notchecked');
    }
    ...
    public function save_settings(stdClass $data) {
        if (empty($data->tarefalvsubmission_onlinetext_wordlimit) || empty($data->tarefalvsubmission_onlinetext_wordlimit_enabled)) {
            $wordlimit = 0;
            $wordlimitenabled = 0;
        } else {
            $wordlimit = $data->tarefalvsubmission_onlinetext_wordlimit;
            $wordlimitenabled = 1;
        }
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
        ...
        $data = file_prepare_standard_editor($data,
                                             ...
                                             'tarefalvsubmission_onlinetext',
                                             TAREFALVSUBMISSION_ONLINETEXT_FILEAREA,
                                             $submissionid);
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        $event = \tarefalvsubmission_onlinetext\event\assessable_uploaded::create($params);
        $event->trigger();
        ...
        if ($onlinetextsubmission) {
            ...
            $updatestatus = $DB->update_record('tarefalvsubmission_onlinetext', $onlinetextsubmission);
            $event = \tarefalvsubmission_onlinetext\event\submission_updated::create($params);
            ...
            $onlinetextsubmission->id = $DB->insert_record('tarefalvsubmission_onlinetext', $onlinetextsubmission);
            $params['objectid'] = $onlinetextsubmission->id;
            $event = \tarefalvsubmission_onlinetext\event\submission_created::create($params);
            $event->set_tarefalv($this->assignment);
            $event->trigger();
    ...
    public function get_editor_fields() {
        return array('onlinetext' => get_string('pluginname', 'tarefalvsubmission_onlinetext'));
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($onlinetextsubmission) {
            ...
            $text = $this->assignment->render_editor_content(TAREFALVSUBMISSION_ONLINETEXT_FILEAREA,
                                                             $onlinetextsubmission->submission,
                                                             $this->get_type(),
                                                             'onlinetext',
                                                             'tarefalvsubmission_onlinetext', true);
            ...
            if ($onlinetext != $shorttext) {
                $wordcount = get_string('numwords', 'tarefalvsubmission_onlinetext', count_words($onlinetext));
    ...
    public function get_files(stdClass $submission, stdClass $user) {
        ...
        if ($onlinetextsubmission) {
            ...
            $filename = get_string('onlinetextfilename', 'tarefalvsubmission_onlinetext');
    ...
    public function view(stdClass $submission) {
        ...
        if ($onlinetextsubmission) {
            ...
            $result .= $this->assignment->render_editor_content(TAREFALVSUBMISSION_ONLINETEXT_FILEAREA,
                                                                $onlinetextsubmission->submission,
                                                                $this->get_type(),
                                                                'onlinetext',
                                                                'tarefalvsubmission_onlinetext');
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('tarefalvsubmission_onlinetext', $onlinetextsubmission) > 0) {
    ...
    public function delete_instance() {
        global $DB;
        $DB->delete_records('tarefalvsubmission_onlinetext',
    ...
    public function get_file_areas() {
        return array(TAREFALVSUBMISSION_ONLINETEXT_FILEAREA=>$this->get_name());
    }
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'tarefalvsubmission_onlinetext',
                                     TAREFALVSUBMISSION_ONLINETEXT_FILEAREA, $sourcesubmission->id, 'id', false);
        foreach ($files as $file) {
            $fieldupdates = array('itemid' => $destsubmission->id);
            $fs->create_file_from_storedfile($fieldupdates, $file);
        }

        // Copy the tarefalvsubmission_onlinetext record.
    ...
    public function check_word_count($submissiontext) {
        ...
        if ($wordcount <= $wordlimit) {
            ...
            $errormsg = get_string('wordlimitexceeded', 'tarefalvsubmission_onlinetext',
