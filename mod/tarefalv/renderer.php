 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_tarefalv_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function tarefalv_files(context $context, $userid, $filearea, $component) {
        return $this->render(new tarefalv_files($context, $userid, $filearea, $component));
    }

    /**
     * Rendering assignment files
     *
     * @param tarefalv_files $tree
     * @return string
     */
    public function render_tarefalv_files(tarefalv_files $tree) {
        $this->htmlid = html_writer::random_id('tarefalv_files_tree');
        $this->page->requires->js_init_call('M.mod_tarefalv.init_tree', array(true, $this->htmlid));
     ...
     * @param tarefalv_gradingmessage $result The result to render
     * @return string
     */
    public function render_tarefalv_gradingmessage(tarefalv_gradingmessage $result) {
     ...
     * @param tarefalv_form $form The form to render
     * @return string
     */
    public function render_tarefalv_form(tarefalv_form $form) {
        $o = '';
     ...
     *
     * @param tarefalv_user_summary $summary The user summary to render
     * @return string
     */
    public function render_tarefalv_user_summary(tarefalv_user_summary $summary) {
        $o = '';
        $supendedclass = '';
        $suspendedicon = '';
        ...
        if ($summary->blindmarking) {
            $o .= get_string('hiddenuser', 'tarefalv') . $summary->uniqueidforuser.$suspendedicon;
        } else {
     ...
     *
     * @param tarefalv_submit_for_grading_page $page
     * @return string
     */
    public function render_tarefalv_submit_for_grading_page($page) {
        $o = '';

        $o .= $this->output->container_start('submitforgrading');
        $o .= $this->output->heading(get_string('confirmsubmissionheading', 'tarefalv'), 3);

        $cancelurl = new moodle_url('/mod/tarefalv/view.php', array('id' => $page->coursemoduleid));
        if (count($page->notifications)) {
            // At least one of the submission plugins is not ready for submission.

            $o .= $this->output->heading(get_string('submissionnotready', 'tarefalv'), 4);
     ...
     * Render the header.
     *
     * @param tarefalv_header $header
     * @return string
     */
    public function render_tarefalv_header(tarefalv_header $header) {
        $o = '';
        ...
        $this->page->set_title(get_string('pluginname', 'tarefalv'));
        $this->page->set_heading($this->page->course->fullname);

        $o .= $this->output->header();
        $heading = format_string($header->tarefalv->name, false, array('context' => $header->context));
        ...
        if ($header->showintro) {
            ...
            $o .= format_module_intro('tarefalv', $header->tarefalv, $header->coursemoduleid);
            $o .= $header->postfix;
            $o .= $this->output->box_end();
        }
     ...
     *
     * @param tarefalv_plugin_header $header
     * @return string
     */
    public function render_tarefalv_plugin_header(tarefalv_plugin_header $header) {
     ...
     *
     * @param tarefalv_grading_summary $summary
     * @return string
     */
    public function render_tarefalv_grading_summary(tarefalv_grading_summary $summary) {
        // Create a table for the data.
        $o = '';
        $o .= $this->output->container_start('gradingsummary');
        $o .= $this->output->heading(get_string('gradingsummary', 'tarefalv'), 3);
        ...
        if ($summary->teamsubmission) {
            if ($summary->warnofungroupedusers) {
                $o .= $this->output->notification(get_string('ungroupedusers', 'tarefalv'));
            }

            $this->add_table_row_tuple($t, get_string('numberofteams', 'tarefalv'),
                                       $summary->participantcount);
        } else {
            $this->add_table_row_tuple($t, get_string('numberofparticipants', 'tarefalv'),
                                       $summary->participantcount);
        }
        ...
        if ($summary->submissiondraftsenabled && $summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofdraftsubmissions', 'tarefalv'),
                                       $summary->submissiondraftscount);
        }

        // Submitted for grading.
        if ($summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofsubmittedassignments', 'tarefalv'),
                                       $summary->submissionssubmittedcount);
            if (!$summary->teamsubmission) {
                $this->add_table_row_tuple($t, get_string('numberofsubmissionsneedgrading', 'tarefalv'),
        ...
        if ($summary->duedate) {
            ...
            $this->add_table_row_tuple($t, get_string('duedate', 'tarefalv'),
                                       userdate($duedate));

            // Time remaining.
            $due = '';
            if ($duedate - $time <= 0) {
                $due = get_string('assignmentisdue', 'tarefalv');
            } else {
                $due = format_time($duedate - $time);
            }
            $this->add_table_row_tuple($t, get_string('timeremaining', 'tarefalv'), $due);

            if ($duedate < $time) {
                $cutoffdate = $summary->cutoffdate;
                if ($cutoffdate) {
                    if ($cutoffdate > $time) {
                        $late = get_string('latesubmissionsaccepted', 'tarefalv', userdate($summary->cutoffdate));
                    } else {
                        $late = get_string('nomoresubmissionsaccepted', 'tarefalv');
                    }
                    $this->add_table_row_tuple($t, get_string('latesubmissions', 'tarefalv'), $late);
                }
            }
        ...
        $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
        $o .= '<a href="' . $url . '" class="btn btn-secondary">' . get_string('viewgrading', 'mod_tarefalv') . '</a> ';
        if ($summary->cangrade) {
     ...
     *
     * @param tarefalv_feedback_status $status
     * @return string
     */
    public function render_tarefalv_feedback_status(tarefalv_feedback_status $status) {
        global $DB, $CFG;
        $o = '';

        $o .= $this->output->container_start('feedback');
        $o .= $this->output->heading(get_string('feedback', 'tarefalv'), 3);
        ...
        if (isset($status->gradefordisplay)) {
            ...
            // Grade date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedon', 'tarefalv'));
        ...
        if ($status->grader) {
            // Grader.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedby', 'tarefalv'));
        ...
        foreach ($status->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() &&
                ...
                $displaymode = tarefalv_feedback_plugin_feedback::SUMMARY;
                $pluginfeedback = new tarefalv_feedback_plugin_feedback($plugin,
                                                                      $status->grade,
                                                                      $displaymode,
     ...
     *
     * @param tarefalv_submission_status_compact $status
     * @return string
     */
    public function render_tarefalv_submission_status_compact(tarefalv_submission_status_compact $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        $o .= $this->output->heading(get_string('submission', 'tarefalv'), 3);
        $time = time();
        ...
        if ($status->teamsubmissionenabled) {
            ...
            if ($group) {
                ...
                if (count($status->usergroups) == 0) {
                    $team = '<span class="alert alert-error">' . get_string('noteam', 'tarefalv') . '</span>';
                } else if (count($status->usergroups) > 1) {
                    $team = '<span class="alert alert-error">' . get_string('multipleteams', 'tarefalv') . '</span>';
                }
            } else {
                $team = get_string('defaultteam', 'tarefalv');
            }
            $o .= $this->output->container(get_string('teamname', 'tarefalv', $team), 'teamname');
        }

        if (!$status->teamsubmissionenabled) {
            if ($status->submission && $status->submission->status != TAREFALV_SUBMISSION_STATUS_NEW) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'tarefalv');
                $o .= $this->output->container($statusstr, 'submissionstatus' . $status->submission->status);
            } else {
                if (!$status->submissionsenabled) {
                    $o .= $this->output->container(get_string('noonlinesubmissions', 'tarefalv'), 'submissionstatus');
                } else {
                    $o .= $this->output->container(get_string('noattempt', 'tarefalv'), 'submissionstatus');
                }
            }
        } else {
            $group = $status->submissiongroup;
            if (!$group && $status->preventsubmissionnotingroup) {
                $o .= $this->output->container(get_string('nosubmission', 'tarefalv'), 'submissionstatus');
            } else if ($status->teamsubmission && $status->teamsubmission->status != TAREFALV_SUBMISSION_STATUS_NEW) {
                $teamstatus = $status->teamsubmission->status;
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'tarefalv');
                $groupid = 0;
                ...
                foreach ($members as $member) {
                    ...
                    $url = new moodle_url('/user/view.php', $urlparams);
                    if ($status->view == tarefalv_submission_status::GRADER_VIEW && $status->blindmarking) {
                ...
                if (count($userslist) > 0) {
                    ...
                    $formatteduserstr = get_string('userswhoneedtosubmit', 'tarefalv', $userstr);
        ...
        // Is locked?
        if ($status->locked) {
            $o .= $this->output->container(get_string('submissionslocked', 'tarefalv'), 'submissionlocked');
        }

        // Grading status.
        $statusstr = '';
        $classname = 'gradingstatus';
        if ($status->gradingstatus == TAREFALV_GRADING_STATUS_GRADED ||
            $status->gradingstatus == TAREFALV_GRADING_STATUS_NOT_GRADED) {
            $statusstr = get_string($status->gradingstatus, 'tarefalv');
        } else {
            $gradingstatus = 'markingworkflowstate' . $status->gradingstatus;
            $statusstr = get_string($gradingstatus, 'tarefalv');
        }
        if ($status->gradingstatus == TAREFALV_GRADING_STATUS_GRADED ||
            $status->gradingstatus == TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($duedate > 0) {
            ...
            if ($duedate - $time <= 0) {
                if (!$submission ||
                    ...
                    if ($status->submissionsenabled) {
                        $remaining = get_string('overdue', 'tarefalv', format_time($time - $duedate));
                        $classname = 'overdue';
                    } else {
                        $remaining = get_string('duedatereached', 'tarefalv');
                    }
                } else {
                ...
                $remaining = get_string('paramtimeremaining', 'tarefalv', format_time($duedate - $time));
            }
            $o .= $this->output->container($remaining, $classname);
        }

        // Show graders whether this submission is editable by students.
        if ($status->view == tarefalv_submission_status::GRADER_VIEW) {
            if ($status->canedit) {
                $o .= $this->output->container(get_string('submissioneditable', 'tarefalv'), 'submissioneditable');
            } else {
                $o .= $this->output->container(get_string('submissionnoteditable', 'tarefalv'), 'submissionnoteditable');
            }
        }
            ...
            if (!$status->teamsubmission || $status->submissiongroup != false || !$status->preventsubmissionnotingroup) {
                foreach ($status->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $displaymode = tarefalv_submission_plugin_submission::SUMMARY;
                        $pluginsubmission = new tarefalv_submission_plugin_submission($plugin,
                            $submission,
                            $displaymode,
                            $status->coursemoduleid,
                            $status->returnaction,
                        ...
                        $o .= $this->output->container($this->render($pluginsubmission), 'tarefalvsubmission ' . $plugincomponent);
                    }
                }
            }
        }
     ...
     *
     * @param tarefalv_submission_status $status
     * @return string
     */
    public function render_tarefalv_submission_status(tarefalv_submission_status $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        $o .= $this->output->heading(get_string('submissionstatusheading', 'tarefalv'), 3);
        $time = time();
        ...
        if ($status->allowsubmissionsfromdate &&
            ...
            if ($status->alwaysshowdescription) {
                ...
                $o .= get_string('allowsubmissionsfromdatesummary', 'tarefalv', $date);
            } else {
                $date = userdate($status->allowsubmissionsfromdate);
                $o .= get_string('allowsubmissionsanddescriptionfromdatesummary', 'tarefalv', $date);
            }
            $o .= $this->output->box_end();
        }
        ...
        if ($status->teamsubmissionenabled) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('submissionteam', 'tarefalv'));
            $group = $status->submissiongroup;
            if ($group) {
                $cell2 = new html_table_cell(format_string($group->name, false, $status->context));
            } else if ($status->preventsubmissionnotingroup) {
                if (count($status->usergroups) == 0) {
                    $notification = new \core\output\notification(get_string('noteam', 'tarefalv'), 'error');
                    $notification->set_show_closebutton(false);
                    $cell2 = new html_table_cell(
                        $this->output->render($notification)
                    );
                    $warningmsg = $this->output->notification(get_string('noteam_desc', 'tarefalv'), 'error');
                } else if (count($status->usergroups) > 1) {
                    $notification = new \core\output\notification(get_string('multipleteams', 'tarefalv'), 'error');
                    $notification->set_show_closebutton(false);
                    $cell2 = new html_table_cell(
                        $this->output->render($notification)
                    );
                    $warningmsg = $this->output->notification(get_string('multipleteams_desc', 'tarefalv'), 'error');
                }
            } else {
                $cell2 = new html_table_cell(get_string('defaultteam', 'tarefalv'));
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
        }

        if ($status->attemptreopenmethod != TAREFALV_ATTEMPT_REOPEN_METHOD_NONE) {
            $currentattempt = 1;
            if (!$status->teamsubmissionenabled) {
            ...
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('attemptnumber', 'tarefalv'));
            $maxattempts = $status->maxattempts;
            if ($maxattempts == TAREFALV_UNLIMITED_ATTEMPTS) {
                $message = get_string('currentattempt', 'tarefalv', $currentattempt);
            } else {
                $message = get_string('currentattemptof', 'tarefalv', array('attemptnumber'=>$currentattempt,
        ...
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('submissionstatus', 'tarefalv'));
        if (!$status->teamsubmissionenabled) {
            if ($status->submission && $status->submission->status != TAREFALV_SUBMISSION_STATUS_NEW) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'tarefalv');
                ...
                if (!$status->submissionsenabled) {
                    $cell2 = new html_table_cell(get_string('noonlinesubmissions', 'tarefalv'));
                } else {
                    $cell2 = new html_table_cell(get_string('noattempt', 'tarefalv'));
                }
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            $group = $status->submissiongroup;
            if (!$group && $status->preventsubmissionnotingroup) {
                $cell2 = new html_table_cell(get_string('nosubmission', 'tarefalv'));
            } else if ($status->teamsubmission && $status->teamsubmission->status != TAREFALV_SUBMISSION_STATUS_NEW) {
                $teamstatus = $status->teamsubmission->status;
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'tarefalv');
                $groupid = 0;
        ...
        if ($status->locked) {
            ...
            $cell2 = new html_table_cell(get_string('submissionslocked', 'tarefalv'));
        ...
        // Grading status.
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('gradingstatus', 'tarefalv'));

        if ($status->gradingstatus == TAREFALV_GRADING_STATUS_GRADED ||
            $status->gradingstatus == TAREFALV_GRADING_STATUS_NOT_GRADED) {
            $cell2 = new html_table_cell(get_string($status->gradingstatus, 'tarefalv'));
        } else {
            $gradingstatus = 'markingworkflowstate' . $status->gradingstatus;
            $cell2 = new html_table_cell(get_string($gradingstatus, 'tarefalv'));
        }
        if ($status->gradingstatus == TAREFALV_GRADING_STATUS_GRADED ||
            $status->gradingstatus == TAREFALV_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($duedate > 0) {
            // Due date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('duedate', 'tarefalv'));
            ...
            if ($status->view == tarefalv_submission_status::GRADER_VIEW) {
                if ($status->cutoffdate) {
                    // Cut off date.
                    $row = new html_table_row();
                    $cell1 = new html_table_cell(get_string('cutoffdate', 'tarefalv'));
            ...
            if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('extensionduedate', 'tarefalv'));
            ...
            // Time remaining.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timeremaining', 'tarefalv'));
            if ($duedate - $time <= 0) {
                if (!$submission ||
                        $submission->status != TAREFALV_SUBMISSION_STATUS_SUBMITTED) {
                    if ($status->submissionsenabled) {
                        $overduestr = get_string('overdue', 'tarefalv', format_time($time - $duedate));
                        $cell2 = new html_table_cell($overduestr);
                        $cell2->attributes = array('class'=>'overdue');
                    } else {
                        $cell2 = new html_table_cell(get_string('duedatereached', 'tarefalv'));
                    }
                } else {
                    if ($submission->timemodified > $duedate) {
        ...
        if ($status->view == tarefalv_submission_status::GRADER_VIEW) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('editingstatus', 'tarefalv'));
            if ($status->canedit) {
                $cell2 = new html_table_cell(get_string('submissioneditable', 'tarefalv'));
                $cell2->attributes = array('class'=>'submissioneditable');
            } else {
                $cell2 = new html_table_cell(get_string('submissionnoteditable', 'tarefalv'));
        ...
        if (!empty($status->gradingcontrollerpreview)) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradingmethodpreview', 'tarefalv'));
        ...
        if ($submission) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timemodified', 'tarefalv'));
        ...
        // Links.
        if ($status->view == tarefalv_submission_status::STUDENT_VIEW) {
            if ($status->canedit) {
                if (!$submission || $submission->status == TAREFALV_SUBMISSION_STATUS_NEW) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/tarefalv/view.php', $urlparams),
                                                       get_string('addsubmission', 'tarefalv'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'tarefalv');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else if ($submission->status == TAREFALV_SUBMISSION_STATUS_REOPENED) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid,
                                       'action' => 'editprevioussubmission',
                                       'sesskey'=>sesskey());
                    $o .= $this->output->single_button(new moodle_url('/mod/tarefalv/view.php', $urlparams),
                                                       get_string('addnewattemptfromprevious', 'tarefalv'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattemptfromprevious_help', 'tarefalv');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/tarefalv/view.php', $urlparams),
                                                       get_string('addnewattempt', 'tarefalv'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattempt_help', 'tarefalv');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/tarefalv/view.php', $urlparams),
                                                       get_string('editsubmission', 'tarefalv'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'tarefalv');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
            ...
            if ($status->cansubmit) {
                ...
                $o .= $this->output->box_start('generalbox submissionaction');
                $o .= $this->output->single_button(new moodle_url('/mod/tarefalv/view.php', $urlparams),
                                                   get_string('submitassignment', 'tarefalv'), 'get');
                $o .= $this->output->box_start('boxaligncenter submithelp');
                $o .= get_string('submitassignment_help', 'tarefalv');
                $o .= $this->output->box_end();
                $o .= $this->output->box_end();
            }
        }
     ...
     *
     * @param tarefalv_attempt_history_chooser $history
     * @return string
     */
    public function render_tarefalv_attempt_history_chooser(tarefalv_attempt_history_chooser $history) {
        $o = '';

        $context = $history->export_for_template($this);
        $o .= $this->render_from_template('mod_tarefalv/attempt_history_chooser', $context);
     ...
     *
     * @param tarefalv_attempt_history $history
     * @return string
     */
    public function render_tarefalv_attempt_history(tarefalv_attempt_history $history) {
        $o = '';

        $submittedstr = get_string('submitted', 'tarefalv');
        $gradestr = get_string('grade');
        $gradedonstr = get_string('gradedon', 'tarefalv');
        $gradedbystr = get_string('gradedby', 'tarefalv');
        ...
        $o .= $this->output->heading(get_string('attempthistory', 'tarefalv'), 3);
        ...
        foreach ($history->submissions as $i => $submission) {
            ...
            if ($submission) {
                ...
                $submissionsummary = get_string('nosubmission', 'tarefalv');
            }

            $attemptsummaryparams = array('attemptnumber'=>$submission->attemptnumber+1,
                                          'submissionsummary'=>$submissionsummary);
            $o .= $this->heading(get_string('attemptheading', 'tarefalv', $attemptsummaryparams), 4);

            $t = new html_table();

            if ($submission) {
                $cell1 = new html_table_cell(get_string('submissionstatus', 'tarefalv'));
                $cell2 = new html_table_cell(get_string('submissionstatus_' . $submission->status, 'tarefalv'));
                ...
                foreach ($history->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $pluginsubmission = new tarefalv_submission_plugin_submission($plugin,
                                                                                    $submission,
                                                                                    tarefalv_submission_plugin_submission::SUMMARY,
            ...
            if ($grade) {
                // Heading 'feedback'.
                $title = get_string('feedback', 'tarefalv', $i);
                ...
                if ($history->cangrade) {
                    ...
                    $icon = new pix_icon('gradefeedback',
                                            get_string('editattemptfeedback', 'tarefalv', $grade->attemptnumber+1),
                                            'mod_tarefalv');
                ...
                foreach ($history->feedbackplugins as $plugin) {
                    if ($plugin->is_enabled() &&
                        ...
                        $cell1 = new html_table_cell($plugin->get_name());
                        $pluginfeedback = new tarefalv_feedback_plugin_feedback(
                            $plugin, $grade, tarefalv_feedback_plugin_feedback::SUMMARY, $history->coursemoduleid,
        ...
        $this->page->requires->yui_module('moodle-mod_tarefalv-history', 'Y.one("#' . $containerid . '").history');
     ...
     *
     * @param tarefalv_submission_plugin_submission $submissionplugin
     * @return string
     */
    public function render_tarefalv_submission_plugin_submission(tarefalv_submission_plugin_submission $submissionplugin) {
        $o = '';

        if ($submissionplugin->view == tarefalv_submission_plugin_submission::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewsubmission', 'tarefalv');
                $icon = $this->output->pix_icon('t/preview', $previewstr);

                $expandstr = get_string('viewfull', 'tarefalv');
                ...
                $this->page->requires->js_init_call('M.mod_tarefalv.init_plugin_summary', $jsparams);
            ...
            if ($showviewlink) {
                ...
                $o .= $this->output->pix_icon('t/switch_minus',
                                              get_string('viewsummary', 'tarefalv'),
                                              null,
                                              array('class'=>$classes));
                $o .= $submissionplugin->plugin->view($submissionplugin->submission);
                $o .= $this->output->box_end();
            }
        } else if ($submissionplugin->view == tarefalv_submission_plugin_submission::FULL) {
     ...
     *
     * @param tarefalv_grading_table $table
     * @return string
     */
    public function render_tarefalv_grading_table(tarefalv_grading_table $table) {
        $o = '';
        $o .= $this->output->box_start('boxaligncenter gradingtable');

        $this->page->requires->js_init_call('M.mod_tarefalv.init_grading_table', array());
        $this->page->requires->string_for_js('nousersselected', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmgrantextension', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmlock', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmreverttodraft', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmunlock', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmaddattempt', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmdownloadselected', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmsetmarkingworkflowstate', 'tarefalv');
        $this->page->requires->string_for_js('batchoperationconfirmsetmarkingallocation', 'tarefalv');
        $this->page->requires->string_for_js('editaction', 'tarefalv');
     ...
     *
     * @param tarefalv_feedback_plugin_feedback $feedbackplugin
     * @return string
     */
    public function render_tarefalv_feedback_plugin_feedback(tarefalv_feedback_plugin_feedback $feedbackplugin) {
        $o = '';

        if ($feedbackplugin->view == tarefalv_feedback_plugin_feedback::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewfeedback', 'tarefalv');
                $icon = $this->output->pix_icon('t/preview', $previewstr);
        ...
        } else if ($feedbackplugin->view == tarefalv_feedback_plugin_feedback::FULL) {
     ...
     *
     * @param tarefalv_course_index_summary $indexsummary
     * @return string
     */
    public function render_tarefalv_course_index_summary(tarefalv_course_index_summary $indexsummary) {
        $o = '';
        ...
        $strsubmission = get_string('submission', 'tarefalv');
        ...
        foreach ($indexsummary->assignments as $info) {
            ...
            $link = html_writer::link(new moodle_url('/mod/tarefalv/view.php', $params),
                                      $info['cmname']);
     ...
     * @return string
     */
    protected function htmllize_tree(tarefalv_files $tree, $dir) {
        global $CFG;
        $yuiconfig = array();
        $yuiconfig['type'] = 'html';
    ...
    public function render_grading_app(grading_app $app) {
        ...
        return $this->render_from_template('mod_tarefalv/grading_app', $context);
    }
}
