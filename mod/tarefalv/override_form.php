...
/**
 * Settings form for overrides in the tarefalv module.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the tarefalv settings object. */
    protected $tarefalv;

    /** @var context the tarefalv context. */
    protected $context;
     ...
     * @param object $tarefalv the tarefalv settings object.
     * @param object $context the tarefalv context.
     ...
     */
    public function __construct($submiturl, $cm, $tarefalv, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'tarefalv'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'tarefalv'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/tarefalv/overrides.php', array('cmid' => $cm->id));
                    print_error('groupsnone', 'tarefalv', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'tarefalv'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/tarefalv/overrides.php', array('cmid' => $cm->id));
                    print_error('usersnone', 'tarefalv', $link);
                }
        ...
        foreach ($users as $value) {
            $extension = $DB->get_record('tarefalv_user_flags', array('assignment' => $this->tarefalv->get_instance()->id,
                'userid' => $value));
            if ($extension) {
        ...
        if ($extensionmax) {
            $this->tarefalv->get_instance()->extensionduedate = $extensionmax;
        }

        // Open and close dates.
        $mform->addElement('date_time_selector', 'allowsubmissionsfromdate',
            get_string('allowsubmissionsfromdate', 'tarefalv'), array('optional' => true));
        $mform->setDefault('allowsubmissionsfromdate', $this->tarefalv->get_instance()->allowsubmissionsfromdate);

        $mform->addElement('date_time_selector', 'duedate', get_string('duedate', 'tarefalv'), array('optional' => true));
        $mform->setDefault('duedate', $this->tarefalv->get_instance()->duedate);

        $mform->addElement('date_time_selector', 'cutoffdate', get_string('cutoffdate', 'tarefalv'), array('optional' => true));
        $mform->setDefault('cutoffdate', $this->tarefalv->get_instance()->cutoffdate);

        if (isset($this->tarefalv->get_instance()->extensionduedate)) {
            $mform->addElement('static', 'extensionduedate', get_string('extensionduedate', 'tarefalv'),
                userdate($this->tarefalv->get_instance()->extensionduedate));
        }

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'tarefalv'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'tarefalv'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'tarefalv'));
    ...
    public function validation($data, $files) {
        ...
        if (!empty($tarefalv->get_instance()->extensionduedate) && !empty($data['duedate'])) {
            if ($tarefalv->get_instance()->extensionduedate < $data['duedate']) {
                $errors['duedate'] = get_string('extensionnotafterduedate', 'tarefalv');
            }
        }
        if (!empty($tarefalv->get_instance()->extensionduedate) && !empty($data['allowsubmissionsfromdate'])) {
            if ($tarefalv->get_instance()->extensionduedate < $data['allowsubmissionsfromdate']) {
                $errors['allowsubmissionsfromdate'] = get_string('extensionnotafterfromdate', 'tarefalv');
            }
        }

        // Ensure that at least one tarefalv setting was changed.
        $changed = false;
        $keys = array('duedate', 'cutoffdate', 'allowsubmissionsfromdate');
        foreach ($keys as $key) {
            if ($data[$key] != $tarefalv->get_instance()->{$key}) {
                $changed = true;
                break;
            }
        }
        ...
        if (!$changed) {
            $errors['allowsubmissionsfromdate'] = get_string('nooverridedata', 'tarefalv');
        }
