 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tarefalv_grading_table extends table_sql implements renderable {
    /** @var tarefalv $assignment */
     ...
     *
     * @param tarefalv $assignment The assignment class
     ...
     */
    public function __construct(tarefalv $assignment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_tarefalv_grading');
        $this->is_persistent(true);
        $this->assignment = $assignment;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/tarefalv:grantextension',
                                                  $this->assignment->get_context());
        $this->hasgrade = $this->assignment->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/tarefalv:viewblinddetails',
                $this->assignment->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_tarefalv');

        $urlparams = array('action'=>'grading', 'id'=>$assignment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/tarefalv/view.php', $urlparams);
        ...
        $params['newstatus'] = TAREFALV_SUBMISSION_STATUS_NEW;
        ...
        // For group submissions we don't immediately create an entry in the tarefalv_submission table for each user,
        ...
        $from .= 'LEFT JOIN {tarefalv_user_flags} uf
                         ON u.id = uf.userid
        ...
        if ($hasoverrides) {
            ...
            $from .= ' LEFT JOIN (
               SELECT merged.userid, min(merged.priority) priority FROM (
                  ...
                  ( SELECT uo.userid, 0 AS priority
                      FROM {tarefalv_overrides} uo
                     WHERE uo.tarefalvid = :assignmentid5
                  )
                  UNION
                  ( SELECT gm.userid, go.sortorder AS priority
                      FROM {tarefalv_overrides} go
                      JOIN {groups} g ON g.id = go.groupid
                      JOIN {groups_members} gm ON gm.groupid = g.id
                     WHERE go.tarefalvid = :assignmentid6
                  )
                ) merged
            ...
            JOIN (
              (SELECT 9999999 AS priority,
                      ...
                      a.cutoffdate
                 FROM {user} u
                 JOIN {tarefalv} a ON a.id = :assignmentid7
              )
              UNION
              (SELECT 0 AS priority,
                      ...
                      uo.cutoffdate
                 FROM {tarefalv_overrides} uo
                WHERE uo.tarefalvid = :assignmentid8
              )
              UNION
              (SELECT go.sortorder AS priority,
                 ...
                 JOIN {groups} g ON g.id = go.groupid
                 JOIN {groups_members} gm ON gm.groupid = g.id
                WHERE go.tarefalvid = :assignmentid9
              )
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if ($filter == TAREFALV_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = TAREFALV_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == TAREFALV_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status <> :submitted) ';
                $params['submitted'] = TAREFALV_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == TAREFALV_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
            ...
            } else if ($filter == TAREFALV_FILTER_GRANTED_EXTENSION) {
                $where .= ' AND uf.extensionduedate > 0 ';

            } else if (strpos($filter, TAREFALV_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->assignment->get_instance()->markingworkflow &&
            ...
            if (has_capability('mod/tarefalv:manageallocations', $this->assignment->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('tarefalv_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == TAREFALV_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->assignment->get_instance()->markingworkflow) {
            ...
            if (!empty($workflowstates)) {
                ...
                if ($workflowfilter == TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('tarefalv_user_flags', 'workflowstate', true, true).')';
        ...
        if ($this->hasviewblind || !$this->assignment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                ...
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'tarefalv');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'tarefalv');

        if ($hasoverrides) {
            // Allowsubmissionsfromdate.
            $columns[] = 'allowsubmissionsfromdate';
            $headers[] = get_string('allowsubmissionsfromdate', 'tarefalv');

            // Duedate.
            $columns[] = 'duedate';
            $headers[] = get_string('duedate', 'tarefalv');

            // Cutoffdate.
            $columns[] = 'cutoffdate';
            $headers[] = get_string('cutoffdate', 'tarefalv');
        }

        // Team submission columns.
        if ($assignment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'tarefalv');
        }
        // Allocated marker.
        if ($this->assignment->get_instance()->markingworkflow &&
            $this->assignment->get_instance()->markingallocation &&
            has_capability('mod/tarefalv:manageallocations', $this->assignment->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'tarefalv');
        }
        // Grade.
        $columns[] = 'grade';
        ...
        if ($assignment->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'tarefalv');
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'tarefalv');
    ...
    public function col_recordid(stdClass $row) {
        ...
        return get_string('hiddenuser', 'tarefalv') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        if (empty($workflowstate)) {
            $workflowstate = TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED;
        }
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'tarefalv'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'tarefalv');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_enrolled_users($this->assignment->get_context(), 'mod/tarefalv:grade', 0, 'u.*', $sort);
            $markerlist[0] = get_string('choosemarker', 'tarefalv');
        ...
        if ($this->quickgrading && has_capability('mod/tarefalv:manageallocations', $this->assignment->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == TAREFALV_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            ...
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'tarefalv');
            } else {
                return get_string('noteamgrader', 'tarefalv');
            }
        }
        return get_string('defaultteam', 'tarefalv');
    }
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'tarefalv', $this->assignment->fullname($row));
        $selectcol .= '</label>';
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != TAREFALV_SUBMISSION_STATUS_NEW) {
            $o = userdate($submission->timemodified);
        } else if ($row->timesubmitted && $row->status != TAREFALV_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'tarefalv'),
                                           array('class'=>'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != TAREFALV_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'tarefalv');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'tarefalv'), 'submissiongraded');
                } else if (!$timesubmitted || $status == TAREFALV_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'tarefalv', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'tarefalv', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'tarefalv');
        }
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == TAREFALV_SUBMISSION_STATUS_DRAFT ||
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'tarefalv');
                    ...
                    $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'tarefalv');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
                $description = get_string('editsubmission', 'tarefalv');
        ...
        if (($this->assignment->get_instance()->duedate ||
             ...
             $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
             $description = get_string('grantextension', 'tarefalv');
        ...
        if ($row->status == TAREFALV_SUBMISSION_STATUS_SUBMITTED &&
                $this->assignment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'tarefalv');
        ...
        if ($row->status == TAREFALV_SUBMISSION_STATUS_DRAFT &&
                $this->assignment->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
            $description = get_string('submitforgrading', 'tarefalv');
        ...
        $ismanual = $this->assignment->get_instance()->attemptreopenmethod == TAREFALV_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != TAREFALV_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->assignment->get_instance()->maxattempts == TAREFALV_UNLIMITED_ATTEMPTS;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/tarefalv/view.php', $urlparams);
            $description = get_string('addattempt', 'tarefalv');
     ...
     *
     * @param tarefalv_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(tarefalv_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('tarefalv')), 'tarefalv');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'tarefalvsubmission') {
                if ($this->assignment->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == TAREFALV_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
