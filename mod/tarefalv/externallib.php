 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_tarefalv_external extends external_api {
    ...
    /**
     * Returns grade information from tarefalv_grades for the requested assignment ids
    ...
    public static function get_grades($assignmentids, $since = 0) {
        ...
        $placeholders['modname'] = 'tarefalv';
        $cms = $DB->get_records_sql($sql, $placeholders);
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/tarefalv:grade', $context);
            } catch (Exception $e) {
        ...
        if (count ($requestedassignmentids) > 0) {
            ...
            $sql = "SELECT ag.id,
                           ...
                           ag.grade,
                           ag.attemptnumber
                      FROM {tarefalv_grades} ag, {tarefalv_submission} s
    ...
    /**
     * Creates an tarefalv_grades external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.4
     */
    private static function tarefalv_grades() {
        return new external_single_structure(
            array (
    ...
    public static function get_grades_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::tarefalv_grades(), 'list of assignment grade information'),
    ...
    public static function get_assignments($courseids = array(), $capabilities = array(), $includenotenrolledcourses = false) {
        ...
        foreach ($courses as $id => $course) {
            ...
            if ($modules = get_coursemodules_in_course('tarefalv', $courses[$id]->id, $extrafields)) {
                foreach ($modules as $module) {
                    ...
                    // Update tarefalv with override information.
                    $tarefalv->update_effective_access($USER->id);

                    // Get configurations for only enabled plugins.
                    $plugins = $tarefalv->get_submission_plugins();
                    $plugins = array_merge($plugins, $tarefalv->get_feedback_plugins());
                    ...
                    $assignment = array(
                        ...
                        'duedate' => $tarefalv->get_instance()->duedate,
                        'allowsubmissionsfromdate' => $tarefalv->get_instance()->allowsubmissionsfromdate,
                        'grade' => $module->grade,
                        'timemodified' => $module->timemodified,
                        'completionsubmit' => $module->completionsubmit,
                        'cutoffdate' => $tarefalv->get_instance()->cutoffdate,
                        'gradingduedate' => $tarefalv->get_instance()->gradingduedate,
                    ...
                    if ($tarefalv->show_intro()) {
                        ...
                        list($assignment['intro'], $assignment['introformat']) = external_format_text($module->intro,
                            $module->introformat, $context->id, 'mod_tarefalv', 'intro', null);
                        $assignment['introfiles'] = external_util::get_area_files($context->id, 'mod_tarefalv', 'intro', false,
                                                                                    false);

                        $assignment['introattachments'] = external_util::get_area_files($context->id, 'mod_tarefalv',
                                                            TAREFALV_INTROATTACHMENT_FILEAREA, 0);
                    }
                    ...
                    if ($module->requiresubmissionstatement) {
                        ...
                        list($assignment['submissionstatement'], $assignment['submissionstatementformat']) = external_format_text(
                                $adminconfig->submissionstatement, FORMAT_MOODLE, $context->id, 'mod_tarefalv', '', 0);
                    }
    ...
    /**
     * Creates an tarefalv_plugin_config external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_assignments_config_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'tarefalv_plugin_config id', VALUE_OPTIONAL),
                'assignment' => new external_value(PARAM_INT, 'assignment id', VALUE_OPTIONAL),
     ...
     *
     * @param  tarefalv $tarefalv the assignment object
     * @param  array $tarefalvplugins array of assignment plugins (submission or feedback)
     * @param  stdClass $item the item object (submission or grade)
     * @return array an array containing the plugins returned information
     */
    private static function get_plugins_data($tarefalv, $tarefalvplugins, $item) {
        global $CFG;

        $plugins = array();
        $fs = get_file_storage();

        foreach ($tarefalvplugins as $tarefalvplugin) {

            if (!$tarefalvplugin->is_enabled() or !$tarefalvplugin->is_visible()) {
                continue;
            }

            $plugin = array(
                'name' => $tarefalvplugin->get_name(),
                'type' => $tarefalvplugin->get_type()
            );
            // Subtype is 'tarefalvsubmission', type is currently 'file' or 'onlinetext'.
            $component = $tarefalvplugin->get_subtype().'_'.$tarefalvplugin->get_type();

            $fileareas = $tarefalvplugin->get_file_areas();
            foreach ($fileareas as $filearea => $name) {
                $fileareainfo = array('area' => $filearea);

                $fileareainfo['files'] = external_util::get_area_files(
                    $tarefalv->get_context()->id,
                    $component,
                    $filearea,
                    $item->id
                );
            ...
            $editorfields = $tarefalvplugin->get_editor_fields();
            foreach ($editorfields as $name => $description) {
                $editorfieldinfo = array(
                    'name' => $name,
                    'description' => $description,
                    'text' => $tarefalvplugin->get_editor_text($name, $item->id),
                    'format' => $tarefalvplugin->get_editor_format($name, $item->id)
                );

                // Now format the text.
                foreach ($fileareas as $filearea => $name) {
                    list($editorfieldinfo['text'], $editorfieldinfo['format']) = external_format_text(
                        $editorfieldinfo['text'], $editorfieldinfo['format'], $tarefalv->get_context()->id,
    ...
    public static function get_submissions($assignmentids, $status = '', $since = 0, $before = 0) {
        ...
        foreach ($tarefalvs as $tarefalv) {
            $submissions = array();
            $placeholders = array('tarefalvid1' => $tarefalv->get_instance()->id,
                                  'tarefalvid2' => $tarefalv->get_instance()->id);

            $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                     FROM {tarefalv_submission} mxs
                                     WHERE mxs.assignment = :tarefalvid1 GROUP BY mxs.userid';

            $sql = "SELECT mas.id, mas.assignment,mas.userid,".
                   "mas.timecreated,mas.timemodified,mas.status,mas.groupid,mas.attemptnumber ".
                   "FROM {tarefalv_submission} mas ".
                   "JOIN ( " . $submissionmaxattempt . " ) smx ON mas.userid = smx.userid ".
                   "WHERE mas.assignment = :tarefalvid2 AND mas.attemptnumber = smx.maxattempt";
            ...
            if (!empty($submissionrecords)) {
                $submissionplugins = $tarefalv->get_submission_plugins();
                foreach ($submissionrecords as $submissionrecord) {
                    $submission = array(
                        ...
                        'plugins' => self::get_plugins_data($tarefalv, $submissionplugins, $submissionrecord),
                        'gradingstatus' => $tarefalv->get_grading_status($submissionrecord->userid)
                    );
                ...
                $warnings[] = array(
                    'item' => 'module',
                    'itemid' => $tarefalv->get_instance()->id,
                    'warningcode' => '3',
            ...
            $assignments[] = array(
                'assignmentid' => $tarefalv->get_instance()->id,
    ...
    /**
     * Creates an tarefalv_submissions external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    ...
    public static function set_user_flags($assignmentid, $userflags = array()) {
        ...
        list($tarefalv, $course, $cm, $context) = self::validate_tarefalv($params['assignmentid']);
        require_capability('mod/tarefalv:grade', $context);
        ...
        foreach ($params['userflags'] as $userflag) {
            ...
            $record = $tarefalv->get_user_flags($userflag['userid'], false);
            if ($record) {
                ...
                if ($tarefalv->update_user_flags($record)) {
                    $result['id'] = $record->id;
                ...
                $record = $tarefalv->get_user_flags($userflag['userid'], true);
    ...
    /**
     * Returns user flag information from tarefalv_user_flags for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_flags($assignmentids) {
        ...
        // Create the query and populate an array of tarefalv_user_flags records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);
    ...
    /**
     * Creates an tarefalv_user_flags external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function tarefalv_user_flags() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_flags_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::tarefalv_user_flags(), 'list of tarefalv user flag information'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    /**
     * Returns user mapping information from tarefalv_user_mapping for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_mappings($assignmentids) {
        ...
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/tarefalv:revealidentities', $context);
            } catch (Exception $e) {
                $requestedassignmentids = array_diff($requestedassignmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'assignment';
        ...
        // Create the query and populate an array of tarefalv_user_mapping records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT aum.id,aum.assignment,aum.userid ".
                   "FROM {tarefalv_user_mapping} aum ".
    ...
    /**
     * Creates an tarefalv_user_mappings external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function tarefalv_user_mappings() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_mappings_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::tarefalv_user_mappings(), 'list of tarefalv user mapping data'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    public static function lock_submissions($assignmentid, $userids) {
        ...
        list($assignment, $course, $cm, $context) = self::validate_tarefalv($params['assignmentid']);
    ...
    public static function submit_grading_form($assignmentid, $userid, $jsonformdata) {
        ...
        if (WS_SERVER) {
            ...
            $data['_qf__mod_tarefalv_grade_form_'.$params['userid']] = 1;
        }
        ...
        $mform = new mod_tarefalv_grade_form(null, $formparams, 'post', '', null, true, $data);
    ...
    public static function save_submission_parameters() {
        global $CFG;
        $instance = new tarefalv(null, null, null);
    ...
    public static function view_grading_table_parameters() {
        return new external_function_parameters(
            array(
                'tarefalvid' => new external_value(PARAM_INT, 'tarefalv instance id')
            )
        );
    }
     ...
     *
     * @param int $tarefalvid the tarefalv instance id
     * @return array of warnings and status result
     * @since Moodle 3.0
     * @throws moodle_exception
     */
    public static function view_grading_table($tarefalvid) {

        $params = self::validate_parameters(self::view_grading_table_parameters(),
                                            array(
                                                'tarefalvid' => $tarefalvid
                                            ));
        $warnings = array();

        list($tarefalv, $course, $cm, $context) = self::validate_tarefalv($params['tarefalvid']);

        $tarefalv->require_view_grades();
        \mod_tarefalv\event\grading_table_viewed::create_from_tarefalv($tarefalv)->trigger();
     ...
     *
     * @param int $tarefalvid tarefalv instance id
     * @return array of warnings and status result
     * @since Moodle 3.1
     */
    public static function view_submission_status($tarefalvid) {
        ...
        \mod_tarefalv\event\submission_status_viewed::create_from_tarefalv($tarefalv)->trigger();
    ...
    public static function get_submission_status_parameters() {
        return new external_function_parameters (
            array(
                'tarefalvid' => new external_value(PARAM_INT, 'assignment instance id'),
     ...
     *
     * @param int $tarefalvid assignment instance id
     * @param int $userid user id (empty for current user)
     * @return array of warnings and grading, status, feedback and previous attempts information
     * @since Moodle 3.1
     * @throws required_capability_exception
     */
    public static function get_submission_status($tarefalvid, $userid = 0) {
        global $USER;
        ...
        if (!$tarefalv->can_view_submission($user->id)) {
            throw new required_capability_exception($context, 'mod/tarefalv:viewgrades', 'nopermission', '');
        }

        $gradingsummary = $lastattempt = $feedback = $previousattempts = null;

        // Get the renderable since it contais all the info we need.
        if ($tarefalv->can_view_grades()) {
            $gradingsummary = $tarefalv->get_tarefalv_grading_summary_renderable();
        }

        // Retrieve the rest of the renderable objects.
        if (has_capability('mod/tarefalv:submit', $tarefalv->get_context(), $user)) {
            $lastattempt = $tarefalv->get_tarefalv_submission_status_renderable($user, true);
        }

        $feedback = $tarefalv->get_tarefalv_feedback_status_renderable($user);

        $previousattempts = $tarefalv->get_tarefalv_attempt_history_renderable($user);
        ...
        if ($lastattempt) {
            ...
            if (empty($lastattempt->submission)) {
                ...
                $lastattempt->submission->plugins = self::get_plugins_data($tarefalv, $submissionplugins, $lastattempt->submission);
            }

            if (empty($lastattempt->teamsubmission)) {
                unset($lastattempt->teamsubmission);
            } else {
                $lastattempt->teamsubmission->plugins = self::get_plugins_data($tarefalv, $submissionplugins,
            ...
            $lastattempt->caneditowner = $tarefalv->submissions_open($user->id) && $tarefalv->is_any_submission_plugin_enabled();
        ...
        if ($feedback) {
            if ($feedback->grade) {
                $feedbackplugins = $tarefalv->get_feedback_plugins();
                $feedback->plugins = self::get_plugins_data($tarefalv, $feedbackplugins, $feedback->grade);
            } else {
        ...
        if ($previousattempts and count($previousattempts->submissions) > 1) {
            ...
            foreach ($previousattempts->submissions as $i => $submission) {
                ...
                if ($submission) {
                    $submission->plugins = self::get_plugins_data($tarefalv, $previousattempts->submissionplugins, $submission);
                ...
                if ($grade) {
                    ...
                    $feedbackplugins = self::get_plugins_data($tarefalv, $previousattempts->feedbackplugins, $grade);
     ...
     * @throws moodle_exception
     */
    public static function list_participants($tarefalvid, $groupid, $filter, $skip, $limit, $onlyids, $includeenrolments) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/tarefalv/locallib.php");
        ...
        if (groups_group_visible($params['groupid'], $course, $cm)) {
            $participants = $tarefalv->list_participants_with_filter_status_and_group($params['groupid']);
        }
        ...
        foreach ($participants as $record) {
            ...
            if ($match) {
                ...
                if (!$tarefalv->is_blind_marking() && !$params['onlyids']) {
    ...
    /**
     * Returns the description of the results of the mod_tarefalv_external::list_participants() method.
     *
     * @return external_description
     * @since Moodle 3.1
     */
     ...
     * @throws moodle_exception
     */
    public static function get_participant($tarefalvid, $userid, $embeduser) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/tarefalv/locallib.php");
        require_once($CFG->dirroot . "/user/lib.php");
        ...
        $participant = $tarefalv->get_participant($params['userid']);

        // Update tarefalv with override information.
        $tarefalv->update_effective_access($params['userid']);
        ...
        $return = array(
            ...
            'blindmarking' => $tarefalv->is_blind_marking(),
            'allowsubmissionsfromdate' => $tarefalv->get_instance()->allowsubmissionsfromdate,
            'duedate' => $tarefalv->get_instance()->duedate,
            'cutoffdate' => $tarefalv->get_instance()->cutoffdate,
            'duedatestr' => userdate($tarefalv->get_instance()->duedate, get_string('strftimedatetime', 'langconfig')),
        );
        ...
        if (!$tarefalv->is_blind_marking() && $embeduser) {
    ...
    /**
     * Utility function for validating an tarefalv.
     *
     * @param int $tarefalvid tarefalv instance id
     * @return array array containing the tarefalv, course, context and course module objects
     * @since  Moodle 3.2
     */
    protected static function validate_tarefalv($tarefalvid) {
        global $DB;

        // Request and permission validation.
        $tarefalv = $DB->get_record('tarefalv', array('id' => $tarefalvid), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($tarefalv, 'tarefalv');

        $context = context_module::instance($cm->id);
        // Please, note that is not required to check mod/tarefalv:view because is done by validate_context->require_login.
        self::validate_context($context);
        $tarefalv = new tarefalv($context, $cm, $course);

        return array($tarefalv, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_tarefalv.
     *
     * @return external_function_parameters
     * @since Moodle 3.2
     */
    public static function view_tarefalv_parameters() {
        return new external_function_parameters (
            array(
                'tarefalvid' => new external_value(PARAM_INT, 'tarefalv instance id'),
            )
     ...
     * @since Moodle 3.2
     */
    public static function view_tarefalv($tarefalvid) {
        $warnings = array();
        $params = array(
            'tarefalvid' => $tarefalvid,
        );
        $params = self::validate_parameters(self::view_tarefalv_parameters(), $params);
    ...
    /**
     * Describes the view_tarefalv return value.
     *
     * @return external_single_structure
     * @since Moodle 3.2
     */
    public static function view_tarefalv_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings(),
