...
$url = new moodle_url('/mod/tarefalv/type/upload/upload.php', array('contextid'=>$contextid,
...
$instance = new tarefalv_upload($cm->id, $tarefalv, $cm, $course);
$submission = $instance->get_submission($formdata->userid, false);

$filemanager_options = array('subdirs'=>1, 'maxbytes'=>$tarefalv->maxbytes, 'maxfiles'=>$tarefalv->var1, 'accepted_types'=>'*', 'return_types'=>FILE_INTERNAL);

    $mform = new mod_tarefalv_upload_form(null, array('contextid'=>$contextid, 'userid'=>$formdata->userid, 'options'=>$filemanager_options));
...
if ($instance->can_upload_file($submission) && ($id==null)) {
    ...
    $data = file_prepare_standard_filemanager($data, 'files', $filemanager_options, $context, 'mod_tarefalv', 'submission', $submissionid);
    // set file manager itemid, so it will find the files in draft area
    $mform->set_data($data);
    $mform->display();
}else {
    echo $OUTPUT->notification(get_string('uploaderror', 'tarefalv'));
    echo $OUTPUT->continue_button(new moodle_url('/mod/tarefalv/view.php', array('id'=>$cm->id)));
}
echo $OUTPUT->box_end();
