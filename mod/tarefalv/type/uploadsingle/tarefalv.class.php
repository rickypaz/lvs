...
class tarefalv_uploadsingle extends tarefalv_base {
    ...
    function tarefalv_uploadsingle($cmid='staticonly', $tarefalv=NULL, $cm=NULL, $course=NULL) {
    ...
    function view() {
        ...
        if (is_enrolled($this->context, $USER, 'mod/tarefalv:submit') && $this->isopen() && (!$filecount || $this->tarefalv->resubmit || !$submission->timemarked)) {
    ...
    function view_upload_form() {
        ...
        if ($submission = $this->get_submission($USER->id)) {
            if ($files = $fs->get_area_files($this->context->id, 'mod_tarefalv', 'submission', $submission->id, "timemodified", false)) {
                $str = get_string('editthisfile', 'tarefalv');
            } else {
                $str = get_string('uploadafile', 'tarefalv');
            }
        } else {
            $str = get_string('uploadafile', 'tarefalv');
        }
        echo $OUTPUT->single_button(new moodle_url('/mod/tarefalv/type/uploadsingle/upload.php', array('contextid'=>$this->context->id, 'userid'=>$USER->id)), $str, 'get');
        echo $OUTPUT->box_end();
    }
    ...
    function upload_file($mform) {
        ...
        $viewurl = new moodle_url('/mod/tarefalv/view.php', array('id'=>$this->cm->id));
        ...
        if ($this->isopen() && (!$filecount || $this->tarefalv->resubmit || !$submission->timemarked)) {
            if ($submission = $this->get_submission($USER->id)) {
                //TODO: change later to ">= 0", to prevent resubmission when graded 0
                if (($submission->grade > 0) and !$this->tarefalv->resubmit) {
                    redirect($viewurl, get_string('alreadygraded', 'tarefalv'));
                }
            }
            ...
            if ($formdata = $mform->get_data()) {
                ...
                if ($newfilename = $mform->get_new_filename('tarefalv_file')) {
                    $file = $mform->save_stored_file('tarefalv_file', $this->context->id, 'mod_tarefalv', 'submission',
                ...
                redirect($viewurl, get_string('uploaderror', 'tarefalv'));  //submitting not allowed!
            }
        }
    ...
    function upload_responsefile($mform) {
        ...
        $returnurl = new moodle_url("/mod/tarefalv/submissions.php", array('id'=>$this->cm->id,'userid'=>$userid,'mode'=>$mode,'offset'=>$offset)); //not xhtml, just url.
        ...
        if ($formdata = $mform->get_data() and $this->can_manage_responsefiles()) {
            ...
            $fs->delete_area_files($this->context->id, 'mod_tarefalv', 'response', $submission->id);

            if ($newfilename = $mform->get_new_filename('tarefalv_file')) {
                $file = $mform->save_stored_file('tarefalv_file', $this->context->id,
                        'mod_tarefalv', 'response',$submission->id, '/', $newfilename);
            }
            redirect($returnurl, get_string('uploadedfile'));
        } else {
            redirect($returnurl, get_string('uploaderror', 'tarefalv'));  //submitting not allowed!
        }
    }
    ...
    function send_file($filearea, $args, $forcedownload, array $options=array()) {
        ...
        $fullpath = '/'.$this->context->id.'/mod_tarefalv/'.$filearea.'/'.$submissionid.'/'.$relativepath;
    ...
    function extend_settings_navigation($node) {
        ...
        if (isset($submission->numfiles) AND $submission->numfiles) {
            ...
            if (!empty($submission->timemodified)) {
                $submissionnode = $node->add(get_string('submitted', 'tarefalv') . ' ' . userdate($submission->timemodified));
...
class mod_tarefalv_uploadsingle_response_form extends moodleform {
    function definition() {
        $mform = $this->_form;
        $instance = $this->_customdata;

        // visible elements
        $mform->addElement('filepicker', 'tarefalv_file', get_string('uploadafile'), null, $instance->options);
