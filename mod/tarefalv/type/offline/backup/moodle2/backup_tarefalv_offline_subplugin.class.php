 ...
 * needed to backup one tarefalv->offline subplugin.
 *
 * Note: Offline tarefalvs really haven't any special subplugin
 ...
 * elements (tarefalv and submission) and conditions
 */
class backup_tarefalv_offline_subplugin extends backup_subplugin {

    /**
     * Returns the subplugin information to attach at tarefalv element
     */
    protected function define_tarefalv_subplugin_structure() {
        ...
        /**
         * Here we are defining the information that will be attached, within the "tarefalv" element
         * when tarefalvs of type "offline" are sent to backup, so we define the backup_subplugin_element
         * as not having any final element (null) and with the condition of the '/tarefalv/tarefalvtype'
         * being 'offline' (that will be checked on execution)
         *
         * Note that, while, we allow direct "injection" of final_elements at the "tarefalv" level (without
         ...
         */
        $subplugin = $this->get_subplugin_element(null, '/tarefalv/tarefalvtype', 'offline');
         ...
         * with tarefalvs. Take it as just one example. The only important bit is that it's highly recommended to
         * use some exclusive name in the main nested element (something that won't conflict with other subplugins/parts).
         * So we are using 'subplugin_tarefalv_offline_tarefalv' as name here (the type of the subplugin, the name of the
        ...
        $config->set_source_table('config', array('id' => '/tarefalv/id'));
