...
class restore_tarefalv_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('tarefalv', '/activity/tarefalv');
        if ($userinfo) {
            $submission = new restore_path_element('tarefalv_submission',
                                                   '/activity/tarefalv/submissions/submission');
            $paths[] = $submission;
            $this->add_subplugin_structure('tarefalvsubmission', $submission);
            $grade = new restore_path_element('tarefalv_grade', '/activity/tarefalv/grades/grade');
            $paths[] = $grade;
            $this->add_subplugin_structure('tarefalvfeedback', $grade);
            $userflag = new restore_path_element('tarefalv_userflag',
                                                   '/activity/tarefalv/userflags/userflag');
            $paths[] = $userflag;
        }

        $paths[] = new restore_path_element('tarefalv_override', '/activity/tarefalv/overrides/override');
        $paths[] = new restore_path_element('tarefalv_plugin_config',
                                            '/activity/tarefalv/plugin_configs/plugin_config');

        return $this->prepare_activity_structure($paths);
    }

    /**
     * Process an tarefalv restore.
     *
    ...
    protected function process_tarefalv_submission($data) {
        ...
        $newitemid = $DB->insert_record('tarefalv_submission', $data);
     ...
     * @return void
     */
    protected function process_tarefalv_userflag($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('tarefalv_user_flags', $data);
    }
     ...
     * @return void
     */
    protected function process_tarefalv_grade($data) {
        global $DB;
        ...
        if (!empty($data->extensionduedate) ||
                ...
                !empty($data->locked)) {
            $flags = new stdClass();
            $flags->assignment = $this->get_new_parentid('tarefalv');
            if (!empty($data->extensionduedate)) {
        ...
        if ($data->grade < 0 && $data->grade != TAREFALV_GRADE_NOT_SET) {
            $data->grade = TAREFALV_GRADE_NOT_SET;
        }
        $newitemid = $DB->insert_record('tarefalv_grades', $data);
     ...
     * @return void
     */
    protected function process_tarefalv_plugin_config($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('tarefalv_plugin_config', $data);
    }
    ...
    protected function set_latest_submission_field() {
        ...
        $assignmentid = $this->get_new_parentid('tarefalv');
        ...
        $DB->insert_records('tarefalv_submission', $submissions);
        ...
        $sql = 'SELECT DISTINCT userid FROM {tarefalv_submission} WHERE assignment = ? AND groupid = ?';
        ...
        $sql = 'SELECT DISTINCT groupid FROM {tarefalv_submission} WHERE assignment = ? AND userid = ?';
        $params = array($assignmentid, 0);
    ...
    /**
     * Process a tarefalv override restore
     * @param object $data The data in object form
     * @return void
     */
    protected function process_tarefalv_override($data) {
        global $DB;
        ...
        $data->tarefalvid = $this->get_new_parentid('tarefalv');
        ...
        $newitemid = $DB->insert_record('tarefalv_overrides', $data);

        // Add mapping, restore of logs needs it.
        $this->set_mapping('tarefalv_override', $oldid, $newitemid);
    }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('mod_tarefalv', 'introattachment', null);

        $this->add_plugin_config_files('tarefalvsubmission');
        $this->add_plugin_config_files('tarefalvfeedback');
