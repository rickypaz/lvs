...
/**
 * This file contains the functions for tarefalv_plugin abstract class
 *
 *
...
/**
 * Abstract class for tarefalv_plugin (submission/feedback).
 *
 * @package   mod_tarefalv
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class tarefalv_plugin {

    /** @var tarefalv $assignment the assignment record that contains the global
     *              settings for this tarefalv instance
     */
     ...
     *
     * @param tarefalv $assignment
     * @param string $type
     */
    public final function __construct(tarefalv $assignment, $type) {
     ...
     *
     * @return string - either 'tarefalvsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - tarefalv_submission or tarefalv_grade.
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/tarefalv/' . substr($this->get_subtype(), strlen('tarefalv')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
                          ...
                          'name'=>$name);
        $current = $DB->get_record('tarefalv_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('tarefalv_plugin_config', $current);
        } else {
            ...
            return $DB->insert_record('tarefalv_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($assignment) {
                $dbparams = array('assignment'=>$assignment->id,
                                  ...
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('tarefalv_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('tarefalv_plugin_config', $dbparams);
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldassignment, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_tarefalv', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade tarefalv_submission or tarefalv_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_tarefalv', $params);
        return false;
    }
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'tarefalvsubmission') {
            if ($itemid) {
                $record = $DB->get_record('tarefalv_submission', array('id' => $itemid), 'userid,groupid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
     ...
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=tarefalvfeedback|tarefalvsubmission
