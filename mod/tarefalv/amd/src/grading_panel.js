 ...
 *
 * @module     mod_tarefalv/grading_panel
...
define(['jquery', 'core/yui', 'core/notification', 'core/templates', 'core/fragment',
        'core/ajax', 'core/str', 'mod_tarefalv/grading_form_change_checker',
        'mod_tarefalv/grading_events'],
    ...
    GradingPanel.prototype._submitForm = function(event, nextUserId, nextUser) {
        ...
        // Now we can continue...
        ajax.call([{
            methodname: 'mod_tarefalv_submit_grading_form',
    ...
    GradingPanel.prototype._handleFormSubmissionResponse = function(formdata, nextUserId, nextUser, response) {
        ...
        if (response.length) {
            ...
            str.get_strings([
                ...
                {key: 'gradechangessaveddetail', component: 'mod_tarefalv'},
            ]).done(function(strs) {
    ...
    GradingPanel.prototype._chooseAttempt = function(e) {
        ...
        str.get_strings([
            {key: 'viewadifferentattempt', component: 'mod_tarefalv'},
    ...
    GradingPanel.prototype._addPopoutButtons = function(selector) {
        ...
        templates.render('mod_tarefalv/popout_button', {}).done(function(html) {
    ...
    GradingPanel.prototype._refreshGradingPanel = function(event, userid, submissiondata, attemptnumber) {
        ...
        window.M.util.js_pending('mod-tarefalv-loading-user');
        // First insert the loading template.
        templates.render('mod_tarefalv/loading', {}).done(function(html, js) {
            // Update the page.
            this._niceReplaceNodeContents(this._region, html, js).done(function() {
                if (userid > 0) {
                    ...
                    fragment.loadFragment('mod_tarefalv', 'gradingpanel', contextid, params).done(function(html, js) {
                        ...
                        .done(function() {
                            ...
                            window.M.util.js_complete('mod-tarefalv-loading-user');
                        }.bind(this))
                        .fail(notification.exception);
                    }.bind(this)).fail(notification.exception);
