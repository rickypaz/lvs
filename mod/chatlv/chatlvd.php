...
echo 'Moodle chatlv daemon v1.0 on PHP '.$phpversion."\n\n";

// Set up all the variables we need.

// The $CFG variables are now defined in database by chatlv/lib.php.

$_SERVER['PHP_SELF']        = 'dummy';
$_SERVER['SERVER_NAME']     = 'dummy';
$_SERVER['HTTP_USER_AGENT'] = 'dummy';

$_SERVER['SERVER_NAME'] = $CFG->chatlv_serverhost;
$_SERVER['PHP_SELF']    = "http://$CFG->chatlv_serverhost:$CFG->chatlv_serverport/mod/chatlv/chatlvd.php";

core_php_time_limit::raise(0);
error_reporting(E_ALL);

function chatlv_empty_connection() {
...
class ChatDaemon {
    ...
    public $_logfile_name      = 'chatlvd.log';
    ...
    public $chatlvrooms = array(); // Keyed by chatlvid, holding arrays of data.

    // IMPORTANT: $connectionssets, $setsinfo and $chatlvrooms must remain synchronized!
    ...
    public function __construct() {
        ...
        $this->_beepsoundsrc        = $GLOBALS['CFG']->wwwroot.'/mod/chatlv/beep.wav';
        $this->_freq_update_records = 20;
        $this->_freq_poll_idle_chatlv = $GLOBALS['CFG']->chatlv_old_ping;
    ...
    public function poll_idle_chatlvs($now) {
        $this->trace('Polling chatlvs to detect disconnected users');
        if (!empty($this->chatlvrooms)) {
            foreach ($this->chatlvrooms as $chatlvid => $chatlvroom) {
                if (!empty($chatlvroom['users'])) {
                    foreach ($chatlvroom['users'] as $sessionid => $userid) {
                        // We will be polling each user as required.
                        $this->trace('...shall we poll '.$sessionid.'?');
                        if (!empty($this->sets_info[$sessionid]) && isset($this->sets_info[$sessionid]['chatlvuser']) &&
                                ...
                                ($this->sets_info[$sessionid]['chatlvuser']->lastmessageping < $this->_last_idle_poll)) {
                            $this->trace('YES!');
    ...
    public function user_lazy_update($sessionid) {
        ...
        if ($now - $this->sets_info[$sessionid]['lastinfocommit'] > $this->_freq_update_records) {
            ...
            $DB->update_record('chatlv_users', $this->sets_info[$sessionid]['chatlvuser']);
        }
        return true;
    }
    ...
    public function get_user_window($sessionid) {
        ...
        if (empty($str)) {
            $str = new stdClass();
            $str->idle  = get_string("idle", "chatlv");
            $str->beep  = get_string("beep", "chatlv");
        ...
        ob_start();
        $refreshinval = $CFG->chatlv_refresh_userlist * 1000;
        echo <<<EOD
        <html><head>
...
EOD;
        ...
        // Get the users from that chatlvroom.
        $users = $this->chatlvrooms[$info['chatlvid']]['users'];
        ...
        foreach ($users as $usersessionid => $userid) {
            ...
            $lastping = $timenow - $userinfo['chatlvuser']->lastmessageping;
            ...
            $action = new popup_action('click', $link, 'user'.$userinfo['chatlvuser']->id);
            ...
            echo '<a target="empty" href="http://'.$CFG->chatlv_serverhost.':'.$CFG->chatlv_serverport.
                 '/?win=beep&amp;beep='.$userinfo['user']->id.
                 '&chatlv_sid='.$sessionid.'">'.$str->beep."</a>\n";
            echo "</font></p>";
            echo "<td></tr>";
        }
    ...
    public function dispatch_sidekick($handle, $type, $sessionid, $customdata) {
        ...
        switch($type) {
            case CHAT_SIDEKICK_BEEP:
                ...
                // Incoming beep.
                $msg = new stdClass;
                $msg->chatlvid    = $this->sets_info[$sessionid]['chatlvid'];
                ...
                // Commit to DB.
                chatlv_send_chatlvmessage($this->sets_info[$sessionid]['chatlvuser'], $msg->message, false,
                ...
                $this->sets_info[$sessionid]['chatlvuser']->lastping        = $msg->timestamp;
                $this->sets_info[$sessionid]['chatlvuser']->lastmessageping = $msg->timestamp;
            ...
            case CHAT_SIDEKICK_USERS:
                ...
                $header .= "Refresh: ".(intval($CFG->chatlv_refresh_userlist) + 2).
                           "; url=http://$CFG->chatlv_serverhost:$CFG->chatlv_serverport/?win=users&".
                           "chatlv_sid=".$sessionid."\n";
                $header .= "\n";
                ...
                $this->sets_info[$sessionid]['chatlvuser']->lastping = time();
                $this->user_lazy_update($sessionid);
    ...
    public function promote_final($sessionid, $customdata) {
        ...
        $chatlvuser = $DB->get_record('chatlv_users', array('sid' => $sessionid));
        if ($chatlvuser === false) {
            $this->dismiss_half($sessionid);
            return false;
        }
        $chatlv = $DB->get_record('chatlv', array('id' => $chatlvuser->chatlvid));
        if ($chatlv === false) {
            $this->dismiss_half($sessionid);
            return false;
        }
        ...
        $course = $DB->get_record('course', array('id' => $chatlv->course));
        if ($course === false) {
            $this->dismiss_half($sessionid);
            return false;
        }
        ...
        $this->sets_info[$sessionid] = array(
            ...
            'course'    => $course,
            'courseid'  => $course->id,
            'chatlvuser'  => $chatlvuser,
            'chatlvid'    => $chatlv->id,
            'cm'        => $cm,
            'user'      => $user,
            'userid'    => $user->id,
            'groupid'   => $chatlvuser->groupid,
            'lang'      => $chatlvuser->lang,
            'quirks'    => $customdata['quirks']
        );

        // If we know nothing about this chatlvroom, initialize it and add the user.
        if (!isset($this->chatlvrooms[$chatlv->id]['users'])) {
            $this->chatlvrooms[$chatlv->id]['users'] = array($sessionid => $user->id);
        } else {
            // Otherwise just add the user.
            $this->chatlvrooms[$chatlv->id]['users'][$sessionid] = $user->id;
        }
        ...
        $this->trace('Connection accepted: '
                     ...
                     .$chatlvuser->userid
                     .' GID: '
                     .$chatlvuser->groupid, E_USER_WARNING);

        // Finally, broadcast the "entered the chatlv" message.

        $msg = new stdClass;
        $msg->chatlvid = $chatlvuser->chatlvid;
        $msg->userid = $chatlvuser->userid;
        $msg->groupid = $chatlvuser->groupid;
        $msg->system = 1;
        $msg->message = 'enter';
        $msg->timestamp = time();

        chatlv_send_chatlvmessage($chatlvuser, $msg->message, true);
        $this->message_broadcast($msg, $this->sets_info[$sessionid]['user']);
    ...
    public function message_broadcast($message, $sender) {
        ...
        // First of all, mark this chatlvroom as having had activity now.
        $this->chatlvrooms[$message->chatlvid]['lastactivity'] = $now;

        foreach ($this->sets_info as $sessionid => $info) {
            // We need to get handles from users that are in the same chatlvroom, same group.
            if ($info['chatlvid'] == $message->chatlvid &&
              ($info['groupid'] == $message->groupid || $message->groupid == 0)) {

                // Simply give them the message.
                $output = chatlv_format_message_manually($message, $info['courseid'], $sender, $info['user']);
                if ($output !== false) {
    ...
    public function disconnect_session($sessionid) {
        ...
        $DB->delete_records('chatlv_users', array('sid' => $sessionid));
        $msg = new stdClass;
        $msg->chatlvid = $info['chatlvid'];
        ...
        chatlv_send_chatlvmessage($info['chatlvuser'], $msg->message, true);
    ...
    public function init_sockets() {
        ...
        if (!socket_bind($this->listen_socket, $CFG->chatlv_serverip, $CFG->chatlv_serverport)) {
            // Failed to bind socket.
            $lasterr = socket_last_error();
            $this->fatal('socket_bind() failed: '. socket_strerror($lasterr).' ['.$lasterr.']');
        }

        if (!socket_listen($this->listen_socket, $CFG->chatlv_servermax)) {
        ...
        $this->trace('Socket opened on port '.$CFG->chatlv_serverport);
...
if (!$daemon->query_start()) {
    ...
    echo 'Starts the Moodle chatlv socket server on port '.$CFG->chatlv_serverport;
    echo "\n\n";
    echo "Usage: chatlvd.php [parameters]\n\n";
    echo "Parameters:\n";
    echo "  --start         Starts the daemon\n";
    echo "  -v              Verbose mode (prints trivial information messages)\n";
    echo "  -l [logfile]    Log all messages to logfile (if not specified, chatlvd.log)\n";
    echo "Example:\n";
    echo "  chatlvd.php --start -l\n\n";
    die();
}
...
$daemon->trace('Started Moodle chatlvd on port '.$CFG->chatlv_serverport.', listening socket '.$daemon->listen_socket, E_USER_WARNING);

// Clear the decks of old stuff.
$DB->delete_records('chatlv_users', array('version' => 'sockets'));
...
while (true) {
    ...
    if ($daemon->conn_activity_ufo($active)) {
        foreach ($active as $handle) {
            ...
            if ($changed > 0) {
                ...
                if (!preg_match('/win=(chatlv|users|message|beep).*&chatlv_sid=([a-zA-Z0-9]*) HTTP/', $data, $info)) {
                    // Malformed data.
                ...
                switch($type) {
                    ...
                    case 'message':
                        ...
                        if (!preg_match('/chatlv_message=([^&]*)[& ]chatlv_msgidnr=([^&]*)[& ]/', $data, $info)) {
    ...
    // Clean up chatlvrooms with no activity as required.
    if ($now - $daemon->_last_idle_poll >= $daemon->_freq_poll_idle_chatlv) {
        $daemon->poll_idle_chatlvs($now);
    }
