...
/// $CFG variables are now defined in database by chatlv/lib.php
...
class ChatDaemon {
    ...
    var $chatlvrooms = array();    // Keyed by chatlvid, holding arrays of data

    // IMPORTANT: $conn_sets, $sets_info and $chatlvrooms must remain synchronized!
    ...
    function poll_idle_chatlvs($now) {
            ...
            foreach($this->chatlvrooms as $chatlvid => $chatlvroom) {
                if(!empty($chatlvroom['users'])) {
                    foreach($chatlvroom['users'] as $sessionid => $userid) {
                        ...
                        if($this->sets_info[$sessionid]['chatlvuser']->lastmessageping < $this->_last_idle_poll) {
                            $this->trace('YES!');
...
//        return false;
    ...
    function get_user_window($sessionid) {
        ...
        ob_start();
        $refresh_inval = $CFG->chatlv_refresh_userlist * 1000;
        echo <<<EOD
        <html><head>
...
EOD;
        ...
        foreach ($users as $usersessionid => $userid) {
            ...
            echo '<a target="empty" href="http://'.$CFG->chatlv_serverhost.':'.$CFG->chatlv_serverport.'/?win=beep&amp;beep='.$userinfo['user']->id.
    ...
    function dispatch_sidekick($handle, $type, $sessionid, $customdata) {
        ...
        switch($type) {
            case CHAT_SIDEKICK_BEEP:
                ...
                // Commit to DB
                $DB->insert_record('chatlv_messages', $msg, false);
                $DB->insert_record('chatlv_messages_current', $msg, false);
            ...
            case CHAT_SIDEKICK_USERS:
                ...
                $header .= "Refresh: ".(intval($CFG->chatlv_refresh_userlist) + 2)."; url=http://$CFG->chatlv_serverhost:$CFG->chatlv_serverport/?win=users&".
    ...
    function promote_final($sessionid, $customdata) {
        ...
        $this->trace('Connection accepted: '.$this->conn_sets[$sessionid][CHAT_CONNECTION_CHANNEL].', SID: '.$sessionid.' UID: '.$chatlvuser->userid.' GID: '.$chatlvuser->groupid, E_USER_WARNING);
