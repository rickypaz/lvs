...
$chatlvlastid   = optional_param('chatlv_lastid',    0, PARAM_INT);

$url = new moodle_url('/mod/chatlv/gui_header_js/jsupdated.php', array('chatlv_sid' => $chatlvsid));
if ($chatlvlasttime !== 0) {
...
if ($chatlvlastid !== 1) {
    $url->param('chatlv_lastid', $chatlvlastid);
}
$PAGE->set_url($url);
...
$refreshurl = "{$CFG->wwwroot}/mod/chatlv/gui_header_js/jsupdated.php?".
              "chatlv_sid=$chatlvsid&chatlv_lasttime=$chatlvlasttime&chatlv_lastrow=$chatlvnewrow&chatlv_lastid=$chatlvlastid";
?>
...
for ($n = 0; $n <= CHAT_MAX_CLIENT_UPDATES; $n++) {
    ...
    $params = array('groupid' => $chatlvuser->groupid,
                    'lastid' => $chatlvlastid,
                    'lasttime' => $chatlvlasttime,
                    'chatlvid' => $chatlvuser->chatlvid);
    ...
    if ($chatlvlastid > 0) {
        ...
        if ($chatlvlasttime == 0) { // Display some previous messages.
            $chatlvlasttime = $timenow - $CFG->chatlv_old_ping; // TO DO - any better value?
        }
    ...
    if (($chatlvlasttime != $chatlvnewlasttime) and $messages) {
        ...
        $chatlvlasttime = $message->timestamp;
        $chatlvlastid   = $message->id;
    }
...
$refreshurl = "{$CFG->wwwroot}/mod/chatlv/gui_header_js/jsupdated.php?";
$refreshurl .= "chatlv_sid=$chatlvsid&chatlv_lasttime=$chatlvlasttime&chatlv_lastrow=$chatlvnewrow&chatlv_lastid=$chatlvlastid";
