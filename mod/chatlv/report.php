...
if ($start and $end and !$confirmdelete) {   // Show a full transcript.
    ...
    if (!$messages = $DB->get_records_select('chatlv_messages',
                                             "chatlvid = :chatlvid AND timestamp >= :start AND timestamp <= :end $groupselect",
                                             $params,
                                             "timestamp ASC")) {
...
echo $OUTPUT->heading(format_string($chatlv->name).': '.get_string('sessions', 'chatlv'), 2);
...
if (empty($messages)) {   // May have already got them above.
    if (!$messages = $DB->get_records_select('chatlv_messages', "chatlvid = :chatlvid $groupselect", $params, "timestamp DESC")) {
        echo $OUTPUT->heading(get_string('nomessages', 'chatlv'), 3);
        echo $OUTPUT->footer();
        exit;
    }
}
...
if ($showall) {
    $headingstr = get_string('listing_all_sessions', 'chatlv') . '&nbsp;';
    $headingstr .= html_writer::link("report.php?id={$cm->id}&show_all=0", get_string('list_complete_sessions', 'chatlv'));
...
if (!$showall and $completesessions == 0) {
    ...
    echo get_string('no_complete_sessions_found', 'chatlv') . '&nbsp;';
    echo html_writer::link('report.php?id='.$cm->id.'&show_all=1', get_string('list_all_sessions', 'chatlv'));
