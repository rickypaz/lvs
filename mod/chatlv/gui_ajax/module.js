    ...
    var gui_ajax = {
        ...
        init : function(cfg) {
            ...
            this.layout = new Y.YUI2.widget.Layout({
                units : [
                     {position: 'left', width: 300, resize: true, gutter: '5px', scroll: true, body: 'chatlv-grades', animate: false},
                     {position: 'right', width: 180, resize: true, gutter: '5px', scroll: true, body: 'chatlv-userlist', animate: false},
                     {position: 'bottom', height: 42, resize: false, body: 'chatlv-input-area', gutter: '5px', collapse: false, resize: false},
                     {position: 'center', body: 'chatlv-messages', gutter: '5px', scroll: true}
                ]
            });
            ...
            this.thememenu = new Y.Overlay({
                bodyContent : '<div class="menuitem"><a href="'+this.cfg.chatlvurl+'&theme=bubble">Bubble</a></div><div class="menuitem"><a href="'+this.cfg.chatlvurl+'&theme=compact">Compact</a></div>',
                visible : false,
                zIndex : 2,
                align : {
                    node : '#choosetheme',
            ...
            */
            ...
            this.thememenu.addItems([
                ...
                //{text: "Bubble", url: this.cfg.chatlvurl+'&theme=bubble'},
                {text: "Compact", url: this.cfg.chatlvurl+'&theme=compact'}
            ]);
            this.thememenu.render(document.body);
            Y.one('#choosetheme').on('click', function(e){
        ...
        send : function(e, beep) {
            this.sendbutton.set('value', M.str.chatlv.sending);
        ...
        send_callback : function(tid, outcome, args) {
            ...
            this.sendbutton.set('value', M.str.chatlv.send);
            this.messageinput.set('value', '');
        ...
        update_users : function(users) {
            ...
            for (var i in users) {
                ...
                if (users[i].id == this.cfg.userid) {
                    ...
                    var talk = Y.Node.create('<a href="###">'+M.str.chatlv.talk+'</a>');
                    talk.on('click', this.talkto, this, users[i].name);
                    var beep = Y.Node.create('<a href="###">'+M.str.chatlv.beep+'</a>');
