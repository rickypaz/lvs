...
if (isguestuser()) {
    $PAGE->set_title(format_string($chatlv->name));
    echo $OUTPUT->header();
...
add_to_log($course->id, 'chatlv', 'view', "view.php?id=$cm->id", $chatlv->id, $cm->id);
...
echo $OUTPUT->heading(format_string($chatlv->name));
...
if (has_capability('mod/chatlv:chatlv', $context)) {
    ...
    echo '<p>';
    echo $OUTPUT->action_link($chatlvtarget, $strenterchatlv, new popup_action('click', $chatlvtarget, "chatlv$course->id$chatlv->id$groupparam", array('height' => 500, 'width' => 700)));
    echo '</p>';

    $params['id'] = $chatlv->id;
    $link = new moodle_url('/mod/chatlv/gui_basic/index.php', $params);
    $action = new popup_action('click', $link, "chatlv{$course->id}{$chatlv->id}{$groupparam}", array('height' => 500, 'width' => 700));
    echo '<p>';
    echo $OUTPUT->action_link($link, get_string('noframesjs', 'message'), $action, array('title'=>get_string('modulename', 'chatlv')));
    echo '</p>';
    ...
    if ($chatlv->studentlogs or has_capability('mod/chatlv:readlog', $context)) {
        if ($msg = $DB->get_records_select('chatlv_messages', "chatlvid = ? $groupselect", array($chatlv->id))) {
            echo '<p>';
            echo html_writer::link(new moodle_url('/mod/chatlv/report.php', array('id'=>$cm->id)), get_string('viewreport', 'chatlv'));
            echo '</p>';
        }
    }
