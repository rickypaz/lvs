...
/**
 * Subscribe to or unsubscribe from a forumlv discussion.
 *
 * @package    mod_forumlv
...
$forumlvid        = required_param('forumlvid', PARAM_INT);             // The forumlv to subscribe or unsubscribe.
...
if (!$discussion = $DB->get_record('forumlv_discussions', array('id' => $discussionid, 'forumlv' => $forumlvid))) {
...
if (is_guest($context, $USER)) {
    ...
    throw new moodle_exception('noguestsubscribe', 'mod_forumlv');
}
...
if (\mod_forumlv\subscriptions::is_subscribed($USER->id, $forumlv, $discussion->id, $cm)) {
    // The user is subscribed, unsubscribe them.
    \mod_forumlv\subscriptions::unsubscribe_user_from_discussion($USER->id, $discussion, $context);
} else {
...
$return->icon = forumlv_get_discussion_subscription_icon($forumlv, $discussion->id, null, $includetext);
echo json_encode($return);
die;
