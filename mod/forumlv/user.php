...
$url = new moodle_url('/mod/forumlv/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_forumlv');
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'forumlv');
        } else {
        ...
        if ($discussionsonly) {
            ...
            $notification = get_string('nopostsmadebyuser', 'forumlv', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'forumlv');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('forumlv_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular forumlv,
...
foreach ($result->posts as $post) {
    if (!isset($result->forumlvs[$post->forumlv]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $forumlv = $result->forumlvs[$post->forumlv];
    $cm = $forumlv->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $forumlvurl = new moodle_url('/mod/forumlv/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/forumlv/discuss.php', array('d' => $post->discussion));
    ...
    if ($forumlv->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($forumlv->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $forumlvurl;
        } else {
    ...
    $forumlvname = format_string($forumlv->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($forumlvurl, $forumlvname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $forumlvname);
    }
    if ($forumlv->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/forumlv/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "forumlv"));

    $postoutput[] = forumlv_print_post($post, $discussion, $forumlv, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_forumlv', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_forumlv', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        ...
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_forumlv', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'forumlv', $userfullname));
} else {
