...
/**
 * Set the mail digest option in a specific forumlv for a user.
 *
...
$forumlv = $DB->get_record('forumlv', array('id' => $id));
...
$url = new moodle_url('/mod/forumlv/maildigest.php', array(
    'id' => $id,
...
forumlv_set_user_maildigest($forumlv, $maildigest);
...
if ($maildigest === -1) {
    ...
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_forumlv', $info);
    $updatemessage = get_string('emaildigestupdated_default', 'forumlv', $info);
} else {
    $info->maildigesttitle = $digestoptions[$info->maildigest];
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_forumlv', $info);
    $updatemessage = get_string('emaildigestupdated', 'forumlv', $info);
}
