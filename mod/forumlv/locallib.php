...
/**
 * Library of functions for forumlv outside of the core api
 */
 ...
 */
class forumlv_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'forumlv');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('forumlv_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'forumlv');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('forumlv_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'forumlv');
        }

        if (!$this->forumlv = $DB->get_record('forumlv', array('id' => $this->discussion->forumlv))) {
            throw new portfolio_caller_exception('invalidforumlvid', 'forumlv');
        }

        if (!$this->cm = get_coursemodule_from_instance('forumlv', $this->forumlv->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumlv', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumlv', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = forumlv_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumlv', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumlv', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/forumlv/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/forumlv/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('forumlvdiscussion' . $this->discussionid,
                    get_string('discussion', 'forumlv') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single forumlv post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('forumlvpost' . $post->id,  $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'forumlvpost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in forumlv_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'forumlv') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/forumlv:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/forumlv:exportownpost', $context)));
        }
        return has_capability('mod/forumlv:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'forumlv');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class forumlv_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_forumlv', $this->filearea, $record->itemid))
