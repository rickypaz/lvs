...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($forumlvid)) {
        $search .= ' forumlvid:'.$forumlvid;
    }
    if (!empty($user)) {
        $search .= ' '.forumlv_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.forumlv_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.forumlv_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.forumlv_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = forumlv_clean_search_terms($search);
}
...
$strsearchresults = get_string("searchresults", "forumlv");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strforumlvs, new moodle_url('/mod/forumlv/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'forumlv'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    forumlv_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchterms = str_replace('forumlvid:', 'instance:', $search);
$searchterms = explode(' ', $searchterms);

$searchform = forumlv_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/forumlv/search.php', array('id'=>$course->id)));
$PAGE->navbar->add($strsearchresults);
if (!$posts = forumlv_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
...
foreach ($posts as $post) {
    ...
    // Replace the simple subject with the three items forumlv name -> thread name -> subject
    ...
    $fullsubject = "<a href=\"view.php?f=$forumlv->id\">".format_string($forumlv->name,true)."</a>";
    ...
    //Unfortunately seem to have do this individually as posts may be from different forumlvs
    ...
    // the start of the message text. However, do not do the highlighting here. forumlv_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','forumlv');
...
function forumlv_print_big_search_form($course) {
    ...
    echo $OUTPUT->box(get_string('searchforumlvintro', 'forumlv'), 'searchbox boxaligncenter', 'intro');

    echo $OUTPUT->box_start('generalbox boxaligncenter');

    echo html_writer::script('', $CFG->wwwroot.'/mod/forumlv/forumlv.js');

    echo '<form id="searchform" action="search.php" method="get">';
    echo '<table cellpadding="10" class="searchbox" id="form">';

    echo '<tr>';
    echo '<td class="c0"><label for="words">'.get_string('searchwords', 'forumlv').'</label>';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="phrase">'.get_string('searchphrase', 'forumlv').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="phrase" id="phrase" value="'.s($phrase, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="notwords">'.get_string('searchnotwords', 'forumlv').'</label></td>';
    ...
    if ($DB->get_dbfamily() == 'mysql' || $DB->get_dbfamily() == 'postgres') {
        echo '<tr>';
        echo '<td class="c0"><label for="fullwords">'.get_string('searchfullwords', 'forumlv').'</label></td>';
        echo '<td class="c1"><input type="text" size="35" name="fullwords" id="fullwords" value="'.s($fullwords, true).'" alt="" /></td>';
        echo '</tr>';
    }

    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdatefrom', 'forumlv').'</td>';
    echo '<td class="c1">';
    if (empty($datefrom)) {
        $datefromchecked = '';
    ...
    echo '<input name="timefromrestrict" type="checkbox" value="1" alt="'.get_string('searchdatefrom', 'forumlv').'" onclick="return lockoptions(\'searchform\', \'timefromrestrict\', timefromitems)" '.  $datefromchecked . ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdateto', 'forumlv').'</td>';
    echo '<td class="c1">';
    if (empty($dateto)) {
        $datetochecked = '';
        $dateto = time()+3600;
    ...
    echo '<input name="timetorestrict" type="checkbox" value="1" alt="'.get_string('searchdateto', 'forumlv').'" onclick="return lockoptions(\'searchform\', \'timetorestrict\', timetoitems)" ' .$datetochecked. ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="menuforumlvid">'.get_string('searchwhichforumlvs', 'forumlv').'</label></td>';
    echo '<td class="c1">';
    echo html_writer::select(forumlv_menu_list($course), 'forumlvid', '', array(''=>get_string('allforumlvs', 'forumlv')));
    echo '</td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="subject">'.get_string('searchsubject', 'forumlv').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="subject" id="subject" value="'.s($subject, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="user">'.get_string('searchuser', 'forumlv').'</label></td>';
    ...
    echo '<input type="submit" value="'.get_string('searchforumlvs', 'forumlv').'" alt="" /></td>';
    echo '</tr>';
...
function forumlv_menu_list($course)  {
    ...
    foreach ($modinfo->instances['forumlv'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
