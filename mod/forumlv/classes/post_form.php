...
/**
 * File containing the form definition to post in the forumlv.
 *
 * @package   mod_forumlv
...
class mod_forumlv_post_form extends moodleform {
    ...
    public static function editor_options(context_module $context, $postid) {
        ...
        return array(
            ...
            'subdirs' => file_area_contains_subdirs($context, 'mod_forumlv', 'post', $postid)
        );
    }
    ...
    function definition() {
        ...
        $mform->addElement('editor', 'message', get_string('message', 'forumlv'), null, self::editor_options($modcontext, (empty($post->id) ? null : $post->id)));
        ...
        if (\mod_forumlv\subscriptions::is_forcesubscribed($forumlv)) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'forumlv'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'forcesubscribed', 'forumlv');

        } else if (\mod_forumlv\subscriptions::subscription_disabled($forumlv) && !$manageactivities) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'forumlv'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'disallowsubscription', 'forumlv');

        } else {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'forumlv'));
            $mform->addHelpButton('discussionsubscribe', 'discussionsubscription', 'forumlv');
        }
        ...
        if (!$post->parent && has_capability('mod/forumlv:pindiscussions', $modcontext)) {
            $mform->addElement('checkbox', 'pinned', get_string('discussionpinned', 'forumlv'));
            $mform->addHelpButton('pinned', 'discussionpinned', 'forumlv');
        }
        ...
        if (!empty($CFG->forumlv_enabletimedposts) && !$post->parent && has_capability('mod/forumlv:viewhiddentimedposts', $coursecontext)) { // hack alert
            ...
            $mform->addElement('date_time_selector', 'timestart', get_string('displaystart', 'forumlv'), array('optional' => true));
            $mform->addHelpButton('timestart', 'displaystart', 'forumlv');

            $mform->addElement('date_time_selector', 'timeend', get_string('displayend', 'forumlv'), array('optional' => true));
        ...
        if ($groupmode = groups_get_activity_groupmode($cm, $course)) {
            ...
            foreach ($groupdata as $groupid => $group) {
                ...
                if (forumlv_user_can_post_discussion($forumlv, $groupid, null, $cm, $modcontext)) {
            ...
            $canposttoowngroups = $canposttoowngroups && has_capability('mod/forumlv:canposttomygroups', $modcontext);
            if ($canposttoowngroups) {
                ...
                // user is a member of.
                $mform->addElement('checkbox', 'posttomygroups', get_string('posttomygroups', 'forumlv'));
                $mform->addHelpButton('posttomygroups', 'posttomygroups', 'forumlv');
            ...
            if (forumlv_user_can_post_discussion($forumlv, -1, null, $cm, $modcontext)) {
            ...
            // We allow this because the post may have been moved from another forumlv where groups are not available.
            // We show this even if no groups are available as groups *may* have been available but now are not.
            $canselectgroupformove = $groupcount && !empty($post->edit) && has_capability('mod/forumlv:movediscussions', $modcontext);
