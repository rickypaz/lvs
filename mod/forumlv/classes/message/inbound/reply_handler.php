...
/**
 * A Handler to process replies to forumlv posts.
 *
 * @package    mod_forumlv
...
namespace mod_forumlv\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_forumlv');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_forumlv');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('forumlv_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $cm = get_fast_modinfo($course->id)->instances['forumlv'][$forumlv->id];
        ...
        if (!$canpost) {
            $data = new \stdClass();
            $data->forumlv = $forumlv;
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostforumlv', 'mod_forumlv', $data);
        }
        ...
        if (!\core_availability\info_module::is_user_visible($cm)) {
            $data = new \stdClass();
            $data->forumlv = $forumlv;
            throw new \core\message\inbound\processing_failed_exception('messageinboundforumlvhidden', 'mod_forumlv', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'mod_forumlv', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'forumlv');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($forumlv->maxattachments) || $forumlv->maxbytes == 1 ||
                    !has_capability('mod/forumlv:createattachment', $modcontext)) {
                // Attachments are not allowed.
                mtrace("--> User does not have permission to attach files in this forumlv. Rejecting e-mail.");

                $data = new \stdClass();
                $data->forumlv = $forumlv;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'mod_forumlv', $data);
            }

            if ($forumlv->maxattachments < $attachmentcount) {
                // Too many attachments.
                mtrace("--> User attached {$attachmentcount} files when only {$forumlv->maxattachments} where allowed. "
                     . " Rejecting e-mail.");

                $data = new \stdClass();
                $data->forumlv = $forumlv;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'mod_forumlv', $data);
            }
            ...
            if ($forumlv->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$forumlv->maxbytes} where allowed. "
                     . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->forumlv = $forumlv;
                $data->maxbytes = display_size($forumlv->maxbytes);
                $data->filesize = display_size($filesize);
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'mod_forumlv', $data);
            }
        }
        ...
        $addpost->id = forumlv_add_new_post($addpost, true);
        ...
        $event->add_record_snapshot('forumlv_posts', $addpost);
        $event->add_record_snapshot('forumlv_discussions', $discussion);
        $event->trigger();

        // Update completion state.
        $completion = new \completion_info($course);
        if ($completion->is_enabled($cm) && ($forumlv->completionreplies || $forumlv->completionposts)) {
            $completion->update_state($cm, COMPLETION_COMPLETE);

            mtrace("--> Updating completion status for user {$USER->id} in forumlv {$forumlv->id} for post {$addpost->id}.");
        }
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/forumlv/discuss.php', array('d' => $handlerresult->discussion));
        $discussionurl->set_anchor('p' . $handlerresult->id);
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_forumlv', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_forumlv', $a);
        return $message;
    }
}
