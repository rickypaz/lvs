...
require_once($CFG->dirroot . '/mod/forumlv/tests/generator/lib.php');
require_once($CFG->dirroot . '/mod/forumlv/lib.php');

/**
 * Provides the unit tests for forumlv search.
 *
 * @package     mod_forumlv
 * @category    test
 * @copyright   2015 David Monllao {@link http://www.davidmonllao.com}
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_forumlv_search_testcase extends advanced_testcase {

    /**
     * @var string Area id
     */
    protected $forumlvpostareaid = null;

    public function setUp() {
        $this->resetAfterTest(true);
        set_config('enableglobalsearch', true);

        $this->forumlvpostareaid = \core_search\manager::generate_areaid('mod_forumlv', 'post');
    ...
    public function test_search_enabled() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->forumlvpostareaid);
    ...
    public function test_posts_indexing() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->forumlvpostareaid);
        $this->assertInstanceOf('\mod_forumlv\search\post', $searcharea);
    ...
    public function test_posts_document() {
        ...
        set_coursemodule_visible($forumlv2->cmid, 0);
        ...
        $post1 = $DB->get_record('forumlv_posts', array('id' => $discussion1reply1->id));
        $post1->forumlvid = $forumlv1->id;
        $post1->courseid = $forumlv1->course;

        $doc = $searcharea->get_document($post1);
        $this->assertInstanceOf('\core_search\document', $doc);
        $this->assertEquals($discussion1reply1->id, $doc->get('itemid'));
        $this->assertEquals($this->forumlvpostareaid . '-' . $discussion1reply1->id, $doc->get('id'));
    ...
    public function test_attach_files() {
        ...
        $post = $DB->get_record('forumlv_posts', array('discussion' => $discussion1->id));
        $filerecord = array(
            'contextid' => context_module::instance($forumlv1->cmid)->id,
