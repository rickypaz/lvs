...
/**
 * The forumlv module mail generation tests.
 *
 * @package    mod_forumlv
 * @category   external
...
class mod_forumlv_mail_testcase extends advanced_testcase {
    ...
    /**
     * Perform message inbound setup for the mod_forumlv reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_forumlv\message\inbound\reply_handler'));
        $record->enabled = true;
     ...
     */
    protected function helper_post_to_forumlv($forumlv, $author, $fields = array()) {
        global $DB;
    ...
    protected function helper_update_post_time($post, $factor) {
        ...
        $DB->set_field('forumlv_posts', 'created', $post->created + $factor, array('id' => $post->id));
    }
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        ...
        $sub = $DB->get_record('forumlv_discussion_subs', array('userid' => $user->id, 'discussion' => $discussion->id));

        // Update the subscription to have a preference in the past.
        $DB->set_field('forumlv_discussion_subs', 'preference', $sub->preference + $factor, array('id' => $sub->id));
    }
     ...
     * @param stdClass $author The author to post as
     * @return stdClass The forumlv post
     */
    protected function helper_post_to_discussion($forumlv, $discussion, $author) {
        global $DB;
    ...
    /**
     * Run the forumlv cron, and check that the specified post was sent the
     * specified number of times.
     *
     * @param stdClass $post The forumlv post object
    ...
    /**
     * Run the forumlv cron, and check that the specified posts were sent the
     * specified number of times.
     *
     * @param stdClass $post The forumlv post object
     * @param integer $expected The number of times that the post should have been sent
    ...
    public function test_subscription_disabled() {
        ...
        \mod_forumlv\subscriptions::subscribe_user($recipient->id, $forumlv);

        $this->assertEquals($expected, $DB->count_records('forumlv_subscriptions', array(
            'userid'        => $recipient->id,
            'forumlv'         => $forumlv->id,
        )));

        // Run cron and check that the expected number of users received the notification.
        list($discussion, $post) = $this->helper_post_to_forumlv($forumlv, $recipient);
        $messages = $this->helper_run_cron_check_count($post, $expected);

        // Unsubscribe the user again.
        \mod_forumlv\subscriptions::unsubscribe_user($recipient->id, $forumlv);
        ...
        \mod_forumlv\subscriptions::subscribe_user_to_discussion($recipient->id, $discussion);
        $this->helper_update_subscription_time($recipient, $discussion, -60);

        $reply = $this->helper_post_to_discussion($forumlv, $discussion, $author);
    ...
    public function test_automatic_with_unsubscribed_user() {
        ...
        // Unsubscribe the 'author' user from the forumlv.
    ...
    public function test_optional_with_subscribed_user() {
        ...
        // Subscribe the 'recipient' user from the forumlv.
        \mod_forumlv\subscriptions::subscribe_user($recipient->id, $forumlv);
    ...
    public function test_automatic_with_unsubscribed_discussion() {
        ...
        $this->assertTrue(\mod_forumlv\subscriptions::is_subscribed($recipient->id, $forumlv, $discussion->id));
    ...
    public function test_automatic_with_subscribed_discussion_in_unsubscribed_forumlv() {
        $this->resetAfterTest(true);
        ...
        // We expect just the user subscribed to the forumlv to receive this post at the moment as the discussion
    ...
    public function test_optional_with_unsubscribed_discussion_in_subscribed_forumlv() {
        $this->resetAfterTest(true);
        ...
        \mod_forumlv\subscriptions::unsubscribe_user_from_discussion($recipient->id, $discussion);
    ...
    /**
     * Test that a user unsubscribed from a forumlv who has subscribed to a discussion, only receives posts made after
     * they subscribed to the discussion.
     */
    public function test_forumlv_discussion_subscription_forumlv_unsubscribed_discussion_subscribed_after_post() {
        $this->resetAfterTest(true);
    ...
    public function test_forumlv_message_inbound_multiple_posts() {
        $this->resetAfterTest(true);
        ...
        // Ensure that messageinbound is enabled and configured for the forumlv handler.
        $this->helper_spoof_message_inbound_setup();

        $author->emailstop = '0';
        set_user_preference('message_provider_mod_forumlv_posts_loggedoff', 'email', $author);
        set_user_preference('message_provider_mod_forumlv_posts_loggedin', 'email', $author);
    ...
    public function test_long_subject() {
        ...
        // Post a discussion to the forumlv.
        $subject = 'This is the very long forumlv post subject that somebody was very kind of leaving, it is intended to check if long subject comes in mail correctly. Thank you.';
        $a = (object)array('courseshortname' => $course->shortname, 'forumlvname' => $forumlv->name, 'subject' => $subject);
        $expectedsubject = get_string('postmailsubject', 'forumlv', $a);
        list($discussion, $post) = $this->helper_post_to_forumlv($forumlv, $author, array('name' => $subject));
    ...
    public function test_subjects() {
        ...
        $reply = $this->helper_post_to_discussion($forumlv, $discussion, $commenter);
        $messages = $this->helper_run_cron_check_count($reply, 2);
        $this->assertContains($strre, $messages[0]->subject);
    }

    /**
     * dataProvider for test_forumlv_post_email_templates().
     */
    public function forumlv_post_email_templates_provider() {
                ...
                array(
                    ...
                    'contents' => array(
                        ...
                        'Attachment example.txt:\n' .
                            'http://www.example.com/moodle/pluginfile.php/\d*/mod_forumlv/attachment/\d*/example.txt\n',
                        'Hello Moodle', 'Moodle Forumlv', 'Welcome.*Moodle', 'Love Moodle', '1\d1'
                    ),
                ),
            ),
        );
        ...
        $newcase['forumlvs'][0]['name'] = 'Moodle Forumlv\'"';
        $newcase['forumlvs'][0]['forumlvposts'][0]['name'] = 'Hello Moodle\'"';
        $newcase['forumlvs'][0]['forumlvposts'][0]['message'] = 'Welcome to Moodle\'"';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;(quot|\#039);', 'Love Moodle\'', '101\'', 'Moodle Forumlv\'"',
        ...
        $newcase['forumlvs'][0]['forumlvposts'][0]['message'] = 'Welcome to Moodle>';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;gt;', 'Love Moodle>', '101>', 'Moodle Forumlv>',
            'Hello Moodle>', 'Welcome to Moodle>');
        ...
        $newcase = $base;
        $newcase['forumlvs'][0]['forumlvposts'][0]['name'] = 'Text and image';
        $newcase['forumlvs'][0]['forumlvposts'][0]['message'] = 'Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            'Attachment example.txt:\n' .
            'http://www.example.com/moodle/pluginfile.php/\d*/mod_forumlv/attachment/\d*/example.txt\n',
            'Text and image', 'Moodle Forumlv',
            'Welcome to Moodle, *\n.*'
                .'http://www.example.com/moodle/pluginfile.php/\d+/mod_forumlv/post/\d+/'
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&amp;(amp|lt|gt|quot|\#039);',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*Hello Moodle\'"&gt;&amp;', '>Moodle Forumlv\'"&gt;&amp;',
            '>Welcome.*Moodle\'"&gt;&amp;', '>Love Moodle&\#039;&quot;&gt;&amp;', '>101\'"&gt;&amp');
        $htmlcases['HTML mail with quotes, gt, lt and ampersand  everywhere'] = array('data' => $newcase);

        // Text+image message i.e. @@PLUGINFILE@@ token handling.
        $newcase = $htmlbase;
        $newcase['forumlvs'][0]['forumlvposts'][0]['name'] = 'HTML text and image';
        $newcase['forumlvs'][0]['forumlvposts'][0]['message'] = '<p>Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&(amp|lt|gt|quot|\#039);(?!course)',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*HTML text and image', '>Moodle Forumlv',
            '<p>Welcome to Moodle, '
                .'<img src="http://www.example.com/moodle/pluginfile.php/\d+/mod_forumlv/post/\d+/'
                .'Screen%20Shot%202016-03-22%20at%205\.54\.36%20AM%20%281%29\.png"'
                .' alt="" width="200" height="393" class="img-responsive" />!</p>',
            '>Love Moodle', '>1\d1');
    ...
    /**
     * Verify forumlv emails body using templates to generate the expected results.
     *
     * @dataProvider forumlv_post_email_templates_provider
     * @param array $data provider samples.
     */
    public function test_forumlv_post_email_templates($data) {
        global $DB;
        ...
        // Create forumlvs, always force susbscribed (for easy), with the specified options.
        $posts = array();
        foreach ($data['forumlvs'] as $dataforumlv) {
            $forumlvposts = isset($dataforumlv['forumlvposts']) ? $dataforumlv['forumlvposts'] : array();
            unset($dataforumlv['forumlvposts']);
            $options = array('course' => $course->id, 'forcesubscribe' => FORUMLV_FORCESUBSCRIBE);
            foreach ($dataforumlv as $option => $value) {
                $options[$option] = $value;
            }
            $forumlv = $this->getDataGenerator()->create_module('forumlv', $options);

            // Create posts, always for immediate delivery (for easy), with the specified options.
            foreach ($forumlvposts as $forumlvpost) {
                $attachments = isset($forumlvpost['attachments']) ? $forumlvpost['attachments'] : array();
                unset($forumlvpost['attachments']);
                $postoptions = array('course' => $course->id, 'forumlv' => $forumlv->id, 'userid' => $user->id,
                    'mailnow' => 1, 'attachment' => !empty($attachments));
                foreach ($forumlvpost as $option => $value) {
                    $postoptions[$option] = $value;
                }
                list($discussion, $post) = $this->helper_post_to_forumlv($forumlv, $user, $postoptions);
                ...
                if ($attachments) {
                    ...
                    foreach ($attachments as $attachment) {
                        $filerecord = array(
                            'contextid' => context_module::instance($forumlv->cmid)->id,
                    ...
                    $DB->set_field('forumlv_posts', 'attachment', '1', array('id' => $post->id));
                }
            }
        }
        ...
        forumlv_cron(); // It's really annoying that we have to run cron to test this.
