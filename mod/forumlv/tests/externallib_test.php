...
class mod_forumlv_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_mod_forumlv_get_forumlvs_by_courses() {
        ...
        $cm2 = get_coursemodule_from_id('forumlv', $forumlv2->id, 0, false, MUST_EXIST);
        ...
        external_api::clean_returnvalue(mod_forumlv_external::get_forumlvs_by_courses_returns(), $forumlvs);
        $this->assertEquals($expectedforumlvs, $forumlvs);
        ...
        try {
            mod_forumlv_external::get_forumlvs_by_courses(array($course2->id));
        ...
        $this->unassignUserCapability('mod/forumlv:viewdiscussion', null, null, $course1->id);
        try {
            $forumlvs = mod_forumlv_external::get_forumlvs_by_courses(array($course1->id));
    ...
    /**
     * Test get forumlv discussions
     */
    public function test_mod_forumlv_get_forumlv_discussions() {
        global $USER, $CFG, $DB;
        ...
        // Second forumlv of type 'qanda' with tracking enabled.
        $record = new stdClass();
        $record->course = $course2->id;
        $record->type = 'qanda';
        $record->trackingtype = FORUMLV_TRACKING_ON;
        $forumlv2 = self::getDataGenerator()->create_module('forumlv', $record);

        // Third forumlv where we will only have one discussion with no replies.
        $record = new stdClass();
        $record->course = $course2->id;
        $forumlv3 = self::getDataGenerator()->create_module('forumlv', $record);
        ...
        $record->userid = $user1->id;
        $discussion2reply1 = self::getDataGenerator()->get_plugin_generator('mod_forumlv')->create_post($record);

        $record->parent = $discussion2reply1->id;
        $record->userid = $user3->id;
        $discussion2reply2 = self::getDataGenerator()->get_plugin_generator('mod_forumlv')->create_post($record);

        // Check the forumlvs were correctly created.
        $this->assertEquals(3, $DB->count_records_select('forumlv', 'id = :forumlv1 OR id = :forumlv2 OR id = :forumlv3',
                array('forumlv1' => $forumlv1->id, 'forumlv2' => $forumlv2->id, 'forumlv3' => $forumlv3->id)));

        // Check the discussions were correctly created.
        $this->assertEquals(3, $DB->count_records_select('forumlv_discussions', 'forumlv = :forumlv1 OR forumlv = :forumlv2
                OR id = :forumlv3', array('forumlv1' => $forumlv1->id, 'forumlv2' => $forumlv2->id, 'forumlv3' => $forumlv3->id)));

        // Check the posts were correctly created, don't forget each discussion created also creates a post.
        $this->assertEquals(7, $DB->count_records_select('forumlv_posts', 'discussion = :discussion1 OR discussion = :discussion2',
        ...
        // Assign capabilities to view discussions for forumlv 2.
        $cm = get_coursemodule_from_id('forumlv', $forumlv2->id, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('mod/forumlv:viewdiscussion', $context->id, $newrole);

        // Assign capabilities to view discussions for forumlv 3.
        $cm = get_coursemodule_from_id('forumlv', $forumlv3->id, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $this->assignUserCapability('mod/forumlv:viewdiscussion', $context->id, $newrole);
        ...
        // Call the external function passing forumlv ids.
        $discussions = mod_forumlv_external::get_forumlv_discussions(array($forumlv1->id, $forumlv2->id, $forumlv3->id));
        external_api::clean_returnvalue(mod_forumlv_external::get_forumlv_discussions_returns(), $discussions);
        $this->assertEquals($expecteddiscussions, $discussions);

        // Remove the users post from the qanda forumlv and ensure they can not return the discussion.
        $DB->delete_records('forumlv_posts', array('id' => $discussion2reply1->id));
        try {
            mod_forumlv_external::get_forumlv_discussions(array($forumlv2->id));
            $this->fail('Exception expected due to attempting to access qanda forumlv without posting.');
        } catch (moodle_exception $e) {
            $this->assertEquals('nopermissions', $e->errorcode);
        }
        ...
        $this->unassignUserCapability('mod/forumlv:viewdiscussion', null, null, $course1->id);
        try {
            mod_forumlv_external::get_forumlv_discussions(array($forumlv1->id));
            $this->fail('Exception expected due to missing capability.');
        } catch (moodle_exception $e) {
            $this->assertEquals('nopermissions', $e->errorcode);
        }
