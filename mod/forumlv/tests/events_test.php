 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_forumlv_events_testcase extends advanced_testcase {
    ...
    public function test_course_searched_searchterm_validation() {
        ...
        \mod_forumlv\event\course_searched::create($params);
    }
    ...
    public function test_course_searched() {
        ...
        $this->assertInstanceOf('\mod_forumlv\event\course_searched', $event);
        $this->assertEquals($coursectx, $event->get_context());
        $expected = array($course->id, 'forumlv', 'search', "search.php?id={$course->id}&amp;search={$searchterm}", $searchterm);
    ...
    /**
     * Ensure discussion_created event validates that forumlvid is set.
     */
    public function test_discussion_created_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $context = context_module::instance($forumlv->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\discussion_created::create($params);
    }
    ...
    public function test_discussion_created() {
        ...
        $discussion = $this->getDataGenerator()->get_plugin_generator('mod_forumlv')->create_discussion($record);
        ...
        $this->assertInstanceOf('\mod_forumlv\event\discussion_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'add discussion', "discuss.php?d={$discussion->id}", $discussion->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_updated event validates that forumlvid is set.
     */
    public function test_discussion_updated_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $context = context_module::instance($forumlv->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\discussion_updated::create($params);
    }
    ...
    public function test_discussion_updated() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\discussion_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_deleted event validates that forumlvid is set.
     */
    public function test_discussion_deleted_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $context = context_module::instance($forumlv->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\discussion_deleted::create($params);
    }
    ...
    public function test_discussion_deleted() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\discussion_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'delete discussion', "view.php?id={$forumlv->cmid}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_moved event validates that fromforumlvid is set.
     */
    public function test_discussion_moved_fromforumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $toforumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));

        $context = context_module::instance($toforumlv->cmid);

        $params = array(
            'context' => $context,
            'other' => array('toforumlvid' => $toforumlv->id)
        );

        $this->setExpectedException('coding_exception', 'The \'fromforumlvid\' value must be set in other.');
        \mod_forumlv\event\discussion_moved::create($params);
    }

    /**
     * Ensure discussion_moved event validates that toforumlvid is set.
     */
    public function test_discussion_moved_toforumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $fromforumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $toforumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $context = context_module::instance($toforumlv->cmid);

        $params = array(
            'context' => $context,
            'other' => array('fromforumlvid' => $fromforumlv->id)
        );

        $this->setExpectedException('coding_exception', 'The \'toforumlvid\' value must be set in other.');
        \mod_forumlv\event\discussion_moved::create($params);
    }
    ...
    public function test_discussion_moved_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $discussion->id,
            'other' => array('fromforumlvid' => $fromforumlv->id, 'toforumlvid' => $toforumlv->id)
        );
    ...
    public function test_discussion_moved() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\discussion_moved', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'move discussion', "discuss.php?d={$discussion->id}",
            $discussion->id, $toforumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_forumlv\event\discussion_viewed::create($params);
    }
    ...
    public function test_course_module_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_forumlv\event\course_module_viewed::create($params);
    }
    ...
    public function test_course_module_viewed() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'view forumlv', "view.php?f={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    ...
    /**
     * Ensure subscription_created event validates that the forumlvid is set.
     */
    public function test_subscription_created_forumlvid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\subscription_created::create($params);
    }
    ...
    public function test_subscription_created() {
        ...
        $record['forumlv'] = $forumlv->id;
        $record['userid'] = $user->id;
        $subscription = $this->getDataGenerator()->get_plugin_generator('mod_forumlv')->create_subscription($record);
        ...
        $event = \mod_forumlv\event\subscription_created::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\subscription_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'subscribe', "view.php?f={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/subscribers.php', array('id' => $forumlv->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure subscription_deleted event validates that the forumlvid is set.
     */
    public function test_subscription_deleted_forumlvid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\subscription_deleted::create($params);
    }
    ...
    public function test_subscription_deleted() {
        ...
        $event = \mod_forumlv\event\subscription_deleted::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\subscription_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'unsubscribe', "view.php?f={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/subscribers.php', array('id' => $forumlv->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure readtracking_enabled event validates that the forumlvid is set.
     */
    public function test_readtracking_enabled_forumlvid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\readtracking_enabled::create($params);
    }
    ...
    public function test_readtracking_enabled() {
        ...
        $event = \mod_forumlv\event\readtracking_enabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\readtracking_enabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'start tracking', "view.php?f={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/view.php', array('f' => $forumlv->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure readtracking_disabled event validates that the forumlvid is set.
     */
    public function test_readtracking_disabled_forumlvid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\readtracking_disabled::create($params);
    }
    ...
    public function test_readtracking_disabled() {
        ...
        $event = \mod_forumlv\event\readtracking_disabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\readtracking_disabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'stop tracking', "view.php?f={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/view.php', array('f' => $forumlv->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure subscribers_viewed event validates that the forumlvid is set.
     */
    public function test_subscribers_viewed_forumlvid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'forumlvid\' value must be set in other.');
        \mod_forumlv\event\subscribers_viewed::create($params);
    }
    ...
    public function test_subscribers_viewed() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\subscribers_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'view subscribers', "subscribers.php?id={$forumlv->id}", $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_report_viewed_reportmode_validation() {
        ...
        \mod_forumlv\event\user_report_viewed::create($params);
    }
    ...
    public function test_user_report_viewed() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\user_report_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'user report',
    ...
    public function test_post_created_postid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($forumlv->cmid),
            'other' => array('forumlvid' => $forumlv->id, 'forumlvtype' => $forumlv->type, 'discussionid' => $discussion->id)
        );

        \mod_forumlv\event\post_created::create($params);
    }
    ...
    public function test_post_created_discussionid_validation() {
        ...
        $record['userid'] = $user->id;
        $post = $this->getDataGenerator()->get_plugin_generator('mod_forumlv')->create_post($record);

        $params = array(
            'context' => context_module::instance($forumlv->cmid),
            'objectid' => $post->id,
            'other' => array('forumlvid' => $forumlv->id, 'forumlvtype' => $forumlv->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the forumlvid is set.
     */
    public function test_post_created_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($forumlv->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'forumlvtype' => $forumlv->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the forumlvtype is set.
     */
    public function test_post_created_forumlvtype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($forumlv->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'forumlvid' => $forumlv->id)
        );

        $this->setExpectedException('coding_exception', 'The \'forumlvtype\' value must be set in other.');
        \mod_forumlv\event\post_created::create($params);
    }
    ...
    public function test_post_created_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'forumlvid' => $forumlv->id, 'forumlvtype' => $forumlv->type)
        );
    ...
    public function test_post_created() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'add post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/discuss.php', array('d' => $discussion->id));
    ...
    /**
     * Test the post_created event for a single discussion forumlv.
     */
    public function test_post_created_single() {
        // Setup test data.
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id, 'type' => 'single'));
        $user = $this->getDataGenerator()->create_user();
        ...
        $this->assertInstanceOf('\mod_forumlv\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'add post', "view.php?f={$forumlv->id}#p{$post->id}",
            $forumlv->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/view.php', array('f' => $forumlv->id));
        $url->set_anchor('p'.$event->objectid);
    ...
    public function test_post_deleted_postid_validation() {
        ...
        \mod_forumlv\event\post_deleted::create($params);
    }
    ...
    /**
     *  Ensure post_deleted event validates that the forumlvid is set.
     */
    public function test_post_deleted_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_deleted event validates that the forumlvtype is set.
     */
    public function test_post_deleted_forumlvtype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_deleted() {
        ...
        $discussionpost = $DB->get_records('forumlv_posts');
        ...
        $lastpost = end($posts);
        $sink = $this->redirectEvents();
        forumlv_delete_post($lastpost, true, $course, $cm, $forumlv);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the events contain the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\post_deleted', $event);
        $this->assertEquals(context_module::instance($forumlv->cmid), $event->get_context());
        $expected = array($course->id, 'forumlv', 'delete post', "discuss.php?d={$discussion->id}", $lastpost->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/discuss.php', array('d' => $discussion->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
        ...
        $sink = $this->redirectEvents();
        forumlv_delete_discussion($discussion, true, $course, $cm, $forumlv);
        $events = $sink->get_events();
        ...
        foreach ($events as $event) {
            ...
            $this->assertInstanceOf('\mod_forumlv\event\post_deleted', $event);
            $this->assertEquals(context_module::instance($forumlv->cmid), $event->get_context());
            $expected = array($course->id, 'forumlv', 'delete post', "discuss.php?d={$discussion->id}", $post->id, $forumlv->cmid);
            $this->assertEventLegacyLogData($expected, $event);
            $url = new \moodle_url('/mod/forumlv/discuss.php', array('d' => $discussion->id));
            $this->assertEquals($url, $event->get_url());
            $this->assertEventContextNotUsed($event);
    ...
    /**
     * Test post_deleted event for a single discussion forumlv.
     */
    public function test_post_deleted_single() {
        ...
        $this->assertInstanceOf('\mod_forumlv\event\post_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'delete post', "view.php?f={$forumlv->id}", $post->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/view.php', array('f' => $forumlv->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_post_updated_discussionid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'discussionid\' value must be set in other.');
        \mod_forumlv\event\post_updated::create($params);
    }

    /**
     *  Ensure post_updated event validates that the forumlvid is set.
     */
    public function test_post_updated_forumlvid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_updated event validates that the forumlvtype is set.
     */
    public function test_post_updated_forumlvtype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $forumlv = $this->getDataGenerator()->create_module('forumlv', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_updated() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'update post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $post->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/discuss.php', array('d' => $discussion->id));
        $url->set_anchor('p'.$event->objectid);
        $this->assertEquals($url, $event->get_url());
    ...
    public function test_post_updated_single() {
        ...
        $this->assertInstanceOf('\mod_forumlv\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'forumlv', 'update post', "view.php?f={$forumlv->id}#p{$post->id}",
            $post->id, $forumlv->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/forumlv/view.php', array('f' => $forumlv->id));
    ...
    public function test_discussion_subscription_created() {
        ...
        // Trigger the event by subscribing the user to the forumlv discussion.
        \mod_forumlv\subscriptions::subscribe_user_to_discussion($user->id, $discussion);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\discussion_subscription_created', $event);


        $cm = get_coursemodule_from_instance('forumlv', $discussion->forumlv);
    ...
    public function test_discussion_subscription_created_validation() {
        ...
        // The user is not subscribed to the forumlv. Insert a new discussion subscription.
        ...
        $event = \mod_forumlv\event\discussion_subscription_created::create($params);
    ...
    public function test_discussion_subscription_created_validation_contextlevel() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_forumlv\event\discussion_subscription_created::create($params);
    }
    ...
    /**
     * Test forumlvid validation of discussion_subscription_created event.
     */
    public function test_discussion_subscription_created_validation_forumlvid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/forumlv/lib.php');
    ...
    public function test_discussion_subscription_deleted() {
        ...
        // Trigger the event by unsubscribing the user to the forumlv discussion.
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_forumlv\event\discussion_subscription_deleted', $event);
    ...
    public function test_discussion_subscription_deleted_validation() {
        ...
        $subscription->forumlv = $forumlv->id;
        $subscription->discussion = $discussion->id;
        $subscription->preference = \mod_forumlv\subscriptions::FORUMLV_DISCUSSION_UNSUBSCRIBED;
        ...
        $event = \mod_forumlv\event\discussion_subscription_deleted::create($params);
    ...
    public function test_discussion_subscription_deleted_validation_contextlevel() {
        ...
        // Without an invalid context.
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_forumlv\event\discussion_subscription_deleted::create($params);
    }
    ...
    /**
     * Test forumlvid validation of discussion_subscription_deleted event.
     */
    public function test_discussion_subscription_deleted_validation_forumlvid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/forumlv/lib.php');
     ...
     * users.
     */
    public function test_forumlv_subscription_page_context_valid() {
        global $CFG, $PAGE;
        require_once($CFG->dirroot . '/mod/forumlv/lib.php');
        ...
        // Set up the default page event to use this forumlv.
        ...
        // Trigger the event by subscribing the user to the forumlv.
        ...
        // Trigger the event by unsubscribing the user to the forumlv.
    ...
    /**
     * Test mod_forumlv_observer methods.
     */
    public function test_observers() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $trackedrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => FORUMLV_INITIALSUBSCRIBE);
        $untrackedrecord = array('course' => $course->id, 'type' => 'general');
        $trackedforumlv = $this->getDataGenerator()->create_module('forumlv', $trackedrecord);
        $untrackedforumlv = $this->getDataGenerator()->create_module('forumlv', $untrackedrecord);
        ...
        // The role_assign observer does it's job adding the forumlv_subscriptions record.
        ...
        set_config('forumlv_trackingtype', 1);
        set_config('forumlv_trackreadposts', 1);

        $record = array();
        $record['course'] = $course->id;
        $record['forumlv'] = $trackedforumlv->id;
        $record['userid'] = $user->id;
        ...
        $record['discussion'] = $discussion->id;
        $record['userid'] = $user->id;
        $post = $forumlvgen->create_post($record);

        forumlv_tp_add_read_record($user->id, $post->id);
        forumlv_set_user_maildigest($trackedforumlv, 2, $user);
        forumlv_tp_stop_tracking($untrackedforumlv->id, $user->id);

        $this->assertEquals(1, $DB->count_records('forumlv_subscriptions'));
        $this->assertEquals(1, $DB->count_records('forumlv_digests'));
        $this->assertEquals(1, $DB->count_records('forumlv_track_prefs'));
        $this->assertEquals(1, $DB->count_records('forumlv_read'));

        // The course_module_created observer does it's job adding a subscription.
        $forumlvrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => FORUMLV_INITIALSUBSCRIBE);
        $extraforumlv = $this->getDataGenerator()->create_module('forumlv', $forumlvrecord);
        $this->assertEquals(2, $DB->count_records('forumlv_subscriptions'));

        $manplugin->unenrol_user($manualenrol, $user->id);

        $this->assertEquals(0, $DB->count_records('forumlv_digests'));
        $this->assertEquals(0, $DB->count_records('forumlv_subscriptions'));
        $this->assertEquals(0, $DB->count_records('forumlv_track_prefs'));
        $this->assertEquals(0, $DB->count_records('forumlv_read'));
    }
