...
class mod_forumlv_maildigest_testcase extends advanced_testcase {
    ...
    /**
     * Setup a user, course, and forumlvs.
     *
     * @return stdClass containing the list of forumlvs, courses, forumlvids,
    ...
    protected function helper_setup_user_in_course() {
        ...
        $return->forumlvs = new stdClass();
        $return->forumlvids = array();
        ...
        $return->forumlvs->forumlv1 = $this->getDataGenerator()->create_module('forumlv', $record);
        $return->forumlvsids[] = $return->forumlvs->forumlv1->id;

        $return->forumlvs->forumlv2 = $this->getDataGenerator()->create_module('forumlv', $record);
        $return->forumlvsids[] = $return->forumlvs->forumlv2->id;

        // Check the forumlv was correctly created.
        list ($test, $params) = $DB->get_in_or_equal($return->forumlvsids);
    ...
    /**
     * Helper to falsify all forumlv post records for a digest run.
     */
    protected function helper_force_digest_mail_times() {
        ...
        $DB->set_field('forumlv_posts', 'modified', $digesttime, array('mailed' => 0));
        $DB->set_field('forumlv_posts', 'created', $digesttime, array('mailed' => 0));
    }
    ...
    protected function helper_run_cron_check_count($expected, $individualcount, $digestcount) {
        ...
        foreach ($messages as $message) {
            if (strpos($message->subject, 'forumlv digest') !== false) {
                $counts->digest++;
            } else {
                $counts->individual++;
            }
    ...
    public function test_set_maildigest() {
        ...
        $forumlv1 = $helper->forumlvs->forumlv1;

        // Set to the user.
        self::setUser($helper->user);

        // Confirm that there is no current value.
        $currentsetting = $DB->get_record('forumlv_digests', array(
            'forumlv' => $forumlv1->id,
            'userid' => $user->id,
        ));
        ...
        forumlv_set_user_maildigest($forumlv1, 0, $user);
        $currentsetting = $DB->get_record('forumlv_digests', array(
            'forumlv' => $forumlv1->id,
            'userid' => $user->id,
        ));
        ...
        forumlv_set_user_maildigest($forumlv1, 1, $user);
        $currentsetting = $DB->get_record('forumlv_digests', array(
            'forumlv' => $forumlv1->id,
            'userid' => $user->id,
        ));
        ...
        forumlv_set_user_maildigest($forumlv1, 2, $user);
        $currentsetting = $DB->get_record('forumlv_digests', array(
            'forumlv' => $forumlv1->id,
            'userid' => $user->id,
        ));
        ...
        forumlv_set_user_maildigest($forumlv1, 42, $user);
    }
    ...
    public function test_get_user_digest_options_default() {
        ...
        $digestoptions = array(
            '0' => get_string('emaildigestoffshort', 'mod_forumlv'),
            '1' => get_string('emaildigestcompleteshort', 'mod_forumlv'),
            '2' => get_string('emaildigestsubjectsshort', 'mod_forumlv'),
        );

        // The default settings is 0.
        $this->assertEquals(0, $user->maildigest);
        $options = forumlv_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumlv', $digestoptions[0]));

        // Update the setting to 1.
        $USER->maildigest = 1;
        $this->assertEquals(1, $USER->maildigest);
        $options = forumlv_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumlv', $digestoptions[1]));

        // Update the setting to 2.
        $USER->maildigest = 2;
        $this->assertEquals(2, $USER->maildigest);
        $options = forumlv_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumlv', $digestoptions[2]));
    }
    ...
    public function test_cron_no_posts() {
        ...
        // Initially the forumlv cron should generate no messages as we've made no posts.
    ...
    public function test_cron_profile_single_mails() {
        ...
        $forumlv1 = $userhelper->forumlvs->forumlv1;
        $forumlv2 = $userhelper->forumlvs->forumlv2;

        // Add some discussions to the forumlvs.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;

        // Add 5 discussions to forumlv 1.
        $record->forumlv = $forumlv1->id;
        for ($i = 0; $i < 5; $i++) {
            $this->getDataGenerator()->get_plugin_generator('mod_forumlv')->create_discussion($record);
        }

        // Add 5 discussions to forumlv 2.
        ...
        // Set the maildigest preference for forumlv1 to default.
        forumlv_set_user_maildigest($forumlv1, -1, $user);

        // Set the maildigest preference for forumlv2 to default.
        forumlv_set_user_maildigest($forumlv2, -1, $user);

        // No digests mails should be sent, but 10 forumlv mails will be sent.
    ...
    public function test_cron_profile_digest_email() {
        ...
        // Add a discussion to the forumlvs.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-forumlv setting; and
    ...
    public function test_cron_mixed_email_1() {
        ...
        // Set the maildigest preference for forumlv1 to digest.
        forumlv_set_user_maildigest($forumlv1, 1, $user);

        // Set the maildigest preference for forumlv2 to default (single).
        forumlv_set_user_maildigest($forumlv2, -1, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * single e-mails from the per-forumlv setting; and
    ...
    public function test_cron_mixed_email_2() {
        ...
        // Set the maildigest preference for forumlv2 to single.
        forumlv_set_user_maildigest($forumlv2, 0, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-forumlv setting.
     */
    public function test_cron_forumlv_digest_email() {
        global $DB, $CFG;
        ...
        // Set the maildigest preference for forumlv1 to digest (complete).
        forumlv_set_user_maildigest($forumlv1, 1, $user);

        // Set the maildigest preference for forumlv2 to digest (short).
        forumlv_set_user_maildigest($forumlv2, 2, $user);
