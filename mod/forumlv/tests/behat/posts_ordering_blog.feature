...
Feature: Blog posts are always displayed in reverse chronological order
  In order to use forumlv as a blog
  As a user
  ...
  Background:
    ...
    And I add a "Forumlv" to section "1" and I fill the form with:
      | Forumlv name  | Course blog forumlv                               |
      | Description | Single discussion forumlv description             |
      | Forumlv type  | Standard forumlv displayed in a blog-like format  |
    And I log out
  ...
  Scenario: Replying to a blog post or editing it does not affect its display order
    ...
    And I follow "Course 1"
    And I follow "Course blog forumlv"
    #
    # Add three posts into the blog.
    #
    When I add a new topic to "Course blog forumlv" forumlv with:
      | Subject | Blog post 1             |
      | Message | This is the first post  |
    And I add a new topic to "Course blog forumlv" forumlv with:
      | Subject | Blog post 2             |
    ...
    #
    Then I should see "This is the third post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' forumlvpost ')][position()=1]" "xpath_element"
    And I should see "This is the second post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' forumlvpost ')][position()=2]" "xpath_element"
    And I should see "This is the first post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' forumlvpost ')][position()=3]" "xpath_element"
    #
