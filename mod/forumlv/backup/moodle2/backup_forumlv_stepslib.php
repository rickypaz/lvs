...
/**
 * Define all the backup steps that will be used by the backup_forumlv_activity_task
 */

/**
 * Define the complete forumlv structure for backup, with file and id annotations
 */
class backup_forumlv_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $forumlv = new backup_nested_element('forumlv', array('id'), array(
        ...
        $forumlv->add_child($trackedprefs);
        ...
        $forumlv->set_source_table('forumlv', array('id' => backup::VAR_ACTIVITYID));

        // All these source definitions only happen if we are including user info
        if ($userinfo) {
            $discussion->set_source_sql('
                SELECT *
                  FROM {forumlv_discussions}
                 WHERE forumlv = ?',
            ...
            $track->set_source_table('forumlv_track_prefs', array('forumlvid' => backup::VAR_PARENTID));

            $rating->set_source_table('rating', array('contextid'  => backup::VAR_CONTEXTID,
                                                      'component'  => backup_helper::is_sqlparam('mod_forumlv'),
        ...
        $forumlv->annotate_ids('scale', 'scale');
        ...
        $forumlv->annotate_files('mod_forumlv', 'intro', null); // This file area hasn't itemid

        $post->annotate_files('mod_forumlv', 'post', 'id');
        $post->annotate_files('mod_forumlv', 'attachment', 'id');

        // Return the root element (forumlv), wrapped into standard activity structure
        return $this->prepare_activity_structure($forumlv);
    }
